...
ntopng generates alerts to report the occurrence of events and user-configurable conditions.
...
User-configurable conditions can be set from the web GUI and are programmatically evaluated by ntopng. As soon as ntopng detects a certain condition is met, it immediately triggers the corresponding alert. Examples of conditions include:
...
Conditions can be configured for:

- Local hosts
- Interfaces
- Local networks

The following figure shows all the conditions that can be set on every interface.

.. figure:: ../img/basic_concepts_alerts_thresholds.png
  :align: center
  :alt: Interface Alerts with Configurable Conditions

  The View of Interface Alerts with Configurable Conditions


ntopng performs the evaluation of conditions at regular intervals of time:
...
Conditions can be set at different intervals simply by selecting the corresponding tab.

.. figure:: ../img/basic_concepts_alerts_intervals.png
  :align: center
  :alt: Condition Evaluation Intervals

  The Evaluation Intervals for Alert Conditions


Events vs. Conditions
---------------------

Alerts associated with a condition have a duration, that is, they are active for a certain period of time. This period of time starts then the condition is first met and stops when the condition is no longer met. For this reason, such alerts are said to be *engaged* or *released*, depending on whether the triggering condition is still met or not.


Engaged Alerts
~~~~~~~~~~~~~~

When the condition is first met, puts the corresponding alert in an *engaged* state. The set of alerts that are currently engaged is available from the engaged alerts page.
...
When the triggering condition of an engaged alert is no longer met, the alert is *released* an it will no longer be visible in the engaged alerts page. Alerts, once released, become available from the past alerts page and their duration is indicated in the corresponding column
...
Event and condition alerts are always associated with a severity that tells the importance of such occurrence. For example, the contact of a blacklisted host is emphasized with a *warning*, whereas a threshold crossed by an host is highlighted with an *error*. Currently, severity levels available are three:

- *Info*. Used for informative alerts, such as devices connections and disconnections, that don't directly represent any anomalous event or condition. Identified with a light blu badge.
- *Warning*. Used for alerts that deserve further investigation, such as a SYN probing. Identified with an orange badge.
- *Error*. Used with user-configurable conditions, for example a traffic threshold crossed by an host. Identified with a red badge.
...
- Other Entities
  ...
  - **TCP SYN Flood** (condition): Indicates whether an host is exceeding a configurable number of SYN per second. In the alert message it is also indicated if an host is a flooder of if it is a victim of a SYN flood.
  - **Flows Flood** (condition): Indicates whether an host is creating a number of flows that exceeds a configurable maximum number of flows per second. In the alert message it is also indicated if an host is a flooder of if it is a victim of a flow flood.
  - **Threshold Cross** (condition): Indicates whether an host, a network or an interface has crossed a configurable threshold. Thresholds can be configured from host, network and interface details page.
