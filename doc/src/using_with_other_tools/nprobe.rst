...
Encryption is now **enabled by default**: ntopng no longer accepts cleartext ZMQ traffic by default. If encryption is not explicitly configured by the user, ntopng (and nProbe) fall back to a public builtin default key pair. This default key only prevents accidental cleartext exposure and should be replaced by properly configuring a dedicated key pair.

In order to configure a dedicated key pair, the :code:`--zmq-encryption` option should be added to the configuration file. A private/public key pair is automatically generated by ntopng and the public key is displayed in the interface status page. Alternatively a specific private key can be supplied with :code:`--zmq-encryption-key-priv <key>`.
...
The public key should be configured in nProbe (the same applies to cento and n2disk when used as probes for ntopng) by using the :code:`--zmq-encryption-key '<pub key>'` option.
...
If encryption is not desired at all (e.g. on a fully trusted network, or in old deployments with nProbe instances exporting cleartext ZMQ), it can be turned off with :code:`--zmq-disable-encryption`.
