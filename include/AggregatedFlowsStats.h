...
class AggregatedFlowsStats {
 ...
 public:
  AggregatedFlowsStats(const IpAddress* c, const IpAddress* s,
                       u_int8_t _l4_proto, u_int64_t bytes_sent,
  ...
  inline u_int16_t getHostVLANId() { return (host ? host->getVLANId() : 0); };
  inline u_int16_t getCliVLANId() {
    return (client ? client->getVLANId() : 0);
  };
  inline u_int16_t getSrvVLANId() {
    return (server ? server->getVLANId() : 0);
  };
  ...
  inline const char* getHostIP(char* buf, u_int len) {
    return (host ? host->getIP(buf, len) : (char*)"");
  };
  inline const char* getCliIP(char* buf, u_int len) {
    return (client ? client->getIP(buf, len) : (char*)"");
  };
  inline const char* getSrvIP(char* buf, u_int len) {
    return (server ? server->getIP(buf, len) : (char*)"");
  };

  inline IpAddress* getClientIPaddr() {
    return (client ? client->getIPaddr() : NULL);
  }
  inline IpAddress* getServerIPaddr() {
    return (server ? server->getIPaddr() : NULL);
  }

  inline const char* getHostName(char* buf, u_int len) {
    return (host ? host->getHostName(buf, len) : (char*)"");
  };
  ...
  inline const char* getHostIPHex(char* buf, u_int len) {
  ...
  inline void setHost(IpAddress* _ip, Host* _host) {
    if (!host) {
      host = new (std::nothrow) FlowsHostInfo(_ip, _host);
    }
  };

  inline bool isHostInMem() { return (host->isHostInMem()); };
  inline bool isCliInMem() { return (client->isHostInMem()); };
  inline bool isSrvInMem() { return (server->isHostInMem()); };
  inline void setIsNotGuessed(bool isNotGuessed) {
    is_not_guessed = isNotGuessed;
  };
