...
class LocalHostStats: public HostStats {
 ...
 public:
  ...
  virtual void incSrvHostContacts(IpAddress *peer)    { peer->incCardinality(&num_host_contacts_as_server); peer->incCardinality(&contacts_as_srv); }
  virtual void incCliContactedHosts(IpAddress *peer)  { peer->incCardinality(&num_contacted_hosts_as_client); peer->incCardinality(&contacts_as_cli); }

  virtual void incContactedHosts(char *hostname)      { if(hostname && (hostname[0] != '\0')) num_contacted_hosts.addElement(hostname, strlen(hostname)); } /* Update the hosts contacts */
