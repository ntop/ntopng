...
class Redis {
 private:
  std::string redisVersion;
  std::map<std::string, std::vector<std::string>> listStore;
  std::map<std::string, std::set<std::string>> setStore;
  std::map<std::string, std::string> store;
  std::unique_ptr<StringFifoQueue> localToResolve;
  std::unique_ptr<StringFifoQueue> remoteToResolve;
  struct {
