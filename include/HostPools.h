...
class HostPools {
 private:
#ifdef NTOPNG_PRO
  time_t last_quota_reset[MAX_NUM_HOST_POOLS];
...
public:
  ...
  bool resetPoolStats(u_int16_t host_pool_id);
  ...
  inline time_t getLastQuotaResetTime(u_int16_t pool_id) {
    return((pool_id != NO_HOST_POOL_ID && pool_id < MAX_NUM_HOST_POOLS) ? last_quota_reset[pool_id] : 0);
  }
