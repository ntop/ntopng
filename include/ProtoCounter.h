...
class ProtoCounter {
 ...
 public:
  ProtoCounter(u_int16_t _proto_id, bool enable_throughput_stats, bool enable_behavior_stats);
  ~ProtoCounter();
  ...
  inline bool has_throughput_stats()      { return(
#ifdef NTOPNG_PRO
  ...
  inline bool has_behavior_stats()        { return(behavior_bytes_traffic ? true : false); }
						   
  void updateStats(const struct timeval *tv, time_t nextMinPeriodicUpdate);
  void incStats(u_int32_t when,
