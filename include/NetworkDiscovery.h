...
class NetworkDiscovery {
 private:
  ...
  u_int16_t buildMDNSDiscoveryDatagram(const char *query, u_int32_t sender_ip,
                                       u_int8_t *sender_mac, char *datagram,
                                       u_int datagram_len);
 ...
 public:
  ...
  void queueMDNSResponse(u_int32_t src_ip_nw_byte_order, u_char *buf,
                         u_int buf_len);
  void sendArpNetwork(void *data, u_int32_t netp, u_int32_t maskp,
                      u_int32_t sender_ip);
};
