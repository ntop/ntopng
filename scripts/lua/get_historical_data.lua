...
if stats_type == "top_talkers" then
   if not peer1 and not peer2 and not l7_proto_id then
	 ...
	 record["label"] = getResolvedHostAddress(hostkey2hostinfo(record["addr"]))
      end
...
elseif stats_type =="peers_traffic_histogram" and peer1 and peer2 then
   ...
   for _, record in pairs(res) do
      record["peer1_label"] = getResolvedHostAddress(hostkey2hostinfo(record["peer1_addr"]))
      record["peer2_label"] = getResolvedHostAddress(hostkey2hostinfo(record["peer2_addr"]))
   end
   -- tprint(res)
end
