...
if (host == nil) and (not only_historical) then
   ...
   if host["broadcast_domain_host"] then
      title = title .. " &nbsp;<i class='fas fa-sitemap' aria-hidden='true' title='" .. i18n("hosts_stats.label_broadcast_domain_host") ..
                 "'></i>"
   end
   ...
   page_utils.print_navbar(title, url, {{
      ...
      hidden = have_nedge or host["is_broadcast"] or host["is_multicast"] or only_historical or (total_packets_data == 0),
      active = page == "packets",
      page_name = "packets",
      label = i18n("packets")
   }, {
   ...
   if (((host["bytes.sent"] == 0) or (host["bytes.rcvd"] == 0)) and host["localhost"] and not host["is_multicast"] and
      not host["is_broadcast"]) then
      print("<div class=\"alert alert alert-warning\"><i class='fas fa-exclamation-triangle fa-lg fa-ntopng-warning'></i> " ..
  ...
  <div class="col-md-12 col-lg-12">
    <div class="mt-4 card card-shadow">
      ...
      if ((host["ip"] ~= nil)) then
         if (host["mac"] ~= "00:00:00:00:00:00") and (host["is_broadcast"] == false) then
            if (host.router ~= nil) then
               print("<tr><th width=35%>" .. i18n("details.router_access_point_mac_address") .. "</th><td colspan=2>" ..
                        get_symbolic_mac(host.router, false) .. " </td></tr>")
               print(
                  "<tr><th width=35%>" .. i18n("details.host_mac_address") .. "</th><td>" .. get_symbolic_mac(host["mac"], false) .. " " ..
                     discover.devtype2icon(host["device_type"]))
            else
               if (host.localhost) then
                  print("<tr><th width=35%>" .. i18n("details.mac_address") .. "</th><td>" .. get_symbolic_mac(host["mac"], false) .. " " ..
                           discover.devtype2icon(host["device_type"]))
               else
	    ...
	    local site_name = "(".. site.name .. ")"
         ...
         -- Labels row
         local host_tags_bitmap = interface.getHostTags(host_ip)
         if host_tags_bitmap and host_tags_bitmap ~= 0 then
            local all_labels = tag_badge_utils.getTags()
            local badges_html = ""
            for _, lbl in ipairs(all_labels) do
               -- Check if bit lbl.id is set (bits 32-63)
               if math.floor(host_tags_bitmap / (2 ^ lbl.id)) % 2 ~= 0 then
            ...
            if badges_html ~= "" then
               print('<tr><th>' .. i18n("tags_page.tags") .. '</th><td colspan="2">' .. badges_html .. '</td></tr>')
            end
         end
      else
      ...
      if ((host["asn"] ~= nil) and (host["asn"] > 0)) then
         ...
         print('<td><A class="ntopng-external-link" href="http://itools.com/tool/arin-whois-domain-search?q=' .. host["ip"] ..
                  ...
                  ' <i class="fas fa-external-link-alt"></i></A> <A class="ntopng-external-link" href="https://stat.ripe.net/' .. host["ip"] ..
                  '">' .. i18n("details.ripestat_lookup") .. ' <i class="fas fa-external-link-alt"></i></A></td>')
         print("</td></tr>\n")
      end
      ...
      if (host["ip"] ~= nil) then
         ...
         if ((host["privatehost"] == false) and (host["is_multicast"] == false) and (host["is_broadcast"] == false)) then
            print('  <A class="ntopng-external-link" href="https://www.virustotal.com/gui/ip-address/' .. host["ip"] ..
      ...
      if (host["num_alerts"] > 0) then
         print("<tr><th><i class=\"fas fa-exclamation-triangle\" style='color: #B94A48;'></i> " .. i18n("show_alerts.engaged_alerts") ..
                  "</th><td colspan=2></li>" .. hostinfo2detailshref(host, {
            page = "engaged-alerts"
      ...
      if (host["active_alerted_flows"] > 0) then
         print("<tr><th>" .. i18n("host_details.active_alerted_flows") .. "</th><td colspan=2></li>" .. hostinfo2detailshref(host, {
            page = "flows",
            flow_status = "alerted"
      ...
      if ((host.num_reset_flows ~= nil) and (host.num_reset_flows > 0)) then
         print("<tr><th>" .. i18n("host_details.attempted_reset_flows") .. "</th><td colspan=2></li>" .. "<span id=num_reset_flows>" ..
                  formatValue(host.num_reset_flows) .. "</span>" .. " <span id=flow_alerts_trend></span></td></tr>\n")
      end

      if (host.score_behaviour.tot_num_anomalies > 0) then
         -- TODO: Add JSON update
         print("<tr><th>" .. i18n("host_details.behavioural_anomalies") .. "</th><td colspan=2><span id=beh_anomalies>" ..
      ...
      print("<tr><th>" .. i18n("details.first_last_seen") .. "</th><td nowrap><span id=first_seen>" .. formatEpoch(host["seen.first"]) ..
               " [" .. secondsToTime(os.time() - host["seen.first"]) .. " " .. i18n("details.ago") .. "]" .. "</span></td>\n")
      print(
         "<td  width='35%'><span id=last_seen>" .. formatEpoch(host["seen.last"]) .. " [" .. secondsToTime(os.time() - host["seen.last"]) ..
      ...
      print("<tr><th>" .. i18n("details.traffic_sent_received") .. "</th><td><span id=pkts_sent>" .. formatPackets(host["packets.sent"]) ..
               "</span> / <span id=bytes_sent>" .. bytesToSize(host["bytes.sent"]) ..
               "</span> <span id=sent_trend></span></td><td><span id=pkts_rcvd>" .. formatPackets(host["packets.rcvd"]) ..
               "</span> / <span id=bytes_rcvd>" .. bytesToSize(host["bytes.rcvd"]) .. "</span> <span id=rcvd_trend></span></td></tr>\n")
      ...
      print("<tr><th>" .. flows_th .. "</th><td><span id=active_flows_as_client>" .. formatValue(host["active_flows.as_client"]) ..
      ...
      if host["tcp.packets.seq_problems"] == true then
         ...
         -- SENT ANALYSIS
         local tcp_retx_sent = "<span id=pkt_retransmissions_sent>" .. formatPackets(host["tcpPacketStats.sent"]["retransmissions"]) ..
                                  "</span> <span id=pkt_retransmissions_sent_trend></span>"
         local tcp_ooo_sent = "<span id=pkt_ooo_sent>" .. formatPackets(host["tcpPacketStats.sent"]["out_of_order"]) ..
         ...
         local tcp_keep_alive_sent = "<span id=pkt_keep_alive_sent>" .. formatPackets(host["tcpPacketStats.sent"]["keep_alive"]) ..
                                        "</span> <span id=pkt_keep_alive_sent_trend></span>"

         -- RCVD ANALYSIS
         local tcp_retx_rcvd = "<span id=pkt_retransmissions_rcvd>" .. formatPackets(host["tcpPacketStats.rcvd"]["retransmissions"]) ..
                                  "</span> <span id=pkt_retransmissions_rcvd_trend></span>"
         local tcp_ooo_rcvd = "<span id=pkt_ooo_rcvd>" .. formatPackets(host["tcpPacketStats.rcvd"]["out_of_order"]) ..
         ...
         local tcp_keep_alive_rcvd = "<span id=pkt_keep_alive_rcvd>" .. formatPackets(host["tcpPacketStats.rcvd"]["keep_alive"]) ..
                                        "</span> <span id=pkt_keep_alive_rcvd_trend></span>"

         print("<tr><th rowspan=2>" .. tcp_seq_label .. "</th><th>" .. i18n("sent") .. "</th><th>" .. i18n("received") .. "</th></tr>")
         print("<tr><td>" .. string.format("%s / %s / %s / %s", tcp_retx_sent, tcp_ooo_sent, tcp_lost_sent, tcp_keep_alive_sent) ..
                  "</td><td>" .. string.format("%s / %s / %s / %s", tcp_retx_rcvd, tcp_ooo_rcvd, tcp_lost_rcvd, tcp_keep_alive_rcvd) ..
                  "</td></tr>")
      end
   ...
   <button class="btn btn-secondary" onclick="$('#reset_blacklisted_stats_dialog').modal('show')">]]
      ...
      if num_extra_names > 0 then
         ...
         print('<tr><td width=35% rowspan=' .. (num_extra_names + 1) .. '><b>' .. i18n("details.further_host_names_information") ..
                  ' </a></b></td>')
      ...
      if (table.len(host.os_learning) > 0) then
         print('<tr><td width=35% rowspan=' .. (table.len(host.os_learning) + 1) .. '><b>' .. i18n("details.os_learning") ..
                  ' </a></b></td>')
      ...
      if host["device_ip"] then
         print('<tr><td width=35% rowspan=' .. (table.len(host["devices_ip"]) + 1) .. '><b>' .. i18n("details.probes_ipv4_address") ..
                  ' </a></b></td>')
   ...
   elseif ((page == "traffic")) then
      ...
      template.render("pages/vue_page.template", {
         vue_page_name = "PageHostDetailsTraffic",
         page_context = json_context
      })
         ...
         charts = {
            {
               ...
               update_url = http_prefix .. "/lua/rest/v2/get/interface/iface_ports_list.lua",
...
setInterval(update_icmp_table, 5000);
   ...
   elseif ((page == "ndpi")) then
      ...
      local context = {
         ...
         view = "applications",
         is_ch_enabled = hasClickHouseSupport(),
      ...
      local json_context = json.encode(context)
      template.render("pages/vue_page.template", {
         vue_page_name = "PageHostDetailsApplications",
         page_context = json_context
      })
   ...
   elseif (page == "dns") then
      if ((host.DoH_DoT ~= nil) or (host["dns"] ~= nil)) then
         ...
         if (host.DoH_DoT ~= nil) then
            print("<tr><th>" .. i18n("dns_page.doh_dot_servers") .. "</th><th colspan=4>" .. i18n("dns_page.doh_dot_server_uses") ..
                     "</th></tr>")

            for _, v in pairs(host.DoH_DoT) do
               print("<tr><th>" .. buildHostHREF(v.ip, v.vlan_id, "overview") .. "</th><td colspan=4 align=right>" ..
         ...
         if (host["dns"] ~= nil) then
            print("<tr><th>" .. i18n("dns_page.dns_breakdown") .. "</th><th class='text-end'>" .. i18n("dns_page.queries") ..
                     "</th><th class='text-end'>" .. i18n("dns_page.positive_replies") .. "</th><th class='text-end'>" ..
                     i18n("dns_page.error_replies") .. "</th><th colspan=2 class='text-center'>" .. i18n("dns_page.reply_breakdown") ..
                     "</th></tr>")
            print("<tr><th>" .. i18n("sent") .. "</th><td class=\"text-end\"><span id=dns_sent_num_queries>" ..
                     formatValue(host["dns"]["sent"]["num_queries"]) .. "</span> <span id=trend_sent_num_queries></span></td>")

            print("<td class=\"text-end\"><span id=dns_sent_num_replies_ok>" .. formatValue(host["dns"]["sent"]["num_replies_ok"]) ..
                     "</span> <span id=trend_sent_num_replies_ok></span></td>")
            print("<td class=\"text-end\"><span id=dns_sent_num_replies_error>" .. formatValue(host["dns"]["sent"]["num_replies_error"]) ..
            ...
            print("<tr><th>" .. i18n("dns_page.rcvd") .. "</th><td class=\"text-end\"><span id=dns_rcvd_num_queries>" ..
                     formatValue(host["dns"]["rcvd"]["num_queries"]) .. "</span> <span id=trend_rcvd_num_queries></span></td>")
            print("<td class=\"text-end\"><span id=dns_rcvd_num_replies_ok>" .. formatValue(host["dns"]["rcvd"]["num_replies_ok"]) ..
                     "</span> <span id=trend_rcvd_num_replies_ok></span></td>")
            print("<td class=\"text-end\"><span id=dns_rcvd_num_replies_error>" .. formatValue(host["dns"]["rcvd"]["num_replies_error"]) ..
            ...
            if host["dns"]["rcvd"]["num_replies_ok"] + host["dns"]["rcvd"]["num_replies_error"] > 0 then
               ...
               local dns_ratio = tonumber(host["dns"]["sent"]["num_queries"]) /
                                    tonumber(host["dns"]["rcvd"]["num_replies_ok"] + host["dns"]["rcvd"]["num_replies_error"])
               ...
               graph_utils.breakdownBar(host["dns"]["sent"]["num_queries"], i18n("dns_page.queries"),
                  host["dns"]["rcvd"]["num_replies_ok"] + host["dns"]["rcvd"]["num_replies_error"], i18n("dns_page.replies"), 30, 70)
   ...
   elseif (page == "http") then
      ...
      if http then
         ...
         if http["sender"]["query"]["total"] > 0 then
            print("<tr><th rowspan=6 width=20%><A HREF='http://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol#Request_methods'>" ..
                     i18n("http_page.http_queries") .. "</A></th><th width=20%>" .. i18n("http_page.method") ..
                     "</th><th width=20% class='text-end'>" .. i18n("http_page.requests") .. "</th><th colspan=2 class='text-center'>" ..
            ...
            print("<tr><th>HEAD</th><td style=\"text-align: right;\"><span id=http_query_num_head>" ..
                     formatValue(http["sender"]["query"]["num_head"]) .. "</span> <span id=trend_http_query_num_head></span></td></tr>")
            print("<tr><th>PUT</th><td style=\"text-align: right;\"><span id=http_query_num_put>" ..
                     formatValue(http["sender"]["query"]["num_put"]) .. "</span> <span id=trend_http_query_num_put></span></td></tr>")
            print("<tr><th>" .. i18n("http_page.other_method") .. "</th><td style=\"text-align: right;\"><span id=http_query_num_other>" ..
                     formatValue(http["sender"]["query"]["num_other"]) .. "</span> <span id=trend_http_query_num_other></span></td></tr>")
         end
         ...
         if vh then
            ...
            if (num > 0) then
               ...
               print("<tr><th rowspan=" .. (num + 1) .. " width=20%>" .. i18n("http_page.virtual_hosts") .. "</th><th>Name</th><th>" ..
                        i18n("http_page.traffic_sent") .. "</th><th>" .. i18n("http_page.traffic_received") .. "</th><th>" ..
                        i18n("http_page.requests_served") .. "</th></tr>\n")
               for k, v in pairsByKeys(vh, asc) do
                  ...
                  print("<td align=right><span id=" .. j .. "_num_vhost_req_serv>" .. formatValue(vh[k]["http.requests"]) ..
                           "</span></td></tr>\n")
               end
            end
         end
   ...
   elseif (page == "contacts") then
      ...
      if (num > 0) then
         ...
         if (cnum == 0) then
            ...
            print("<tr><th width=75%>" .. i18n("contacts_page.server_address") .. "</th><th>" .. i18n("contacts_page.contacts") ..
                     "</th></tr>\n")
         ...
         if (snum == 0) then
            ...
            print("<td><table class=\"table table-bordered table-striped\">\n")
            print("<tr><th width=75%>" .. i18n("contacts_page.client_address") .. "</th><th>" .. i18n("contacts_page.contacts") ..
                     "</th></tr>\n")
      ...
      if (vh ~= nil) then
         ...
         for i, method in ipairs(methods) do
            ...
            print('} else {\n\tlast_http_query_num_' .. method .. ' = http["sender"]["query"]["num_' .. method ..
                     '"];$("#trend_http_query_num_' .. method .. '").html(\'<i class=\"fas fa-arrow-up\"></i>\'); }\n')
         end
         ...
         for i, retcode in ipairs(retcodes) do
            ...
            print('} else {\n\tlast_http_response_num_' .. retcode .. ' = http["receiver"]["response"]["num_' .. retcode ..
                     '"];$("#trend_http_response_num_' .. retcode .. '").html(\'<i class=\"fas fa-arrow-up\"></i>\'); }\n')
         end
      end
   end
