...
local function create_rrd(schema, path, timestamp)
   ...
   if isDebugEnabled() then
      traceError(TRACE_NORMAL, TRACE_CONSOLE, string.format("ntop.rrd_create(%s) schema=%s", table.concat(params, ", "), schema.name))
   end
   ...
   if ((last_update ~= nil) and (timestamp <= last_update)) then
      if isDebugEnabled() then
         traceError(TRACE_NORMAL, TRACE_CONSOLE,
            string.format("Skip RRD update in the past: timestamp=%u but last_update=%u", timestamp, last_update))
      end
...
local function touchRRD(rrdname)
   ...
   if ((last ~= nil) and ((now - last) > 3600)) then
      ...
      if isDebugEnabled() then
         traceError(TRACE_NORMAL, TRACE_CONSOLE, string.format("touchRRD(%s, %u), last_update was %u", rrdname, tdiff, last))
         tprint(debug.traceback())
      end
...
function driver:timeseries_query(options)
   ...
   if ((options.epoch_end > last_update) and ((options.epoch_end - last_update) <= 2 * options.schema_info.options.step)) then
   ...
   if count > options.max_num_points then
      sampled_fstep, count, series = sampleSeries(options.schema_info, count, fstep, options.max_num_points, series, consolidation)
   end
...
function driver:query(schema, tstart, tend, tags, options)
   ...
   if isDebugEnabled() then
      traceError(TRACE_NORMAL, TRACE_CONSOLE, string.format("RRD_FETCH schema=%s %s -> (%s): last_update=%u", schema.name,
         table.tconcat(tags, "=", ","), rrdfile, last_update))
   end
   ...
   if options.initial_point then
      local serie_idx = 1
      local _, _, initial_pt, _, _, names = ntop.rrd_fetch_columns(rrdfile, getConsolidationFunction(schema), tstart - schema.options.step,
   ...
   for f in pairs(files or {}) do
      ...
      if ((v ~= nil) and (#v == 1)) then
         ...
         if last_update ~= nil and start_time ~= nil and last_update >= start_time then
            ...
            elseif ((wildcard_tag ~= "protocol") or ((L4_PROTO_KEYS[value] == nil) and interface.getnDPIProtoId(value) ~= 0)) and
         ...
         for f in pairs(files or {}) do
            ...
            if last_update ~= nil and last_update >= start_time then
               ...
               elseif #wildcard_tags == 1 then
                  ...
                  traceError(TRACE_ERROR, TRACE_CONSOLE, "RRD driver does not support listSeries on more then 2 tags")
                  break
               end
...
function driver:timeseries_top(options, top_tags)
   ...
   if table.len(top_series) == 0 then
      return nil
   end
...
function driver:topk(schema, tags, tstart, tend, options, top_tags)
   ...
   for _, serie_tags in pairs(series) do
      ...
      if isDebugEnabled() then
         traceError(TRACE_NORMAL, TRACE_CONSOLE,
            string.format("RRD_FETCH[topk] schema=%s %s[%s] -> (%s): last_update=%u", schema.name, table.tconcat(tags, "=", ","),
               table.concat(top_tags, ","), rrdfile, ntop.rrd_lastupdate(rrdfile)))
      end
...
function driver:delete(schema_prefix, tags)
   ...
   if isDebugEnabled() then
      traceError(TRACE_NORMAL, TRACE_CONSOLE,
         string.format("DELETE schema=%s, %s => %s", schema_prefix, table.tconcat(tags, "=", ","), path_to_del))
   end
