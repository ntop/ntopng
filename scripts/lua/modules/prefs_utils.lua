...
DNS_PRESETS = {{
    ...
    primary_dns = "8.26.56.26",
    secondary_dns = "8.20.247.20"
}, {
    id = "dyn_internet_guide",
    label = "Dyn Internet Guide",
    url = "http://dyn.com/labs/dyn-internet-guide/",
    primary_dns = "216.146.35.35",
    secondary_dns = "216.146.36.36"
},
...
-- {id="greenteam_internet", label="GreenTeam Internet", url="http://members.greentm.co.uk/", primary_dns="81.218.119.11", secondary_dns="209.88.198.133"},
    ...
    secondary_dns = "208.67.220.123",
    child_safe = true
}, {
    id = "quad9_security",
    label = "Quad 9: Security",
    url = "https://quad9.net",
    primary_dns = "9.9.9.9",
    secondary_dns = ""
}, {
...
function isSubpageAvailable(subpage, show_advanced_prefs)
    ...
    if (subpage.hidden) or ((subpage.advanced) and (not show_advanced_prefs)) or
        ((subpage.pro_only) and (not ntop.isPro())) or
        (subpage.enterprise_l_only and (not info["version.enterprise_l_edition"]) and
            (not info["version.nedge_enterprise_edition"])) or
...
function printMenuSubpages(tab)
    ...
    for _, subpage in ipairs(menu_subpages) do
        if not subpage.hidden then
            local url = ternary(subpage.disabled, "#",
                ntop.getHttpPrefix() .. [[/lua/admin/prefs.lua?tab=]] .. (subpage.id))
            print [[<a href="]]
            print(url)
...
function notifyNtopng(key, value)
    ...
    if string.starts(key, 'toggle_logging_level') then
        if verbose then
            io.write('notifying ntopng upon logging level pref change\n')
        end
...
function prefsResolutionButtons(fmt, value, fixed_id, format_spec, max_val)
    ...
    local res = makeResolutionButtons(format_spec or FMT_TO_DATA_TIME, ctrl_id, fmt, value, {
        classes = {"float-right"}
    }, max_val)
...
function prefsInputFieldPrefs(label, comment, prekey, key, default_value, _input_type, showEnabled, disableAutocomplete,
    ...
    if extra.required == true then
        attributes["required"] = ""
    end
    if extra.pattern ~= nil then
        attributes["pattern"] = extra.pattern
    end
    ...
    if _input_type ~= nil then
        input_type = _input_type
    end
    print('<tr id="' .. key .. '" style="display: ' .. showEnabled .. ';"><td width=50%><strong>' .. label ..
              '</strong><p><small>' .. comment .. '</small></td>')
    ...
    print(table.tconcat(attributes, "=", " ", nil, '"'))
    print [[ name="]]
    print(key)
    print [[" style="]]
    print(table.tconcat(style, ":", "; ", ";"))
    print [[" value="]]
    print(value .. '"')
    if disableAutocomplete then
        print(" autocomplete=\"off\"")
    end
    print [[/>]]
    print(extra.append or "")
    print [[
          </td>
        </tr>
        <tr>
...
function prefsDropdownFieldPrefs(label, comment, key, values, default_value, showEnabled, extra)
    ...
    if extra.save_pref and table.len(_POST) > 0 and isEmptyString(_POST[key]) == false and isEmptyString(extra.pref_key) ==
        false then
    ...
    if extra.keys == nil then
        ...
        for idx, _ in pairs(values) do
            ...
            print("<option value=" .. key .. " " .. ternary(key == default_value, "selected", "") .. ">" .. val ..
                      "</option>")
        end
    end
    print [[
...
function toggleTableButton(label, comment, on_label, on_value, on_color, off_label, off_value, off_color, submit_field,
    redis_key, disabled)
    ...
    if (label ~= "") then
        print('<tr><td width=50%><strong>' .. label .. '</strong><p><small>' .. comment ..
...
local function printNestedToSwitchLogic(nested_to_switch, submit_field)
    ...
    for _, item in ipairs(nested_to_switch) do
      ...
      if(($("#]]
        print(submit_field)
        print [[").val() == "]]
        print(item.pref_enabled_value or "1")
        print [[") &&
         ($("#]]
        print(item.parent)
        print [[").val() == "]]
        print(item.parent_enabled_value)
        print [["))
        $("#]]
        print(item.input)
...
local function toggleTableButtonPrefs(label, comment, on_label, on_value, on_color, off_label, off_value, off_color,
    submit_field, redis_key, default_value, disabled, elementToSwitch, hideOn, showElement, nested_to_switch)
    ...
    if (label ~= "") then
        print('<tr id="row_' .. submit_field .. '"' .. objRow .. '><td width=50%><strong>' .. label ..
                  '</strong><p><small>' .. comment .. '</small></td><td align=right>\n')
    end

    print([[
    <div class="form-switch">
      <input ]] .. (value == off_value and '' or 'checked') .. [[ type="checkbox" class="form-check-input" id="check-]] ..
              submit_field .. [[">
...
function prefsToggleButton(subpage_active, params)
    ...
    return toggleTableButtonPrefs(params.title or subpage_active.entries[options.field].title,
        (params.description or subpage_active.entries[options.field].description) ..
            (params.content or subpage_active.entries[options.field].content or ""), options.on_text, options.on_value,
        options.on_class, options.off_text, options.off_value, options.off_class, options.field, redis_key,
        options.default, options.disabled, options.to_switch, options.reverse_switch, not options.hidden,
        options.nested_to_switch)
end

function multipleTableButtonPrefs(label, comment, array_labels, array_values, default_value, selected_color,
    submit_field, redis_key, disabled, elementToSwitch, showElementArray, nested_to_switch, showElement, initialValue,
    toggleElementArray)
    local value
    local disabled_attr
    ...
    if (value ~= nil) then
        ...
        for nameCount = 1, #array_labels do
            ...
            print('<input type="radio" name="' .. submit_field .. '" id="id_' .. submit_field .. '_' ..
                      array_values[nameCount] .. '" value="' .. array_values[nameCount] ..
                      '" class="btn-check btn-primary" ' .. disabled_attr .. '>\n')
            print('<label class="btn btn-sm ' .. type_button .. '"  for="id_' .. submit_field .. '_' ..
                      array_values[nameCount] .. '">' .. array_labels[nameCount] .. '</label>\n')
        end
        print('</div>\n')
        print('<script>\n')
        ...
        for nameCount = 1, #array_labels do
            ...
            for indexLabel = 1, #array_labels do
                ...
                print [[ let class_]]
                print(array_values[indexLabel])
                print [[ = $(`label[for="id_]]
                print(submit_field .. '_')
                print(array_values[indexLabel])
                print [["]`);
        class_]]
                print(array_values[indexLabel])
                print [[.removeAttr("class");]]
                if (array_values[indexLabel] == array_values[nameCount]) then
                    print [[class_]]
                    print(array_values[indexLabel])
                    print [[.attr("class", "btn btn-sm btn-]]
                    print(color)
                    print [[ active");]]
                else
                    print [[class_]]
                    print(array_values[indexLabel])
                    print [[.attr("class", "btn btn-sm btn-secondary");]]
                end
            end
...
function loggingSelector(label, comment, submit_field, redis_key)
    ...
    for i, v in ipairs(logging_values) do
        logging_keys[i] = firstToUpper(v)
    end

    multipleTableButtonPrefs("Log level", "Choose the runtime logging level.", logging_keys, logging_values, value,
        color_map, submit_field, redis_key)
...
function printSaveButton(some_content)
    print('<tr><td colspan=2 style="text-align:right;">' .. (some_content or "") ..
