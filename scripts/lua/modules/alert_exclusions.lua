...
local function _check_alert_key(alert_key, is_flow_exclusion)
    local alert_entities = require("alert_entities")
    local alert_entity_id = alert_entities.flow.entity_id

    if not is_flow_exclusion then
        alert_entity_id = alert_entities.host.entity_id
    end

    if not alert_consts.getAlertType(tonumber(alert_key), tonumber(alert_entity_id)) then
...
local function _toggle_alert_exclusion(subject_key, subject_type, alert_key, add_exclusion, is_flow_exclusion)
    ...
    if not _check_alert_key(alert_key, is_flow_exclusion) and alert_key ~= alert_consts.ALL_ALERT_KEY then
    ...
    if locked then
        ...
        if add_exclusion then
            ...
            if exclusions[subject_key] then
                ...
                if (exclusions[subject_key].flow_alerts == nil or table.len(exclusions[subject_key].flow_alerts) == 0) and
                    (exclusions[subject_key].host_alerts == nil or table.len(exclusions[subject_key].host_alerts) == 0) then
