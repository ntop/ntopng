...
function getFirstInterfaceId()
    ...
    if ifid ~= nil then return ifid, getInterfaceName(ifid) end
...
function getPageUrl(base_url, params)
    if table.empty(params) then return base_url end

    local encoded = {}

    for k, v in pairs(params) do encoded[k] = urlencode(v) end

    local delim = "&"
    if not string.find(base_url, "?") then delim = "?" end
...
function getProbeName(exporter_ip, show_vlan, shorten_len, show_ip_and_alias)
    ...
    if tonumber(exporter_ip) then exporter_ip = ntop.inet_ntoa(exporter_ip) end

    if (exporter_ip ~= nil) then
        local ret = cache_utils.gethostname(exporter_ip)

        if (not isEmptyString(ret)) then return (ret) end
    else
        return (exporter_ip)
    end
    return exporter_ip
...
function getTopInterfaceHosts(howmany, localHostsOnly)
    ...
    for k, v in pairs(hosts_stats) do
        if ((not localHostsOnly) or
            ((v["localhost"] == true) and (v["ip"] ~= nil))) then
...
function getApplicationLabel(name, maxlen)
    ...
    if (maxlen == nil) then maxlen = 12 end
...
function getHostNotes(host_info)
    ...
    if not isEmptyString(notes) then notes = string.lower(notes) end
...
function getDeviceName(device_mac, skip_manufacturer)
    ...
    if name == device_mac then
        ...
        local info = interface.getHostsInfo(false, nil, 1, 0, nil, nil, nil,
                                            tonumber(vlan), nil, nil, device_mac)
        ...
        if (info ~= nil) then
            for x, host in pairs(info.hosts) do
                ...
                if host.broadcast_domain_host and not isEmptyString(host.name) and
                    host.name ~= host.ip and host.name ~= "NoIP" then
                    name = host.name
                elseif host.ip ~= "0.0.0.0" then
                    name = ip2label(host.ip)
                    if name == host.ip then name = nil end
                end
                break
            end
        else
    ...
    if isEmptyString(name) or name == device_mac then return '' end
...
function getInterfaceSpeed(ifid)
    ...
    local ifspeed = ntop.getCache('ntopng.prefs.ifid_' .. tostring(ifid) ..
                                      '.speed')
...
function getFlag(country)
    if ((country == nil) or (country == "")) then
        ...
        return (" <a href='" .. ntop.getHttpPrefix() ..
                   "/lua/hosts_stats.lua?country=" .. country .. "'><img src='" ..
                   ntop.getHttpPrefix() ..
                   "/dist/images/blank.gif' class='flag flag-" ..
                   string.lower(country) .. "'></a> ")
    end
end
...
function getLicensePattern() return [[^[a-zA-Z0-9\+/=]+$]] end
...
function getMacPattern() return "^([0-9a-fA-F][0-9a-fA-F]:){5}[0-9a-fA-F]{2}$" end

-- ##############################################

function getURLPattern() return "^https?://.+$" end
...
function getRestUrl(script, is_pro, is_enterprise)
    if is_enterprise then
        return
            (ntop.getHttpPrefix() .. "/lua/enterprise/rest/v2/get/" .. script)
    elseif is_pro then
...
local magic_short_macs = {["01:00:5E"] = "IPv4mcast", ["33:33:"] = "IPv6mcast"}
        ...
        if (magic_short_macs[m] ~= nil) then
            if (add_extra_info == true) then
                ...
                if no_href then
                    ...
                    return (macInfoWithSymbName(mac_address,
                                                magic_short_macs[m] .. "_" .. t))
                end
            end
        else
            local s = get_mac_classification(m)
            ...
            if (m == s) then
                if no_href then
                    ...
                    return '<a href="' .. ntop.getHttpPrefix() ..
                               '/lua/mac_details.lua?host=' .. mac_address ..
                               '">' .. get_mac_classification(m) .. ":" .. t ..
                               '</a>'
                end
            else
                local href = ""
                ...
                if not no_href then
                    href = '<a href="' .. ntop.getHttpPrefix() ..
                               '/lua/mac_details.lua?host=' .. mac_address ..
                               '">'
                    href_end = "</a>"
                end
                ...
                if (add_extra_info == true) then
                    return (href .. get_mac_classification(m) .. "_" .. t ..
                               " (" .. macInfo(mac_address) .. ")" .. href_end)
                else
                    return (href .. get_mac_classification(m) .. "_" .. t ..
                               href_end)
                end
...
function get_manufacturer_mac(mac_address)
    ...
    if not isEmptyString(ret) then ret = ret:gsub("'", " ") end
...
function getSafeChildIcon()
    return
        ("&nbsp;<font color='#5cb85c'><i class='fas fa-lg fa-child' aria-hidden='true'></i></font>")
end
...
function getGwMacsSet(ifid) return "ntopng.prefs.iface_" .. ifid .. ".gw_macs" end
...
function getFolderSize(path, timeout)
    ...
    if ntop.isWindows() then
        ...
        if isEmptyString(has_timeout) then
            ...
            has_timeout = (os_utils.execWithOutput(
                              "which timeout >/dev/null 2>&1") ~= nil)
        ...
        if size == nil then
            ...
            local periodic_activities_utils =
                require "periodic_activities_utils"
            if ntop.isdir(path) and
                not periodic_activities_utils.have_degraded_performance() then
                local du_cmd = string.format("du -s %s 2>/dev/null", path)
                if has_timeout then
                    du_cmd = string.format("timeout %u%s %s", MAX_TIMEOUT, "s",
                                           du_cmd)
                end

                -- use POSIXLY_CORRECT=1 to guarantee results is returned in 512-byte blocks
                -- both on BSD and Linux
                local line = os_utils.execWithOutput(string.format(
                                                         "POSIXLY_CORRECT=1 %s",
                                                         du_cmd))
                ...
                if #values >= 1 then
                    ...
                    if used ~= nil then
                        ...
                        ntop.setHashCache("ntopng.cache.folder_size", path,
                                          now .. "," .. size)
                    end
                end
            end
...
function getFlowDevAliasKey() return "ntopng.flow_dev_aliases" end
...
function getObsPointAliasKey() return "ntopng.observation_point_aliases" end
...
function getObsPointAlias(observation_point_id, add_id, add_href)
    ...
    if (add_href == true) then
        ret = "<A HREF=\"" .. ntop.getHttpPrefix() ..
                  "/lua/pro/enterprise/observation_points.lua\">" .. ret ..
                  "</A>"
    end
...
function getFullObsPointName(observation_point_id, compact, add_id)
    ...
    if not isEmptyString(observation_point_id) then
        if not isEmptyString(observation_point_id) and alias ~=
            tostring(observation_point_id) then
            if compact then
                alias = shortenString(alias)
                return string.format("%s", alias)
            else
...
function getExtraFlowInfoServerName(alert_json)
    if alert_json then
        ...
        elseif alert_json["proto"] and alert_json["proto"]["tls"] and
            not isEmptyString(
                alert_json["proto"]["tls"]["client_requested_server_name"]) then
...
function get_badge(info)
    ...
    if info ~= true and info ~= 0 then badge = 'danger' end
