...
local function format_post_nat_info(flow, info)
	...
	-- No Post-NAT values
	if
		not nat_values.post_nat_dst_port
		and not nat_values.post_nat_src_port
		and not nat_values.post_nat_dst_ip
		and not nat_values.post_nat_src_ip
	then
		return flow
	end
...
function historical_flow_details_formatter.format_historical_total_traffic(flow)
	return {
		...
		values = {
			formatPackets(flow["PACKETS"] or flow["packets"]) .. " / " .. bytesToSize(
				flow["TOTAL_BYTES"] or flow["total_bytes"]
			),
		},
	}
end
...
function historical_flow_details_formatter.format_qoe(flow)
	...
	if
		json_info
		and json_info.qoe
		and ntop.isEnterpriseL
		and ntop.isEnterpriseL()
		and tonumber(flow.QOE_SCORE) <= 100
	then
...
function historical_flow_details_formatter.format_historical_bytes_progress_bar(flow, info)
	local cli2srv = round(
		((flow["SRC2DST_BYTES"] or flow["cli2srv_bytes"] or 0) * 100) / (flow["TOTAL_BYTES"] or flow["total_bytes"]),
		0
	)
	...
	return {
		name = "",
		values = {
			format_utils.createBreakdown(cli2srv, 100 - cli2srv, info.cli_ip.label or "", info.srv_ip.label or ""),
		},
	}
end
...
local function format_historical_issue_description(
	alert,
	alert_id,
	score,
	title,
...
function historical_flow_details_formatter.format_historical_issues(flow_details, flow, is_alert)
	...
	if alert_json and alert_json.alerts then
		...
		if alert_id ~= 0 then
			...
			html = html
				.. format_historical_issue_description(
					alert,
					tostring(alert_id),
					tonumber(main_alert_score),
					i18n("issues_score"),
					alert_label,
					details,
					alert_scores,
...
local function format_tcp_connection_states(info)
	local conn_states = {}
	conn_states[#conn_states + 1] = string.format(
		"%s: %s (%s)",
		i18n("flow_fields_description.major_connection_state"),
	...
	conn_states[#conn_states + 1] = string.format(
		"%s: %s (%s)",
		i18n("flow_fields_description.minor_connection_state"),
...
local function format_historical_community_id(flow, community_id)
	...
	if not isEmptyString(community_id) then
		cid = community_id
	end
	return {
		name = "<A class='ntopng-external-link' href=\"https://github.com/corelight/community-id-spec\">" .. i18n(
			"db_explorer.community_id"
...
local function format_historical_info(flow)
	local historical_flow_utils = require("historical_flow_utils")
	local info_field =
		historical_flow_utils.get_historical_url(flow["INFO"], "info", flow["INFO"], true, flow["INFO"], true)
...
local function format_historical_probe(flow_details, flow, info)
	...
	local info_field = {
		device_ip = historical_flow_utils.get_historical_url(
			name,
			"exporter_ip",
			info["exporter_ip"]["value"],
			true,
			info["exporter_ip"]["title"]
		),
	}
...
local function format_historical_custom_fields(flow_details, custom_fields)
	if table.len(custom_fields) > 0 then
		...
		for key, value in pairs(custom_fields) do
			...
			if not isEmptyString(nprobe_descr) then
...
local function format_historical_flow_traffic_stats(
	rowspan,
	cli2srv_retr,
	srv2cli_retr,
	cli2srv_ooo,
	...
	if rowspan > 0 then
		flow_details[#flow_details + 1] = {
			...
			values = {
				"",
				i18n("client") .. ' <i class="fas fa-long-arrow-alt-right" ></i> ' .. i18n("server") .. " / " .. i18n(
					"client"
		...
		if (cli2srv_retr and (tonumber(cli2srv_retr) > 0)) or (srv2cli_retr and (tonumber(srv2cli_retr) > 0)) then
			flow_details[#flow_details + 1] = {
				...
				values = {
					...
					formatPackets(cli2srv_retr) .. " / " .. formatPackets(srv2cli_retr),
				},
			}
		end
		if (cli2srv_ooo and (tonumber(cli2srv_ooo) > 0)) or (srv2cli_ooo and (tonumber(srv2cli_ooo) > 0)) then
			flow_details[#flow_details + 1] = {
				...
				values = {
					...
					formatPackets(cli2srv_ooo) .. " / " .. formatPackets(srv2cli_ooo),
				},
			}
		end
		if (cli2srv_ooo and (tonumber(cli2srv_ooo) > 0)) or (srv2cli_ooo and (tonumber(srv2cli_ooo) > 0)) then
...
local function format_historical_labels(flow)
	...
	if isEmptyString(tags_map_hex) then
		return nil
	end
	local bitmap = tonumber(tags_map_hex, 16)
	if not bitmap or bitmap == 0 then
		return nil
	end
	local badges_html = ""
	...
	if isEmptyString(badges_html) then
		return nil
	end
	return {
...
local function format_historical_flow_additional_exporter(exporters, cli_ip, srv_ip)
	...
	for _, idx in ipairs(ordered) do
		...
		if exp then
			...
			exporter_url =
				historical_flow_utils.get_historical_url(exporter_name, "exporter_ip", exporter_ip, true, exporter_name)
			...
			if exporter_ip then
				...
				if (from_ip ~= nil) and (to_ip ~= nil) then
					if flow_trajectory[from_ip] == nil then
						flow_trajectory[from_ip] = {}
					end
					if flow_trajectory[to_ip] == nil then
						flow_trajectory[to_ip] = {}
					end
					...
					if next_hop_ip ~= "0.0.0.0" then
						...
						if not valueFound(flow_trajectory[to_ip], next_hop_ip) then
							table.insert(
								flow_trajectory[to_ip],
								{ next_hop = next_hop_ip, return_path = exp.return_path }
							)
						end
						...
						if exp.next_hop then
							...
							if exp.next_hop_ip ~= nil then
								if flow_trajectory[exp.next_hop_ip] == nil then
									flow_trajectory[next_hop_ip] = {}
								end

								if exp.return_path then
									if
										(not valueFound(flow_trajectory[exp.next_hop_ip], srv_ip))
										and (exp.next_hop_ip ~= srv_ip)
									then
										table.insert(
											flow_trajectory[exp.next_hop_ip],
											{ next_hop = srv_ip, return_path = exp.return_path }
										)
									end
								else
									if
										(not valueFound(flow_trajectory[exp.next_hop_ip], cli_ip))
										and (exp.next_hop_ip ~= cli_ip)
									then
										table.insert(
											flow_trajectory[exp.next_hop_ip],
											{ next_hop = cli_ip, return_path = exp.return_path }
										)
									end
								end
							end
...
function historical_flow_details_formatter.formatHistoricalFlowDetails(flow)
	...
	if flow then
		...
		flow_details[#flow_details + 1] =
			historical_flow_details_formatter.format_historical_bytes_progress_bar(flow, info)
		...
		if bgp_info then
			if bgp_info.src then
				flow_details[#flow_details + 1] =
					format_historical_bgp_info(bgp_info.src, i18n("flow_details.src_bgp_info"))
			end
			if bgp_info.dst then
				flow_details[#flow_details + 1] =
					format_historical_bgp_info(bgp_info.dst, i18n("flow_details.dst_bgp_info"))
			end
		end
		...
		if (tonumber(flow["SERVER_NW_LATENCY_US"]) > 0) or (tonumber(flow["CLIENT_NW_LATENCY_US"]) > 0) then
			flow_details[#flow_details + 1] = format_historical_flow_rtt(
		...
		if info["l4proto"] and (info["l4proto"]["label"] == "TCP") then
			if
				not isEmptyString(flow["TCP_FINGERPRINT"]) or not isEmptyString(protocol_info_json["tcp_fingerprint"])
			then
				flow_details[#flow_details + 1] =
					format_historical_tcp_fingerprint(flow, protocol_info_json["tcp_fingerprint"])
			end
		...
		if
			info["cli_host_pool_id"]
			and (info["cli_host_pool_id"]["value"] ~= "0")
			and (info["srv_host_pool_id"]["value"] ~= "0")
		then
		...
		if
			(info["community_id"] and (not isEmptyString(info["community_id"])))
			or not isEmptyString(protocol_info_json["community_id"])
		then
		...
		if protocol_info_json["traffic_stats"] and table.len(protocol_info_json["traffic_stats"]) > 0 then
			...
			if
				protocol_info_json["traffic_stats"]["cli2srv_retransmissions"] ~= 0
				or protocol_info_json["traffic_stats"]["srv2cli_retransmissions"] ~= 0
			then
				rowspan = rowspan + 1
			end

			if
				protocol_info_json["traffic_stats"]["cli2srv_out_of_order"] ~= 0
				or protocol_info_json["traffic_stats"]["srv2cli_out_of_order"] ~= 0
			then
				rowspan = rowspan + 1
			end

			if
				protocol_info_json["traffic_stats"]["cli2srv_lost"] ~= 0
				or protocol_info_json["traffic_stats"]["srv2cli_lost"] ~= 0
			then
				rowspan = rowspan + 1
			end
			flow_details = table.merge(
				flow_details,
		...
		if protocol_info_json["exporters"] and table.len(protocol_info_json["exporters"]) > 0 then
			flow_details = table.merge(
				flow_details,
				format_historical_flow_additional_exporter(
					protocol_info_json["exporters"],
					info.cli_ip.ip,
					info.srv_ip.ip
				)
			)
		...
		if
			(info.apn_mac and not isEmptyString(info.apn_mac.value))
			or (not isEmptyString(protocol_info_json["wtp_mac_address"]))
		then
			flow_details[#flow_details + 1] =
				format_historical_wtp_mac_address(flow, info, protocol_info_json["wtp_mac_address"])
		end
		...
		if table.len(protocol_info_json["proto"]) > 0 then
			...
			if
				...
				and (table.len(flow_details[#flow_details]["values"]) == 0)
			then
