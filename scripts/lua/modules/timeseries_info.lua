   ...
   if has_top_categories then
      ...
      if not table.empty(series) then
         for _, serie in pairs(series or {}) do
            ...
            local tot = 0
            tmp_tags.category = category_name
            local tot_serie = ts_utils.queryTotal("host:ndpi_categories", tags.epoch_begin, tags.epoch_end, tmp_tags)
            -- Remove serie with no data
            for _, value in pairs(tot_serie or {}) do
               tot = tot + tonumber(value)
            end
...
local function add_top_interface_timeseries(tags, timeseries, is_asn_mode_enabled, include_empty_ts)
   ...
   if has_top_categories then
      ...
      if not table.empty(series) then
         for _, serie in pairs(series or {}) do
            ...
            local tot_serie = ts_utils.queryTotal("iface:ndpi_categories", tags.epoch_begin, tags.epoch_end, tmp_tags)
            -- Remove serie with no data
            for _, value in pairs(tot_serie or {}) do
               tot = tot + tonumber(value)
            end
...
local function add_top_timeseries(tags, prefix, timeseries, is_asn_mode_enabled, include_empty_ts)
   if prefix == 'iface' then
      -- Add the top interface timeseries
      timeseries = add_top_interface_timeseries(tags, timeseries, is_asn_mode_enabled, include_empty_ts)
...
function timeseries_info.retrieve_specific_timeseries(tags, prefix, include_empty_ts)
   ...
   timeseries = add_top_timeseries(tags, prefix, timeseries, is_ntopng_asn_mode, include_empty_ts)
   return timeseries
end
   ...
   elseif rule_type == "asn" then
      ...
      for _, item in ipairs(community_timeseries) do
         if (item.id == timeseries_id.asn) then
            ...
            if (item.schema == "asn:traffic_rcvd" or item.schema == "asn:traffic_sent" or item.schema == "asn:traffic") then
