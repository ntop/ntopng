...
if ntop.isEnterprise() then
   package.path =
      dirs.installdir .. "/scripts/lua/pro/modules/flow_db/?.lua;" ..
      package.path
...
function flow_alert_store:format_location()
   ...
   if cli_location then
      ...
      self:add_filter_condition_list("cli_location", cli_location, "string",
				     value)
   end
   if srv_location then
      local list = split(srv_location, ',')
      local value = self:strip_filter_operator(list[1])
      self:add_filter_condition_list("srv_location", srv_location, "string",
				     value)
   end
...
function flow_alert_store:format_traffic_direction(traffic_direction)
   if traffic_direction then
      ...
      if value == "0" then
	 self:add_filter_condition_list("cli_location", traffic_direction,
					"string", "0")
	 self:add_filter_condition_list("srv_location", traffic_direction,
					"string", "0")
      elseif value == "1" then
	 self:add_filter_condition_list("cli_location", traffic_direction,
					"string", "1")
...
function flow_alert_store:get_column_name(field, is_write, value)
   ...
   if is_write and self._write_table_name then
	 ...
	 col = historical_flow_utils.get_flow_column_by_tag(field)
      ...
      if not col then col = field end
   else
      if field == 'flow_risk' then col = 'flow_risk_bitmap' end
   end
...
function flow_alert_store:acknowledge(label)
   ...
   if ntop.isClickHouseEnabled() then
      ...
      q = string.format(
	 "ALTER TABLE `%s` UPDATE `%s` = %u, `%s` = '%s', `%s` = %u WHERE %s",
	 table_name, self:get_column_name('alert_status', true),
	 alert_consts.alert_status.acknowledged.alert_status_id,
	 self:get_column_name('user_label', true), self:_escape(label),
	 self:get_column_name('user_label_tstamp', true), os.time(),
	 where_clause)
   else
      q = string.format(
	 "UPDATE `%s` SET `alert_status` = %u, `user_label` = '%s', `user_label_tstamp` = %u WHERE %s",
	 table_name,
	 alert_consts.alert_status.acknowledged.alert_status_id,
	 self:_escape(label), os.time(), where_clause)
   end
...
function flow_alert_store:delete()
   ...
   if ntop.isClickHouseEnabled() then
	 ...
	 q = string.format(
	    "ALTER TABLE `%s` UPDATE `IS_ALERT_DELETED` = 1 WHERE %s",
	    table_name, where_clause)
      else
	 q = string.format("ALTER TABLE `%s` DELETE WHERE %s ", table_name,
			   where_clause)
      end
   else
      q =
	 string.format("DELETE FROM `%s` WHERE %s ", table_name, where_clause)
   end
...
function flow_alert_store:insert(alert)
   ...
   if ntop.isClickHouseEnabled() then
      ...
      local alerts_map = string.rep("0", 32 - #alert.alerts_map) ..
	 alert.alerts_map
   ...
   local insert_stmt = string.format(fmt, self:get_write_table_name(),
				     ...
				     alert.first_seen, alert.tstamp, -- 10
				     map_score_to_severity(alert.score), alert.ip_version, alert.cli_ip,
				     alert.srv_ip, alert.cli_port,
				     alert.srv_port, alert.vlan_id,
				     ...
				     alert.proto, alert.l7_proto,
				     alert.l7_master_proto, alert.l7_cat,
				     ...
				     alert.cli2srv_bytes, alert.srv2cli_bytes,
				     alert.cli2srv_packets,
				     alert.srv2cli_packets, alert.first_seen,
				     alert.community_id, alert.score,
				     alert.flow_risk_bitmap or 0, -- 40
				     hex_prefix, alert.alerts_map, tonumber(
					alert.cli_host_pool_id or
					pools.DEFAULT_POOL_ID), tonumber(
					alert.srv_host_pool_id or
					pools.DEFAULT_POOL_ID), tonumber(
					alert.cli_network or
					network_consts.UNKNOWN_NETWORK),
				     tonumber(
					alert.srv_network or
					network_consts.UNKNOWN_NETWORK),
				     alert.probe_ip, alert.input_snmp,
				     alert.output_snmp,
...
function flow_alert_store:top_vlan_historical()
   if not interface.hasVLANs() then return {} end
...
function flow_alert_store:top_srv_ip_domain()
   ...
   if ntop.isClickHouseEnabled() then
      ...
      q = string.format(
	 "SELECT '.' || arrayStringConcat(arraySlice(splitByString('.'," ..
	 ...
	 string.format('JSON_VALUE(%s, \'$.%s\')', field_to_search,
		       "proto.tls.client_requested_server_name") ..
	 "),-2,2),'.') ORDER BY count DESC LIMIT %u",
	 self:get_table_name(), where_clause, self._top_limit)
...
function flow_alert_store:top_ip_merge(top_cli_ip, top_srv_ip)
   ...
   for _, p in ipairs(top_cli_ip) do
      ...
      ip_names[p.cli_ip] = {name = p.cli_name, vlan_id = p.vlan_id}
      p.name = p.cli_name
      p.ip = p.cli_ip
   end
   for _, p in ipairs(top_srv_ip) do
      all_ip[p.srv_ip] = (all_ip[p.srv_ip] or 0) + tonumber(p.count)
      ip_names[p.srv_ip] = {name = p.srv_name, vlan_id = p.vlan_id}
      p.name = p.srv_name
      p.ip = p.srv_ip
   end
...
function flow_alert_store:top_network_merge(top_cli_network, top_srv_network)
   ...
   for _, p in ipairs(top_srv_network) do
      all_network[p.srv_network] = (all_network[p.srv_network] or 0) +
	 tonumber(p.count)
      p.network = p.srv_network
   end

   for network, count in pairsByValues(all_network, rev) do
      top_network[#top_network + 1] = {network = network, count = count}
...
function flow_alert_store:_get_additional_stats(which)
   ...
   if not which or which == "network" then
      stats.top.network = self:top_network_merge(stats.top.cli_network,
						 stats.top.srv_network)
   end
   ...
   if not which or which == "mitre_tactic" then
      stats.top.mitre_tactic =
	 self:top_generic_historical("mitre_tactic", {0})
   end
   if not which or which == "mitre_technique" then
      stats.top.mitre_technique = self:top_generic_historical(
	 "mitre_technique", {0})
   end
   if not which or which == "mitre_subtechnique" then
      stats.top.mitre_subtechnique = self:top_generic_historical(
	 "mitre_subtechnique", {0})
   end
...
function flow_alert_store:_add_additional_request_filters()
   ...
   self:add_filter_condition_list('cli_host_pool_id', cli_host_pool_id,
				  'number')
   self:add_filter_condition_list('srv_host_pool_id', srv_host_pool_id,
				  'number')
   ...
   self:add_filter_condition_list(self:format_query_json_value(
				     'proto.tls.ja4_client_hash'), ja4_client,
				  'string')
   self:add_filter_condition_list(self:format_query_json_value(
				     'proto.l7_error_code'), error_code,
				  'string')
   self:add_filter_condition_list(self:format_query_json_value(
				     'proto.confidence'), confidence, 'string')
   self:add_filter_condition_list(self:format_query_json_value(
				     'proto.tls.client_requested_server_name'),
				  alert_domain, 'string')
...
local RNAME = {
   ...
   FLOW = {
      ...
      export = true,
      elements = {
	 "srv_ip.label", "srv_ip.value", "srv_port", "cli_ip.label",
	 "cli_ip.value", "cli_port"
      }
   },
   ...
   HIST_FLOW_CLI_INFO = {name = "hist_flow_cli_info", export = true},
   HIST_FLOW_SRV_INFO = {name = "hist_flow_srv_info", export = true},
   HIST_FLOW_INFO = {name = "hist_flow_info", export = true},
   MITRE = {name = "mitre_data", export = false}
}

-- ##############################################

function flow_alert_store:get_rnames() return RNAME end
...
function flow_alert_store:format_record(value, no_html, verbose)
   local record = self:format_json_record_common(value, alert_entities.flow
						 .entity_id, no_html)
   ...
   local msg = alert_utils.formatFlowAlertMessage(interface.getId(), value,
						  predominant_alert_info,
						  false, local_explorer)
   local active_url = ""
   local attacker = ""
   local victim = ""
   ...
   local flow_related_info =
      addExtraFlowInfo(alert_json, value, no_html --[[ Send these info in json format ]] )
   ...
   if alert_risk and alert_risk > 0 and -- record.script_key == 'tls_certificate_selfsigned'
      tonumber(value.alert_id) ==
      flow_alert_keys.flow_alert_tls_certificate_selfsigned then
      flow_tls_issuerdn = alert_utils.get_flow_risk_info(alert_risk,
							 alert_info)
   end
   ...
   if not no_html and alert_json and (alert_json["ntopng.key"]) and
      (alert_json["hash_entry_id"]) then
      local active_flow = interface.findFlowByKeyAndHashId(
	 alert_json["ntopng.key"],
	 alert_json["hash_entry_id"])

      if active_flow and active_flow["seen.first"] <
	 tonumber(value["tstamp_end"]) then
	 local href = string.format(
	    "%s/lua/flow_details.lua?flow_key=%u&flow_hash_id=%u",
	    ntop.getHttpPrefix(), active_flow["ntopng.key"],
	    active_flow["hash_entry_id"])
	 active_url = href
      end
   end
   ...
   if (verbose and ntop.isEnterprise()) then
      ...
      local flow = db_search_manager.get_flow(value["rowid"],
					      value["tstamp_epoch"], "")
	 ...
	 major_connection_state = tonumber(
	    flow["MAJOR_CONNECTION_STATE"] or ""),
   ...
   if value.alerts_map then
      other_alerts_by_score, additional_alerts =
	 alert_utils.format_other_alerts(value.alerts_map, value['alert_id'],
					 nil, nil, no_html)
   end
   ...
   for _, messages in pairsByKeys(other_alerts_by_score, rev) do
	 ...
	 record[RNAME.ADDITIONAL_ALERTS.name] =
	    record[RNAME.ADDITIONAL_ALERTS.name] .. cur_msg
      end
   end
   ...
   if no_html then
      ...
      record[RNAME.DESCRIPTION.name] = {descr = msg}
   end
   ...
   record[RNAME.MITRE.name] = {
      ...
      mitre_tactic_i18n = (mitre_utils.tactic_by_id[mitre_tactic] and
			   mitre_utils.tactic_by_id[mitre_tactic].i18n_label) or "",
      mitre_technique_i18n = (mitre_utils.technique_by_id[mitre_technique] and
			      mitre_utils.technique_by_id[mitre_technique].i18n_label) or "",
   ...
   local flow_domain
   if hostnameIsDomain(flow_server_name) then flow_domain = flow_server_name end
   ...
   record[RNAME.FLOW_RELATED_INFO.name] = {descr = flow_related_info or {}}
   ...
   if isEmptyString(value['srv_network']) or tonumber(value['srv_network']) >=
      network_consts.UNKNOWN_NETWORK then
      record[srv_network]['label'] = ''
      record[srv_network]['value'] = ''
   end

   if isEmptyString(value['cli_network']) or tonumber(value['cli_network']) >=
      network_consts.UNKNOWN_NETWORK then
   ...
   if not no_html then
      record[RNAME.MSG.name].configset_ref =
	 alert_utils.getConfigsetAlertLink(predominant_alert_info)
   end
   ...
   if not isEmptyString(cli_ip) and not no_html then
      reference_html = hostinfo2detailshref({ip = cli_ip, value["vlan_id"]},
	 nil, href_icon,
	 i18n('host_details_tooltip'),
	 true, nil, false)
      if reference_html == href_icon then reference_html = "" end
   end
   ...
   if (isEmptyString(country) or country == "nil") and
      not isEmptyString(cli_ip) then
      local host_info = interface.getHostMinInfo(cli_ip)
      if host_info then country = host_info["country"] or "" end
   end
   ...
   if not isEmptyString(value["cli_name"]) and value["cli_name"] ~=
      flow_cli_ip["value"] then flow_cli_ip["name"] = value["cli_name"] end

   if not isEmptyString(cli_ip) then
      local label = hostinfo2label(
	 self:_alert2hostinfo(value, true --[[ As client --]] ),
	 false --[[ Show VLAN --]] , false --[[ Shorten --]] ,
   ...
   if not isEmptyString(value["srv_name"]) and value["srv_name"] ~=
      flow_srv_ip["value"] then flow_srv_ip["name"] = value["srv_name"] end

   if not isEmptyString(srv_ip) then
      local label = hostinfo2label(
	 self:_alert2hostinfo(value, false --[[ As server --]] ),
	 false --[[ Show VLAN --]] , false --[[ Shorten --]] ,
	 true --[[ Skip Resolution ]] )
   ...
   local vlan = {label = "", title = "", value = 0}
   ...
   if tonumber(value["l7_master_proto"]) and tonumber(value["l7_proto"]) then
      l7_protocol = interface.getnDPIFullProtoName(tonumber(
						      value["l7_master_proto"]),
						   tonumber(value["l7_proto"]))
   end
   ...
   record[RNAME.L7_PROTO.name] = {
      value = ternary(tonumber(value["l7_proto"]) ~= 0, value["l7_proto"],
		      value["l7_master_proto"]) or "",
   ...
   if value["srv2cli_pkts"] and value["cli2srv_pkts"] then
      traffic_stats.total_packets = tonumber(value["srv2cli_pkts"]) +
	 tonumber(value["cli2srv_pkts"])
   end
   record[RNAME.TRAFFIC.name] = traffic_stats

   -- Add link to historical flow
   if ntop.isEnterpriseM() and hasClickHouseSupport() and not no_html and
      tonumber(value["tstamp"]) and tonumber(value["tstamp_end"]) then
	 ...
	 ternary(show_cli_port, tostring(value["cli_port"]), ''),
	 op_suffix, ternary(show_srv_port,
			    tostring(value["srv_port"]), ''),
	 op_suffix, l4_protocol or "", op_suffix)
   ...
   if #rules > 0 and tonumber(value["tstamp"]) and
      tonumber(value["tstamp_end"]) then
   ...
   record[RNAME.PROBE_IP.name] = {value = probe_ip, label = probe_label}
...
local function get_flow_link(fmt, add_hyperlink)
   ...
   if add_hyperlink and is_in_memory then label = label .. " " .. reference end
...
local function add_historical_link(value, flow_link)
   ...
   if ntop.isClickHouseEnabled() then
      historical_href = "<a href=\"" .. ntop.getHttpPrefix() ..
	 "/lua/pro/db_flow_details.lua?row_id=" ..
	 value["rowid"] .. "&tstamp=" ..
	 value["tstamp_epoch"] .. "\" title='" ..
...
function flow_alert_store:get_alert_details(value)
   ...
   if (proto_info and (proto_info.l7_error_code ~= nil) and
       (proto_info.l7_error_code ~= 0)) then
      details[#details + 1] = {
...
function flow_alert_store:format_record_telemetry(value)
   ...
   local alert_info = alert_utils.getAlertInfo(value)
   local alert_name = alert_consts.alertTypeLabel(tonumber(value["alert_id"]),
						  true, alert_entities.flow
						  .entity_id)
   -- local msg = alert_utils.formatFlowAlertMessage(interface.getId(), value, alert_info, false, local_explorer)

   -- Flow related info
   local alert_json = not isEmptyString(value.json) and
      (json.decode(value.json) or {}) or {}
   ...
   -- TLS IssuerDN
   local flow_tls_issuerdn = nil
   if tonumber(value.alert_id) ==
      flow_alert_keys.flow_alert_tls_certificate_selfsigned then
      flow_tls_issuerdn = alert_utils.get_flow_risk_info(
	 ntop.getFlowAlertRisk(tonumber(value.alert_id)),
	 alert_info)
      flow_tls_issuerdn = flow_tls_issuerdn or
	 getExtraFlowInfoTLSIssuerDN(alert_json)
   end
   ...
   local t, other_issues = alert_utils.format_other_alerts(flow['ALERTS_MAP'],
							   flow['STATUS'],
							   alert_json, false,
							   true, true)
   ...
   -- client info
   local cli_ip = value["cli_ip"]
   local cli_country = value["cli_country"] or
      (cli_ip and
       interface.getHostMinInfo(cli_ip)["country"]) or
      ""
   ...
   -- server info
   local srv_ip = value["srv_ip"]
   local srv_country = value["srv_country"] or
      (srv_ip and
       interface.getHostMinInfo(srv_ip)["country"])
   ...
   local l4_protocol = not isEmptyString(value["proto"]) and
      l4_proto_to_string(value["proto"])
   local l7_protocol = tonumber(value["l7_master_proto"]) and
      tonumber(value["l7_proto"]) and
      interface.getnDPIFullProtoName(
	 tonumber(value["l7_master_proto"]),
