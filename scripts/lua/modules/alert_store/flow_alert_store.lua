...
function flow_alert_store:insert(alert)
    ...
    local insert_stmt = string.format("INSERT INTO %s " ..
        ...
        self:_convert_ifid(interface.getId()), alert.first_seen, alert.tstamp, map_score_to_severity(alert.score),
        alert.ip_version, alert.cli_ip, alert.srv_ip, alert.cli_port, alert.srv_port, alert.vlan_id,
        ternary(alert.is_cli_attacker, 1, 0), ternary(alert.is_cli_victim, 1, 0), ternary(alert.is_srv_attacker, 1, 0),
        ...
        ternary(alert.cli_blacklisted, 1, 0), ternary(alert.srv_blacklisted, 1, 0), alert.cli_location or 0,
        alert.srv_location or 0, alert.cli2srv_bytes, alert.srv2cli_bytes, alert.cli2srv_packets, alert.srv2cli_packets,
        alert.first_seen, alert.community_id, alert.score, alert.flow_risk_bitmap or 0, hex_prefix, alert.alerts_map,
        alert.cli_host_pool_id or pools.DEFAULT_POOL_ID, alert.srv_host_pool_id or pools.DEFAULT_POOL_ID,
        alert.cli_network or network_consts.UNKNOWN_NETWORK, alert.srv_network or network_consts.UNKNOWN_NETWORK,
        alert.probe_ip, alert.input_snmp, alert.output_snmp, self:_escape(alert.json), self:_escape(alert.info or ''))
...
function flow_alert_store:top_l7_proto_historical()
    ...
    local q = string.format(
        "SELECT l7_proto, sum(score) count FROM %s WHERE %s GROUP BY l7_proto ORDER BY count DESC LIMIT %u",
...
function flow_alert_store:top_vlan_historical()
    ...
    local q = string.format(
        "SELECT vlan_id, sum(score) count FROM %s WHERE %s AND vlan_id != 0 GROUP BY vlan_id ORDER BY count DESC LIMIT %u",
        self:get_table_name(), where_clause, self._top_limit)
    local q_res = interface.alert_store_query(q) or {}
...
function flow_alert_store:top_cli_ip_historical()
    ...
    if ntop.isClickHouseEnabled() then
        q = string.format(
            "SELECT cli_ip, vlan_id, cli_name, sum(score) count FROM %s WHERE %s GROUP BY cli_ip, vlan_id, cli_name ORDER BY count DESC LIMIT %u",
            self:get_table_name(), where_clause, self._top_limit)
    else
        q = string.format(
            "SELECT cli_ip, vlan_id, cli_name, sum(score) count FROM %s WHERE %s GROUP BY cli_ip ORDER BY count DESC LIMIT %u",
            self:get_table_name(), where_clause, self._top_limit)
    end
...
function flow_alert_store:top_srv_ip_historical()
    ...
    if ntop.isClickHouseEnabled() then
        q = string.format(
            "SELECT srv_ip, vlan_id, srv_name, sum(score) count FROM %s WHERE %s GROUP BY srv_ip, vlan_id, srv_name ORDER BY count DESC LIMIT %u",
            self:get_table_name(), where_clause, self._top_limit)
    else
        q = string.format(
            "SELECT srv_ip, vlan_id, srv_name, sum(score) count FROM %s WHERE %s GROUP BY srv_ip ORDER BY count DESC LIMIT %u",
            self:get_table_name(), where_clause, self._top_limit)
    end
    ...
    if ntop.isClickHouseEnabled() then
        ...
        q = string.format("SELECT '.' || arrayStringConcat(arraySlice(splitByString('.'," ..
                ...
                "proto.tls.client_requested_server_name") ..
                              ...
                              string.format('JSON_VALUE(%s, \'$.%s\')', field_to_search,
                "proto.tls.client_requested_server_name") ..
                              "),-2,2),'.') as domain_name_trunc_star, sum(score) count FROM %s WHERE %s GROUP BY vlan_id, '*.' || arrayStringConcat(arraySlice(splitByString('.'," ..
                              string.format('JSON_VALUE(%s, \'$.%s\')', field_to_search,
                "proto.tls.client_requested_server_name") ..
...
function flow_alert_store:top_cli_network_historical()
    ...
    if ntop.isClickHouseEnabled() then
        q = string.format(
            "SELECT cli_network, sum(score) count FROM %s WHERE %s GROUP BY cli_network ORDER BY count DESC LIMIT %u",
            self:get_table_name(), where_clause, self._top_limit)
    else
        q = string.format(
            "SELECT cli_network, sum(score) count FROM %s WHERE %s GROUP BY cli_network ORDER BY count DESC LIMIT %u",
...
function flow_alert_store:top_srv_network_historical()
    ...
    if ntop.isClickHouseEnabled() then
        q = string.format(
            "SELECT srv_network, sum(score) count FROM %s WHERE %s GROUP BY srv_network ORDER BY count DESC LIMIT %u",
            self:get_table_name(), where_clause, self._top_limit)
    else
        q = string.format(
            "SELECT srv_network, sum(score) count FROM %s WHERE %s GROUP BY srv_network ORDER BY count DESC LIMIT %u",
