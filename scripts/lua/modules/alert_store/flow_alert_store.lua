	...
	if ntop.isClickHouseEnabled() then
		q = string.format(
			"SELECT '.' || arrayStringConcat(arraySlice(splitByString('.',"
				.. string.format("JSON_VALUE(%s, '$.%s')", field_to_search, "proto.tls.client_requested_server_name")
...
function flow_alert_store:_add_additional_request_filters()
	...
	self:add_filter_condition_list(
		self:format_query_json_value("proto.tls.client_requested_server_name"),
		alert_domain,
		"string"
	)
end
...
function flow_alert_store:format_record(value, no_html, verbose)
	...
	local alert_name =
		alert_consts.alertTypeLabel(tonumber(value["alert_id"]), true --[[ no_html --]], alert_entities.flow.entity_id)
	local show_cli_port = (value["cli_port"] ~= "" and value["cli_port"] ~= "0")
	local show_srv_port = (value["srv_port"] ~= "" and value["srv_port"] ~= "0")
	local msg = alert_utils.formatFlowAlertDescription(value, predominant_alert_info, value.alert_id)
	local active_url = ""
	local attacker = ""
	local victim = ""
	...
	if
		alert_risk
		and alert_risk > 0 -- record.script_key == 'tls_certificate_selfsigned'
		and tonumber(value.alert_id) == flow_alert_keys.flow_alert_tls_certificate_selfsigned
	then
	...
	if value.alerts_map then
		other_alerts_by_score, additional_alerts =
			alert_utils.format_other_alerts(value.alerts_map, value["alert_id"], nil, nil, no_html)
	end
	...
	record[RNAME.MITRE.name] = {
		...
		mitre_tactic_i18n = (
			mitre_utils.tactic_by_id[mitre_tactic] and mitre_utils.tactic_by_id[mitre_tactic].i18n_label
		) or "",
		mitre_technique_i18n = (
			mitre_utils.technique_by_id[mitre_technique] and mitre_utils.technique_by_id[mitre_technique].i18n_label
		) or "",
		mitre_subtechnique_i18n = (
			mitre_utils.sub_technique_by_id[mitre_subtechnique]
			and mitre_utils.sub_technique_by_id[mitre_subtechnique].i18n_label
		) or "",
	}
	...
	if isEmptyString(value["srv_network"]) or tonumber(value["srv_network"]) >= network_consts.UNKNOWN_NETWORK then
		record[srv_network]["label"] = ""
		record[srv_network]["value"] = ""
	end

	if isEmptyString(value["cli_network"]) or tonumber(value["cli_network"]) >= network_consts.UNKNOWN_NETWORK then
	...
	record[RNAME.MSG.name] = {
		...
		description = msg,
		configset_ref = alert_utils.getConfigsetAlertLink(
	...
	if
		ntop.isEnterpriseM
		and ntop.isEnterpriseM()
		and hasClickHouseSupport()
		and not no_html
		and tonumber(value["tstamp"])
		and tonumber(value["tstamp_end"])
	then
		local op_suffix = flowfilter_utils.SEPARATOR .. "eq"
		local href = string.format(
			"%s/lua/pro/db_search.lua?epoch_begin=%u&epoch_end=%u&cli_ip=%s%s&srv_ip=%s%s%s%s&l4proto=%s%s",
			ntop.getHttpPrefix(),
			...
			value["srv_ip"],
			op_suffix,
			ternary(show_cli_port, tostring("&cli_port=" .. tostring(value["cli_port"]) .. op_suffix), ""),
			ternary(show_srv_port, "&srv_port=" .. tostring(value["srv_port"]) .. op_suffix, ""),
			l4_protocol,
			op_suffix
		)
...
function flow_alert_store:get_flow_link(fmt, add_hyperlink)
	...
	if fmt["flow"]["vlan"] and fmt["flow"]["vlan"]["value"] ~= 0 then
		...
		vlan = "@" .. self.get_label_link(fmt["flow"]["vlan"]["label"], "vlan_id", fmt["flow"]["vlan"], add_hyperlink)
	end
	...
	if fmt["flow"]["cli_ip"]["label_long"] ~= fmt["flow"]["cli_ip"]["value"] then
		if add_hyperlink then
			cli_ip = " [ " .. self.get_label_link(fmt["flow"]["cli_ip"], "cli_ip", value, add_hyperlink) .. " ]"
		end
		value = fmt["flow"]["cli_ip"]["label_long"]
		tag = "cli_name"
	end
	label = label .. self.get_label_link(fmt["flow"]["cli_ip"]["label_long"], tag, value, add_hyperlink) .. cli_ip
	...
	if not isEmptyString(cli_port) and cli_port ~= "0" then
		label = label
			.. vlan
			.. ":"
			.. self.get_label_link(fmt["flow"]["cli_port"], "cli_port", fmt["flow"]["cli_port"], add_hyperlink)
	end
	...
	if fmt["flow"]["srv_ip"]["label_long"] ~= fmt["flow"]["srv_ip"]["value"] then
		if add_hyperlink then
			srv_ip = " [ "
				.. self.get_label_link(fmt["flow"]["srv_ip"]["value"], "srv_ip", value, add_hyperlink)
				.. " ]"
		end
		value = fmt["flow"]["srv_ip"]["label_long"]
		tag = "srv_name"
	end
	label = label .. self.get_label_link(fmt["flow"]["srv_ip"]["label_long"], tag, value, add_hyperlink) .. srv_ip
	...
	if not isEmptyString(srv_port) and srv_port ~= "0" then
		label = label
			.. vlan
			.. ":"
			.. self.get_label_link(fmt["flow"]["srv_port"], "srv_port", fmt["flow"]["srv_port"], add_hyperlink)
	end
...
function flow_alert_store:get_alert_details(value)
	...
	details[#details + 1] = {
		...
		values = { self.get_label_link(fmt["alert_id"]["label"], "alert_id", fmt["alert_id"]["value"], add_hyperlink) },
	}
	...
	details[#details + 1] = {
		...
		values = {
			self.get_label_link(
				fmt["l7_proto"]["l4_label"] .. ":" .. fmt["l7_proto"]["l7_label"],
				"l7proto",
				fmt["l7_proto"]["value"],
				add_hyperlink
			),
	...
	details[1].values[1] = details[1].values[1]
		.. ' [ <A HREF="'
		.. url
		.. '">'
		.. i18n("ask_google_ai")
...
function flow_alert_store:format_record_telemetry(value)
	...
	local t, other_issues =
		alert_utils.format_other_alerts(flow["ALERTS_MAP"], flow["STATUS"], alert_json, false, true, true)
	...
	local l7_protocol = tonumber(value["l7_master_proto"])
			and tonumber(value["l7_proto"])
			and interface.getnDPIFullProtoName(tonumber(value["l7_master_proto"]), tonumber(value["l7_proto"]))
		or ""
