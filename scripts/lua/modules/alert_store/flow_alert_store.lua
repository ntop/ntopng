...
function flow_alert_store:acknowledge(label)
   ...
   if ntop.isClickHouseEnabled() then
      ...
      q = string.format("ALTER TABLE `%s` UPDATE `%s` = %u, `%s` = '%s', `%s` = %u WHERE %s", table_name,
         ...
         self:get_column_name('user_label', true), self:_escape(label), self:get_column_name('user_label_tstamp', true), os.time(),
         where_clause)
   else
      q = string.format("UPDATE `%s` SET `alert_status` = %u, `user_label` = '%s', `user_label_tstamp` = %u WHERE %s", table_name,
...
function flow_alert_store:insert(alert)
   ...
   local insert_stmt = string.format(fmt, self:get_write_table_name(), extra_columns, extra_values, alert.alert_id, alert_status,
      ...
      map_score_to_severity(alert.score), alert.ip_version, alert.cli_ip, alert.srv_ip, alert.cli_port, alert.srv_port, alert.vlan_id,
      ternary(alert.is_cli_attacker, 1, 0), ternary(alert.is_cli_victim, 1, 0), ternary(alert.is_srv_attacker, 1, 0), -- 20
      ternary(alert.is_srv_victim, 1, 0), alert.proto, alert.l7_proto, alert.l7_master_proto, alert.l7_cat, self:_escape(alert.cli_name),
      self:_escape(alert.srv_name), alert.cli_country_name, alert.srv_country_name, ternary(alert.cli_blacklisted, 1, 0), -- 30
      ternary(alert.srv_blacklisted, 1, 0), alert.cli_location or 0, alert.srv_location or 0, alert.cli2srv_bytes, alert.srv2cli_bytes,
...
function flow_alert_store:format_record(value, no_html, verbose)
   ...
   if not no_html and alert_json and (alert_json["ntopng.key"]) and (alert_json["hash_entry_id"]) then
      ...
      if active_flow and active_flow["seen.first"] < tonumber(value["tstamp_end"]) then
         local href = string.format("%s/lua/flow_details.lua?flow_key=%u&flow_hash_id=%u", ntop.getHttpPrefix(), active_flow["ntopng.key"],
   ...
   if value.alerts_map then
      other_alerts_by_score, additional_alerts = alert_utils.format_other_alerts(value.alerts_map, value['alert_id'], nil, nil, no_html)
   end
   ...
   record[RNAME.MSG.name] = {
      ...
      description = msg,
      configset_ref = alert_utils.getConfigsetAlertLink(predominant_alert_info, value["alert_id"], alert_entities.flow.entity_id)
   }
   ...
   if not isEmptyString(cli_ip) then
      local label =
         hostinfo2label(self:_alert2hostinfo(value, true --[[ As client --]] ), false --[[ Show VLAN --]] , false --[[ Shorten --]] , true --[[ Skip Resolution ]] )
   ...
   if not isEmptyString(srv_ip) then
      local label =
         hostinfo2label(self:_alert2hostinfo(value, false --[[ As server --]] ), false --[[ Show VLAN --]] , false --[[ Shorten --]] , true --[[ Skip Resolution ]] )
   ...
   if ntop.isEnterpriseM and ntop.isEnterpriseM() and hasClickHouseSupport() and not no_html and tonumber(value["tstamp"]) and tonumber(value["tstamp_end"]) then
      ...
      local href = string.format(
         ...
         value["srv_ip"], op_suffix, ternary(show_cli_port, tostring(value["cli_port"]), ''), op_suffix,
...
function flow_alert_store:get_flow_link(fmt, add_hyperlink)
   ...
   if not isEmptyString(cli_port) and cli_port ~= '0' then
      label = label .. vlan .. ':' .. self.get_label_link(fmt['flow']['cli_port'], 'cli_port', fmt['flow']['cli_port'], add_hyperlink)
   end
   ...
   if fmt['flow']['srv_ip']['label_long'] ~= fmt['flow']['srv_ip']['value'] then
      if add_hyperlink then
         srv_ip = " [ " .. self.get_label_link(fmt['flow']['srv_ip']['value'], 'srv_ip', value, add_hyperlink) .. " ]"
      end
   ...
   if not isEmptyString(srv_port) and srv_port ~= '0' then
      label = label .. vlan .. ':' .. self.get_label_link(fmt['flow']['srv_port'], 'srv_port', fmt['flow']['srv_port'], add_hyperlink)
   end
...
local function add_historical_link(value, flow_link)
   ...
   if ntop.isClickHouseEnabled() then
      historical_href = "<a href=\"" .. ntop.getHttpPrefix() .. "/lua/pro/db_flow_details.lua?row_id=" .. value["rowid"] .. "&tstamp=" ..
                           value["tstamp_epoch"] .. "\" title='" .. i18n('alert_details.flow_details') ..
...
function flow_alert_store:get_alert_details(value)
   ...
   details[#details + 1] = {
      ...
      values = {self.get_label_link(fmt['l7_proto']['l4_label'] .. ':' .. fmt['l7_proto']['l7_label'], 'l7proto', fmt['l7_proto']['value'],
         add_hyperlink)}
   }
