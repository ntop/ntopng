...
local email = {
    ...
    recipient_params = {
        {param_name = "email_recipient"}, {param_name = "cc", optional = true}
    },
    recipient_template = {
        script_key = "email",
...
local NUM_ATTEMPTS_KEY =
    "ntopng.alerts.modules_notifications_queue.email.num_attemps"

if ntop.getPref("ntopng.prefs.debug_email") == "1" then debug_endpoint = true end
...
function email.format_recipient_params(recipient_params)
    return
        string.format("%s (%s)", recipient_params.email_recipient, email.name)
end
...
function email.isAvailable() return (ntop.sendMail ~= nil) end
...
function email.sendEmail(subject, message_body, settings)
    ...
    subject = subject .. product .. " @ " .. info.instance_name .. " (" ..
                  info.ip .. ")"
    ...
    local message = buildMessageHeader(os.time(), settings.from_addr,
                                       settings.to_addr, settings.cc_addr,
    ...
    while budget_used <= budget and more_available do
        ...
        while i < MAX_ALERTS_PER_EMAIL do
            ...
            if notification then
                ...
                if alert_utils.filter_notification(notification,
                                                   recipient.recipient_id) then
        ...
        for _, notif in ipairs(notifications) do
            messages[#messages + 1] = format_utils.formatMessage(notif, {
                show_entity = true,
                nohtml = false,
                nolabelhtml = true
            })
        ...
        if not rv.success then
            local num_attemps =
                (tonumber(ntop.getCache(NUM_ATTEMPTS_KEY)) or 0) + 1
...
function email.runTest(recipient)
    ...
    local sent = email.sendEmail("TEST MAIL", "Email notification is working",
                                 settings)

    if sent.success then
        message_info = i18n("prefs.email_sent_successfully")
    else
        message_info = i18n("prefs.email_send_error", {
            msg = sent.msg,
            url = "https://www.ntop.org/guides/ntopng/user_interface/shared/alerts/available_endpoints.html#email"
        })
    end
