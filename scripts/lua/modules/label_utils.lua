...
function flowinfo2hostname(flow_info, host_type, alerts_view, add_hostname)
	...
	if host_type == "srv" then
		if
			flow_info["host_server_name"] ~= nil
			and flow_info["host_server_name"] ~= ""
			and flow_info["host_server_name"]:match("%w")
			and not isIPv4(flow_info["host_server_name"])
			and not isIPv6(flow_info["host_server_name"])
		then
...
function getLocalNetworkAliasById(network, short_version)
	...
	for n, ns in pairs(networks_stats) do
		if ns.network_id == network_id then
			label = getFullLocalNetworkName(ns.network_key, short_version)
		end
	end
	return label
end
...
function getFullLocalNetworkName(network, short_version)
	local network_consts = require("network_consts")
	local network_id = tonumber(network)
    local alias = ""

    if networks_name_cache[tostring(network_id)] then
        alias = networks_name_cache[tostring(network_id)]
    else
	    alias = getLocalNetworkAlias(network)
        networks_name_cache[tostring(network_id)] = alias
    end
