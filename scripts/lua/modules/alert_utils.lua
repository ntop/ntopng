...
function alert_utils.formatRawFlow(alert, nohtml)
	...
	flow = '<i class="fas fa-stream"></i> '
		.. (getFlowLabel(flow, false, add_links, time_bounds, {
			page = "alerts",
		}, nil, true, nohtml) or "")
...
function alert_utils.getConfigsetURL(script_key, subdir)
	return string.format(
		"%s/lua/admin/edit_configset.lua?subdir=%s&check=%s#all",
		ntop.getHttpPrefix(),
		url_encode(subdir or "all"),
		url_encode(script_key or "all")
	)
end
      ...
      if not info then
		...
		if alert_entity and (alert_entity == alert_entities.am_host.entity_id) then
			...
			if host then
				...
				return string.format(
					"<a href='%s/lua/active_monitoring.lua?host=%s%s&page=overview'><i class='fas fa-cog' title='%s'></i></a>",
					ntop.getHttpPrefix(),
					host,
					ternary(measurement, "&measurement=" .. measurement, ""),
					i18n("edit_configuration")
				)
			else
				return ' <a href="'
					.. ntop.getHttpPrefix()
...
function alert_utils.formatFlowAlertMessage(
	ifid,
	alert,
	alert_json,
	add_score,
	local_explorer,
	exclude_remediation_link
)
	local msg
	local alert_risk
	...
	if isEmptyString(msg) then
		if
			alert_json
			and alert_json.alert_generation
			and alert_risk
			and alert_risk > 0
			and alert_json.flow_risk_info
		then
	...
	if alert_risk and alert_risk > 0 and not exclude_remediation_link then
		...
		msg = string.format("%s %s", msg, flow_risk_utils.get_remediation_documentation_link(alert_risk, alert_src))
...
function alert_utils.getLinkToPastFlows(ifid, alert, alert_json)
	...
	if alert_id and epoch_begin and epoch_end then
		...
		local res = string.format(
			"%s/lua/pro/db_search.lua?epoch_begin=%u&epoch_end=%u&%s",
			ntop.getHttpPrefix(),
			epoch_begin,
			epoch_end,
...
function alert_utils.formatAlertNotification(notif, options)
	...
	local alert_type_label =
		alert_consts.alertTypeLabel(notif.alert_id, options.nohtml or options.nolabelhtml, notif.entity_id, true)
...
local function notify_ntopng_status(started)
	...
	local score = 10
	local msg
	local msg_details = string.format(
		"%s v.%s (%s) [OS: %s][pid: %s][options: %s]",
		info.product,
		info.version,
		info.revision,
		info.OS,
	...
	if started then
		...
		if not recovery_utils.check_clean_shutdown() then
			...
			msg = string.format(
				"%s %s",
				i18n("alert_messages.ntopng_anomalous_termination", {
...
local function addDayEpoch(res, day_epoch)
	res[day_epoch] = {
		...
		0,
		0,
		0 --[[ Counter for alerts in other hours, until 23:00 to 23:59 --]],
	}
...
function alert_utils.formatOldTimeseries(q_res, _epoch_begin, _epoch_end)
	...
	for _, v in ipairs(q_res) do
		...
		res[day_epoch][hour + 1] =
			tonumber( --[[ Historical ]] v.count --[[ Engaged ]] or ((res[day_epoch][hour + 1] or 0) + 1))
	end -- for
...
function alert_utils.check_alert_policy(entity_id, entity_val, alert_id, alert_info, recipient_id)
	...
	if silence_alerts == "1" then
		...
		local redis_key =
			string.format("ntopng.cache.alert.retention.%s.%s.%s%s", recipient_id, entity_id, alert_id, alert_key)
...
-- Function used to format the flow alerts description
-- first checks if some description is present inside the risks, otherwise format
-- the standard description
function alert_utils.formatFlowAlertDescription(alert, alert_info, alert_id)
	local info = ""
	local riskInfo = {}
	local alert_json = {}
	local alert_risk = ntop.getFlowAlertRisk(tonumber(alert_id))
	-- Decode the alert json, includes various info about the alert
	if not isEmptyString(alert.json) then
		alert_json = json.decode(alert.json) or {}
	end
	-- Check if there is also a flow risk info set by nDPI
	if alert_risk ~= 0 then
		-- Alert risk is not 0, meaning that the alert probably comes from nDPI, check in the flow_risk_info
		if alert_json and alert_json.flow_risk_info then
		...
		-- Risk info present, now check if the alert requested has an info inside
		if riskInfo and not isEmptyString(riskInfo[tostring(alert_risk)]) then
			info = riskInfo[tostring(alert_risk)]
		end
	end
	-- Fallback, no info found in the flow_risk, use the standard formatting
	if isEmptyString(info) then
		info = alert_utils.formatFlowAlertMessage(alert.interface_id, alert, alert_info, false, true, true)
	end
...
-- This function is used to calculate the number of dropped alerts.
-- This is used by the timeseries, because the value should be a counter, but
-- for displaying reasons (it will divide the value for the time), it is better
-- displayed as a gauge.
-- Given the interface stats it will calculate the current number of dropped alerts;
-- also it will handle the case where the counter was reset from the GUI
function alert_utils.computeDroppedAlerts(ifstats)
	-- Get the stats
	local iface_dropped_alerts_key = "ntopng.stats.dropped_alerts."
	local num_prev_dropped_alerts = tonumber(ntop.getCache(iface_dropped_alerts_key .. ifstats.id)) or 0
	-- Set the new counter
	ntop.setCache(iface_dropped_alerts_key .. ifstats.id, ifstats.num_dropped_alerts)
	local tot_dropped_alerts = 0
	-- Calculate the difference, if the previously saved value is higher then the current one
	-- a reset has been done, do not do the difference, it's just the current value
	if ifstats.num_dropped_alerts < num_prev_dropped_alerts then
		tot_dropped_alerts = ifstats.num_dropped_alerts
	else
		tot_dropped_alerts = ifstats.num_dropped_alerts - num_prev_dropped_alerts
	end
    --[[
	traceError(
		TRACE_NORMAL,
		TRACE_CONSOLE,
		string.format(
			"Number of dropped alerts computed [ifid: %s][Curr. alerts: %s][Prev. alerts: %s][Total: %s]",
			ifstats.id,
			ifstats.num_dropped_alerts,
			num_prev_dropped_alerts,
			tot_dropped_alerts
		)
	)
    ]]
	return tot_dropped_alerts
end
