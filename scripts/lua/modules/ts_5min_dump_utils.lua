	...
	for _, asn in pairs(as_list or {}) do
		...
		if ntop.isEnterpriseL and ntop.isEnterpriseL() then
			...
			for id, value in pairs(asn_stats.qoe or {}) do
				local delta = delta_val(
					interface,
					"asn_qoe_stats." .. ifstats.id .. "." .. asn .. "." .. id,
					"5mins",
					value.num,
					true
				)
...
function ts_dump.obs_point_update_rrds(when, ifstats, verbose, config)
	...
	for _, obs_point_stats in pairs(obs_points_info["ObsPoints"]) do
		...
		if
			...
			or config.interface_ndpi_timeseries_creation == "both"
		then
...
function ts_dump.sites_update_rrds(when, ifstats, verbose) 
    local sites_stats = interface.getSitesStats() or {}
...
function ts_dump.host_update_stats_rrds(when, hostname, host, ifstats, verbose)
	...
	if ntop.isEnterpriseL and ntop.isEnterpriseL() then
		...
		for id, value in pairs(host.qoe or {}) do
			local delta = delta_val(
				interface,
				"host_qoe_stats." .. ifstats.id .. "." .. hostname .. "." .. id,
				"5mins",
				value.num,
				true
			)
...
function ts_dump.host_update_rrd(when, hostname, host, ifstats, verbose, config)
	...
	if config.host_ts_creation ~= "off" then
		...
		if config.host_ts_creation == "full" then
			...
			if
				config.host_ndpi_timeseries_creation == "per_protocol"
				or config.host_ndpi_timeseries_creation == "both"
			then
				ts_dump.host_update_ndpi_rrds(when, hostname, host, ifstats, verbose, config)
			end

			if
				config.host_ndpi_timeseries_creation == "per_category"
				or config.host_ndpi_timeseries_creation == "both"
			then
...
function ts_dump.run_5min_dump(_ifname, ifstats, when, verbose)
	...
	if config.host_ts_creation ~= "off" then
		...
		local in_time = callback_utils.foreachLocalTimeseriesHost(
			_ifname,
			true --[[ timeseries ]],
			...
			function(hostname, host_ts)
				...
				if dumped_hosts[host_key] == nil then
					if host_ts.initial_point ~= nil then
						...
						ts_dump.host_update_rrd(
							host_ts.initial_point_time,
							host_key,
							host_ts.initial_point,
							ifstats,
							verbose,
							config
						)
	...
	if ntop.isEnterpriseM and ntop.isEnterpriseM() then
		ts_dump.sites_update_rrds(when, ifstats, verbose)
	end

	-- Create RRDs for flow and sFlow probes
	if
		config.flow_devices_rrd_creation == "1"
		and ntop.isEnterpriseM
		and ntop.isEnterpriseM()
		and not highExporterTimeseriesResolution()
	then
