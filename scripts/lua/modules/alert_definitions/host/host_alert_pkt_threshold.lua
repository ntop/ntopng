...
local host_alert_pkt_threshold = classes.class(alert)
...
function host_alert_pkt_threshold:init(metric, value, operator, threshold)
...
function host_alert_pkt_threshold.format(ifid, alert, alert_type_params)
  local alert_consts = require("alert_consts")
  local entity = alert_consts.formatHostAlert(ifid, alert["ip"], alert["vlan_id"])

  return i18n("alert_messages.host_alert_pkt_threshold", {
    entity = entity,
...
-- @param alert_type_params Table `alert_type_params` as built in the `:init` method
-- @return A human-readable string
function host_alert_pkt_threshold.filter_to_past_flows(ifid, alert, alert_type_params)
   local res = {}
...
return host_alert_pkt_threshold
