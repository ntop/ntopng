...
local FLOW_RISK_SIMPLE_CHECK_DEFINITION_PATH =
    os_utils.fixPath(string.format(
                         "%s/scripts/lua/modules/flow_risk_simple_check_definition.lua",
                         dirs.installdir))
local NUM_FILTERED_KEY =
    "ntopng.cache.checks.exclusion_counter.subdir_%s.script_key_%s"
...
local available_subdirs = {
    {
        ...
        filter = {
            ...
            available_fields = {ip = {lint = http_lint.validateIpAddress}}
        }
    }, {
            ...
            available_fields = {
                alert_entity_val = {
                    ...
                    match = function(context, val)
                        -- Do the comparison
                        if not context or context.alert_entity ~=
                            alert_entities_utils.alertEntity("interface") then
                            return false
                        end

                        -- Match on the interface id
                        return tonumber(val) ==
                                   tonumber(context.alert_entity_val)
                    end,
                    sqlite = function(val)
                        -- Keep in sync with SQLite database schema declared in AlertsManager.cpp
                        return string.format(
                                   "(alert_entity = %u AND alert_entity_val = '%s')",
                                   alert_entities_utils.alertEntity("interface"),
                                   val)
                    end,
            ...
            available_fields = {
                alert_entity_val = {
                    ...
                    match = function(context, val)
                        -- Do the comparison
                        if not context or context.alert_entity ~=
                            alert_entities_utils.alertEntity("network") then
                            return false
                        end
                    ...
                    sqlite = function(val)
                        ...
                        return string.format(
                                   "(alert_entity = %u AND alert_entity_val = '%s')",
                                   alert_entities_utils.alertEntity("network"),
                                   val)
                    end,
                    find = function(alert, alert_json, filter, val)
                        return (alert[filter] and (alert[filter] == val))
            ...
            available_fields = {
                alert_entity_val = {
                    ...
                    match = function(context, val)
                        -- Do the comparison
                        if not context or context.alert_entity ~=
                            alert_entities_utils.alertEntity("snmp_device") then
                            return false
                        end
                    ...
                    sqlite = function(val)
                        ...
                        return string.format(
                                   "(alert_entity = %u AND alert_entity_val = '%s')",
                                   alert_entities_utils.alertEntity(
                                       "snmp_device"), val)
                    end,
                    find = function(alert, alert_json, filter, val)
                        return (alert[filter] and (alert[filter] == val))
    ...
    }, {id = checks.SYSTEM_SUBDIR_NAME, label = "system"},
    {id = checks.ACTIVE_MONITORING_SUBDIR_NAME, label = "active_monitoring"},
    {id = checks.AS_SUBDIR_NAME, label = "as"},
    {id = checks.SYSLOG_SUBDIR_NAME, label = "Syslog"}
}
...
checks.script_types = {
    flow = {
        ...
        hooks = {
            "protocolDetected", "statusChanged", "flowEnd", "periodicUpdate",
            "flowBegin"
        },
        subdirs = {"flow"},
...
local function invokeScriptHook(check, configset, hook_fn, p1, p2, p3)
    ...
    if type(hook_fn) == "function" then return (hook_fn(p1, p2, p3)) end
end
...
local function deleteCachePattern(pattern)
    ...
    for key in pairs(keys or {}) do ntop.delCache(key) end
end
...
function checks.getScriptType(search_subdir)
    for _, script_type in pairs(checks.script_types) do
        for _, subdir in pairs(script_type.subdirs) do
            if (subdir == search_subdir) then return (script_type) end
        end
    end
...
local function getSubdirId(subdir_name)
    for id, values in pairs(available_subdirs) do
        if values["id"] == subdir_name then return id end
    end
...
function checks.getSubdirectoryPath(script_type, subdir)
    ...
    path = string.format("%s/scripts/lua/modules/check_definitions/%s",
...
local function loadAndCheckScriptTemplate(check, check_template)
    ...
    if check.script then
        template_require = script_manager.loadTemplate(check.script.key,
                                                       check_template)
    end
    ...
    if not template_require then
        ...
        local template_path = os_utils.fixPath(dirs.installdir ..
                                                   "/scripts/lua/modules/check_templates/" ..
                                                   check_template .. ".lua")
...
local function init_check(check, mod_fname, full_path, script, script_type,
                          subdir)
    check.key = mod_fname
    check.path = full_path
    check.subdir = subdir
    check.default_enabled =
        ternary(check.default_enabled == false, false, true --[[ a nil value means enabled ]] )
    check.script = script
    ...
    check.num_filtered = tonumber(ntop.getCache(
                                      string.format(NUM_FILTERED_KEY, subdir,
                                                    mod_fname))) or 0 -- math.random(1000,2000)

    if script then check.edition = check.edition or script.edition or "" end
    ...
    if check.gui then
        check.template = loadAndCheckScriptTemplate(check,
                                                    check.gui.input_builder)

        if (check.template == nil) then
            traceError(TRACE_WARNING, TRACE_CONSOLE,
                       string.format(
                           "Unknown template '%s' for user script '%s'",
                           check.gui.input_builder, mod_fname))
        end
...
local function loadAndCheckScript(mod_fname, full_path, script, script_type,
                                  subdir, return_all, scripts_filter,
                                  hook_filter)
    local setup_ok = true
        ...
        if (script.edition == "pro" and not (ntop.isPro and ntop.isPro())) or
            ...
            traceError(TRACE_DEBUG, TRACE_CONSOLE,
                       string.format(
                           ...
                           mod_fname, script.edition))
            return (nil)
        end
    end

    traceError(TRACE_DEBUG, TRACE_CONSOLE,
               string.format("Loading user script '%s'", mod_fname))
    local check = dofile(full_path)

    if (type(check) ~= "table") then
        traceError(TRACE_ERROR, TRACE_CONSOLE,
                   string.format("Loading '%s' failed", full_path))
        return (nil)
    end

    if ((not return_all) and check.packet_interface_only and
        (not interface.isPacketInterface())) then
        traceError(TRACE_DEBUG, TRACE_CONSOLE, string.format(
                       "Skipping module '%s' for non packet interface",
                       mod_fname))
        return (nil)
    end
    ...
    if ((not return_all) and (check.windows_exclude and ntop.isWindows())) then
        traceError(TRACE_DEBUG, TRACE_CONSOLE, string.format(
                       "Skipping module '%s' not supported on Windows",
                       mod_fname))
        return (nil)
    end

    if (subdir ~= "flow" and subdir ~= "host" and subdir ~= "interface" and
        table.empty(check.hooks)) then
        traceError(TRACE_WARNING, TRACE_CONSOLE, string.format(
                       "No 'hooks' defined in user script '%s', skipping",
                       mod_fname))
        return (nil)
    end
    ...
    if (check.l7_proto ~= nil) then
        ...
        if (check.l7_proto_id == -1) then
            traceError(TRACE_WARNING, TRACE_CONSOLE,
                       string.format(
                           "Unknown L7 protocol filter '%s' in user script '%s', skipping",
                           check.l7_proto, mod_fname))
            return (nil)
        end
    end
    ...
    if (full_path == FLOW_RISK_SIMPLE_CHECK_DEFINITION_PATH) then
        ...
        if not flow_risk_alert then
            traceError(TRACE_WARNING, TRACE_CONSOLE,
                       string.format(
                           "Module '%s' is not associated to any known flow risk alert",
                           mod_fname))
        else
    ...
    if ((not check.gui) or (not check.gui.i18n_title) or
        (not check.gui.i18n_description)) then
        traceError(TRACE_WARNING, TRACE_CONSOLE, string.format(
                       "Module '%s' does not define a gui [%s]", mod_fname,
                       full_path))
    end
    ...
    if (hook_filter ~= nil) then
        ...
        if ((check.hooks[hook_filter] == nil) and (check.hooks["all"] == nil)) then
            traceError(TRACE_DEBUG, TRACE_CONSOLE, string.format(
                           "Skipping module '%s' for hook '%s'", check.key,
                           hook_filter))
            return (nil)
        end
    end
    ...
    if (scripts_filter ~= nil) then
        ...
        if (not script_ok) then
            traceError(TRACE_DEBUG, TRACE_CONSOLE, string.format(
                           "Skipping module '%s' for scripts_filter", check.key))
            return (nil)
        end
    end

    -- If a setup function is available, call it
    if (check.setup ~= nil) then setup_ok = check.setup() end

    if ((not return_all) and (not setup_ok)) then
        traceError(TRACE_DEBUG, TRACE_CONSOLE,
                   string.format("Skipping module '%s' as setup() returned %s",
                                 check.key, setup_ok))
        return (nil)
    end
...
function checks.load(ifid, script_type, subdir, options)
    ...
    for _, hook in pairs(script_type.hooks) do rv.hooks[hook] = {} end
    ...
    for mod_fname, loadable_check in pairs(loadable_checks) do
        ...
        if (rv.modules[mod_fname]) then
            traceError(TRACE_ERROR, TRACE_CONSOLE, string.format(
                           "Skipping duplicate module '%s'", mod_fname))
            goto next_module
        end

        local check = loadAndCheckScript(mod_fname, full_path, script,
                                         script_type, subdir, return_all,
                                         scripts_filter, hook_filter)

        if (not check) then goto next_module end
        ...
        for hook, hook_fn in pairs(check.hooks or {}) do
            if (rv.hooks[hook] == nil) then
                traceError(TRACE_WARNING, TRACE_CONSOLE, string.format(
                               "Unknown hook '%s' in module '%s'", hook,
                               check.key))
            else
                rv.hooks[hook][check.key] = hook_fn
            end
        end
        -- tprint(check.key .. " / " .. full_path)
        rv.modules[check.key] = check

        ::next_module::
    end

    if (old_ifid ~= ifid) then interface.select(tostring(old_ifid)) end
...
function checks.loadModule(ifid, script_type, subdir, mod_fname, return_all)
    ...
    for _, checks_dir in pairs(check_dirs) do
        local full_path = os_utils.fixPath(
                              checks_dir .. "/" .. mod_fname .. ".lua")

        if ntop.exists(full_path) then
            check = loadAndCheckScript(mod_fname, full_path, script,
                                       script_type, subdir, return_all)
            break
        end
    end
    ...
    if not check and subdir == "flow" then
        ...
        if flow_risk_alert then
            check = loadAndCheckScript(mod_fname,
                                       FLOW_RISK_SIMPLE_CHECK_DEFINITION_PATH,
                                       script, script_type, subdir)
        end
    end
...
function checks.teardown(available_modules)
    for _, script in pairs(available_modules.modules) do
        if script.teardown then script.teardown() end
    end
end
...
function checks.loadUnloadUserScripts(is_load)
    ...
    for _, subdir in ipairs(checks.listSubdirs()) do
        ...
        local scripts = checks.load(interface.getId(),
                                    checks.getScriptType(subdir.id), subdir.id,
                                    {return_all = true})
        ...
        for name, script in pairsByKeys(scripts.modules) do
            ...
            if not configset.config then
                traceError(TRACE_ERROR, TRACE_CONSOLE,
                           string.format("Configuration is missing"))
                return
            end

            if not configset.config[subdir.id] then
                traceError(TRACE_ERROR, TRACE_CONSOLE, string.format(
                               "Missing subdir '%s' from config", subdir.id))
                return
            end
            ...
            if not configset.config[subdir.id][script.key] then
                ...
                traceError(TRACE_NORMAL, TRACE_CONSOLE, string.format(
                               "Script '%s' configuration is missing from subdir '%s'. New user script?",
                               script.key, subdir.id))
            else
...
local function filterIsEqual(applied_config, new_filter)
    ...
    for counter, filter in pairs(applied_config) do
        if table.compare(filter, new_filter) then return 0 end
...
function checks.updateScriptConfig(script_key, subdir, new_config)
    ...
    local script = checks.loadModule(interface.getId(), script_type, subdir,
                                     script_key)
    ...
    if (script) then
        ...
        for hook, conf in pairs(new_config) do
            ...
            if (not valid) then return false, rv_or_err end
    ...
    if not configset["filters"] then configset["filters"] = {} end
    ...
    if script then
        ...
        for hook, hook_config in pairs(prev_config) do
            ...
            if hook_applied_config then
                if script.onDisable and hook_config.enabled and
                    not hook_applied_config.enabled then
                    -- Hook previously disabled has been enabled
                    script.onDisable(hook, hook_applied_config, confid)
                elseif script.onEnable and not hook_config.enabled and
                    hook_applied_config.enabled then
                    -- Hook previously enabled has now been disabled
                    script.onEnable(hook, hook_applied_config, confid)
                elseif script.onUpdateConfig and
                    not table.compare(hook_config, applied_config[hook]) then
...
local function toggleScriptConfigset(configset, script_key, subdir, enable)
    ...
    config, inconsistency_found = checks.getScriptConfig(configset, script,
                                                         subdir)
    ...
    if (not configset["config"][subdir][script_key]) or
        (table.len(configset["config"][subdir][script_key]) == 0) or
        inconsistency_found then
...
function checks.toggleScript(script_key, subdir, enable)
    ...
    local res, err =
        toggleScriptConfigset(configset, script_key, subdir, enable)
...
function checks.toggleAllScripts(subdir, enable)
    ...
    -- Toggle the configuration (result is put in `configset`)
    local scripts = checks.load(getSystemInterfaceId(),
                                checks.getScriptType(subdir), subdir)

    for script_name, script in pairs(scripts.modules) do
        -- Toggle each script individually
        local res, err = toggleScriptConfigset(configset, script.key, subdir,
                                               enable)
        if not res then return res, err end
    end
    ...
    for type_id, script_type in pairs(checks.script_types) do
        for _, subdir in pairs(script_type.subdirs) do
            ...
            for key, usermod in pairs(scripts.modules) do
                ...
                local hooks = ternary(script_type.has_per_hook_config,
                                      usermod.hooks,
...
function checks.initDefaultConfig()
    ...
    for type_id, script_type in pairs(checks.script_types) do
        for _, subdir in pairs(script_type.subdirs) do
            ...
            for key, usermod in pairs(scripts.modules) do
                ...
                if usermod.default_enabled ~= nil or usermod.default_value ~=
                    nil then
                    ...
                    if default_conf[subdir] and default_conf[subdir][key] then
                        ...
                        default_conf[subdir] = default_conf[subdir] or {}
                        default_conf[subdir][key] =
                            default_conf[subdir][key] or {}
                        local script_config = default_conf[subdir][key]
                        local hooks = ternary(script_type.has_per_hook_config,
                                              usermod.hooks,
                                              {[ALL_HOOKS_CONFIG_KEY] = 1})
                ...
                if usermod.filter and usermod.filter.default_filters then
                    ...
                    if default_filters[subdir][key] then
                        ...
                        -- Add filter
                        default_filters[subdir][key] = usermod.filter
                                                           .default_filters
                    end
                end
            end
    ...
    configset = {config = default_conf, filters = default_filters}
...
function checks.isCheckEnabled(entity_name, script_key)
    ...
    if (script_config) then
        for _, hook in pairs(script_config) do
            if (hook.enabled) then return (true) end
        end
    end
...
function checks.isSystemScriptEnabled(script_key)
    ...
    if (isEmptyString(available)) then
        local m = checks.loadModule(getSystemInterfaceId(),
                                    checks.script_types.system, "system",
                                    script_key)
        available = (m ~= nil)
...
local default_config = {enabled = false, script_conf = {}}
...
function checks.getScriptConfig(configset, script, subdir)
    ...
    if (config) and (config[script_key]) and (table.len(config[script_key]) > 0) then
        ...
        if (table.len(ret) > 0) then return ret, inconsistency_found end
...
function checks.getTargetHookConfig(target_config, script, hook)
    ...
    if (not script_conf) then return (default_config) end
    ...
    for key, value in pairs(default_values) do
        if not conf.script_conf[key] then conf.script_conf[key] = value end
    end
...
function checks.getScriptEditorUrl(script)
    if (script.edition == "community" and script.source_path) then
        local script_file_path = string.sub(script.source_path,
                                            string.len(dirs.scriptdir) + 1)
        local script_path = string.sub(script.script.path,
                                       string.len(dirs.scriptdir) + 1)
        return (string.format(
                   "%s/lua/code_viewer.lua?script_file_path=%s&script_path=%s",
                   ntop.getHttpPrefix(), script_file_path, script_path))
    end
...
function checks.getFilterPreset(alert, alert_info)
    ...
    if not alert_generation then return '' end

    local subdir = alert_generation["subdir"]
    local subdir_id = getSubdirId(subdir)

    if subdir_id == -1 then return '' end

    if not available_subdirs[subdir_id]["filter"] then return '' end
    ...
    for _, field in pairs(filter_to_use) do
        ...
        if not field_val then field_val = alert_info[field] end
...
function checks.prepareFilterSQLiteWhere(subdir, check, filter)
    ...
    local script_where = {
        string.format(
            "json_extract(alert_json, '$.alert_generation.subdir') = '%s'",
            subdir), string.format(
            "json_extract(alert_json, '$.alert_generation.script_key') = '%s'",
            check)
    }
    ...
    local available_fields =
        available_subdirs[subdir_id]["filter"]["available_fields"]
...
function checks.excludeScriptFilters(alert, alert_json, script_key, subdir)
    ...
    if not applied_filter_config then return false end
    ...
    for _, values in pairs(applied_filter_config) do
        ...
        for filter, value in pairs(values) do
            ...
            local find_value =
                available_subdirs[subdir_id]["filter"]["available_fields"][filter]["find"]
...
local function printUserScriptsTable()
    ...
    for _, info in ipairs(checks.listSubdirs()) do
        ...
        for name, script in pairsByKeys(scripts.modules) do
            ...
            -- Execution stats
            if info.id == 'flow' and flow_checks_stats[name] and
                flow_checks_stats[name].stats and
                flow_checks_stats[name].stats.execution_time then
                -- Importing just when needed
                local format_utils = require("format_utils")
                tot_exec_time = format_utils.msToTime(
                                    flow_checks_stats[name].stats.execution_time /
                                        1000000)
            end
            ...
            print(string.format(([[
                ...
                ]]), name, info.label, available, hooks, filters,
                                script.num_filtered, tot_exec_time or "",
                                edit_url or ""))
            ::skip::
        end
    end
                ...
                <table class='table table-bordered table-striped' id='user-scripts'>
                    <thead>
                        <tr>
                            ...
                            <th>]] .. i18n("scripts_overview.filters") ..
              [[</th>
                            <th>]] .. i18n("scripts_overview.filtered") ..
              [[</th>
                            <th>]] ..
              i18n("scripts_overview.total_elapsed_time") .. [[</th>
                            <th>]] .. i18n("action") .. [[</th>
                        </tr>
                    </thead>
                    <tbody>]])
    ...
    $(document).ready(function() {
        ...
        const $userScriptsTable = $('#user-scripts').DataTable({
            ...
            initComplete: function(settings) {
                ...
                addFilterDropdown(']] .. i18n("availability") ..
              [[', availability, 2, "#user-scripts_filter", table);
                addFilterDropdown(']] .. i18n("scripts_overview.type") ..
              [[', types, 1, "#user-scripts_filter", table);
            },
            pageLength: 25,
            language: {
                info: "]] ..
              i18n('showing_x_to_y_rows',
                   {x = '_START_', y = '_END_', tot = '_TOTAL_'}) .. [[",
                search: "]] .. i18n('search') .. [[:",
                infoFiltered: "",
                paginate: {
                    previous: '&lt;',
...
local function snmp_device_run_checks(cached_device, checks_var)
    ...
    for mod_key, hook_fn in pairs(
                                checks_var.available_modules.hooks["snmpDevice"] or
                                    {}) do
        local script = all_modules[mod_key]
        local conf = checks.getTargetHookConfig(device_conf, script)

        if (conf.enabled) then
            invokeScriptHook(script, checks_var.configset, hook_fn, device_ip,
                             info, conf)
        end
    end
    ...
    for mod_key, hook_fn in pairs(
                                checks_var.available_modules.hooks["snmpDeviceInterface"] or
                                    {}) do
        local script = all_modules[mod_key]
        local conf = checks.getTargetHookConfig(device_conf, script)

        -- For each interface of the current device...
        for snmp_interface_index, snmp_interface in pairs(
                                                        cached_device.interfaces) do
            if (script.skip_virtual_interfaces and
                snmp_utils.isVirtualInterfaceType(snmp_interface.type)) then
                goto continue
            end

            if (conf.enabled) then
                local iface_entity = alert_entity_builders.snmpInterfaceEntity(
                                         device_ip, snmp_interface_index)

                -- Augment data with counters and status
                snmp_interface["if_counters"] =
                    cached_device.if_counters[snmp_interface_index]
                snmp_interface["bridge"] =
                    cached_device.bridge[snmp_interface_index]

                invokeScriptHook(script, checks_var.configset, hook_fn,
                                 device_ip, snmp_interface_index,
...
local function setupHostChecks(str_granularity, checks_var, do_trace)
    ...
    checks_var.available_modules = checks.load(checks_var.ifid,
                                               checks.script_types
                                                   .traffic_element, "host",
...
local function setupInterfaceChecks(str_granularity, checks_var, do_trace)
    ...
    checks_var.configset = checks.getConfigset()
    -- Retrieve the configuration associated to the confset
    checks_var.iface_config =
        checks.getConfig(checks_var.configset, "interface")
end
...
local function setupLocalNetworkChecks(str_granularity, checks_var, do_trace)
    ...
    checks_var.available_modules = checks.load(ifid, checks.script_types
                                                   .traffic_element, "network",
                                               {hook_filter = str_granularity})
...
local function setupSystemChecks(str_granularity, checks_var, do_trace)
    if do_trace then
        traceError(TRACE_NORMAL, TRACE_CONSOLE, "system.lua:setup(%s) called",
                   str_granularity)
    end
...
local function setupSNMPChecks(str_granularity, checks_var, do_trace)
    ...
    -- Load the threshold checking functions
    checks_var.available_modules = checks.load(ifid,
                                               checks.script_types.snmp_device,
                                               "snmp_device")
    checks_var.configset = checks.getConfigset()
...
local function runHostChecks(granularity, checks_var, do_trace)
    if table.empty(checks_var.available_modules.hooks[granularity]) then
        if (do_trace) then
            print("host:runScripts(" .. granularity ..
                      "): no modules, skipping\n")
        end
        return
    end
    ...
    local entity_info = alert_entity_builders.interfaceAlertEntity(
                            checks_var.ifid)
...
local function runInterfaceChecks(granularity, checks_var, do_trace)
    if table.empty(checks_var.available_modules.hooks[granularity]) then
        if (do_trace) then
            print("interface:runScripts(" .. granularity ..
                      "): no modules, skipping\n")
        end
        return
    end
...
local function runLocalNetworkChecks(granularity, checks_var, do_trace)
    if table.empty(checks_var.available_modules.hooks[granularity]) then
        if (do_trace) then
            print("network:runScripts(" .. granularity ..
                      "): no modules, skipping\n")
        end
        return
    end
...
local function runSystemChecks(granularity, checks_var, do_trace)
    ...
    if table.empty(checks_var.available_modules.hooks[granularity]) then
        if (do_trace) then
            print("system:runScripts(" .. granularity ..
                      "): no modules, skipping\n")
        end
        return
    end
    ...
    for mod_key, hook_fn in pairs(
                                checks_var.available_modules.hooks[granularity]) do
        local check = checks_var.available_modules.modules[mod_key]
        local conf = checks.getTargetHookConfig(checks_var.system_config, check,
                                                granularity)

        if (conf.enabled) then
            invokeScriptHook(check, checks_var.configset, hook_fn, {
                granularity = granularity,
                alert_entity = alert_entity_builders.interfaceAlertEntity(
            ...
            if interface.getId() ~= tonumber(getSystemInterfaceId()) then
                traceError(TRACE_ERROR, TRACE_CONSOLE, string.format(
                               "Script '%s' changed the interface from '%d' to '%d'. Resetting interface.",
                               mod_key, tonumber(getSystemInterfaceId()),
                               interface.getId()))
                interface.select(getSystemInterfaceId())
            end
        end
...
local function runActiveMonitoringChecks(granularity, checks_var, do_trace)
    ...
    if table.empty(checks_var.available_modules.hooks[granularity]) then
        if (do_trace) then
            print("active_monitoring:runScripts(" .. granularity ..
                      "): no modules, skipping\n")
        end
        return
    end
    ...
    local info = interface.getStats()
    local when = os.time()
    local active_monitoring_conf = checks.getConfig(checks_var.configset,
                                                    "active_monitoring")

    for mod_key, hook_fn in pairs(
                                checks_var.available_modules.hooks[granularity]) do
        local check = checks_var.available_modules.modules[mod_key]
        local conf = checks.getTargetHookConfig(active_monitoring_conf, check,
                                                granularity)
...
local function runASChecks(granularity, checks_var, do_trace)
    if do_trace then print("runASChecks(" .. granularity .. ") called\n") end
...
function checks.asChecks(granularity, checks_var, do_trace)
    if not setupASChecks(granularity, checks_var, do_trace) then return false end
...
-- @brief In some cases, alerts needs to be generated outside the checks routine,
--        this means that the params variable is empty, so create a custom params
--        in order to trigger the alert.
-- @params granularity: the key of the granularity (e.g. min), see scripts/lua/modules/alert_granularities.lua
-- @params alert_entity_name: name of the alert entity in string format
-- @params severity: ID Name of the alert severity, (e.g. notice) see: scripts/lua/modules/alert_severities.lua
...
function checks.generateCustomParams(granularity, alert_entity_name, severity,
    ...
    if (not granularity) or (not alert_granularities[granularity]) then
        traceError(TRACE_WARNING, TRACE_CONSOLE, string.format(
                       "Incorrect granularity requested when generating the params [requested: %s, NOT FOUND]",
                       granularity or "nil"))
        return {}
    end
    ...
    elseif alert_entity_name == "mac" then
        entity_info = alert_entity_builders.macEntity(alert_info.mac)
    elseif alert_entity_name == "user" then
        entity_info = alert_entity_builders.userEntity(alert_info.user)
