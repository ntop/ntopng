...
local REDIS_NETWORKS_SITES_KEY = "ntopng.prefs.networks.sites.v2" -- Stores all sites as hash: id -> JSON
...
local SITE_SCHEMA = {
   ...
   { key = "networks", default = "", exported = true },
}
...
function site_utils.addSite(site)
   ...
   if res then
      ...
      site_id = tostring(current_count)
...
function site_utils.getNetworkSite(network_id)
   ...
   local net_stats = interface.getNetworkStats(tonumber(network_id)) or nil

   if not net_stats then
      -- Not a network, return
      return site_utils.get_default_site()
   end
   ...
   for _, stats in pairs(net_stats or {}) do
      site = ntop.getHashCache(REDIS_NETWORKS_SITES_KEY, stats.network_key)
   end
...
function site_utils.mapNetworkCIDRToSite(network_cidr, site_id)
   if isEmptyString(network_cidr) or isEmptyString(site_id) then
      return false
   end
   ntop.setHashCache(REDIS_NETWORKS_SITES_KEY, network_cidr, tostring(site_id))
   invalidate_site_networks_cache()
   return true
end
...
function site_utils.mapNetworkToSite(network_id, site_id)
   ...
   if get_sites_from_cache()[tostring(site_id)] then
      ...
      for _, stats in pairs(net_stats or {}) do
         ntop.setHashCache(REDIS_NETWORKS_SITES_KEY, stats.network_key, tostring(site_id))
      end
      invalidate_site_networks_cache()
      return true
   end
...
function site_utils.getAllNetworksToSite()
   ...
   for net_cidr, info in pairsByKeys(active_networks) do
      local site_id_associated = associations[tostring(info.network_key)]
   ...
   for _, site in pairsByKeys(sites, asc) do
	 ...
	 for _, f in ipairs(exported_schema) do
	    ...
	    entry[f.key] = v
         ...
         entry.networks = ternary(associations[tostring(entry.id)], json.encode(associations[tostring(entry.id)]), nil)
...
function site_utils.refreshAllNetworksSiteIdMappings()
...
function import_site_entry(ctx, entry)
   ...
   if not isEmptyString(name) then
	 ...
	 local rc, msg, site_id = site_utils.addSite(input)
         local networks = json.decode(entry.networks or "") or {}

         for _, stats in pairs(networks) do
            site_utils.mapNetworkCIDRToSite(stats.network_cidr, site_id)
         end
...
function site_utils.importSites(entries)
   ...
   site_utils.refreshAllNetworksSiteIdMappings()
...
-- Convert the key mapping between networks and sites
-- from network_id -> site to network_cidr -> site
-- in this way it will work better when importing/exporting
-- the configuration in other machines
function site_utils.startup()
   local old_sites_networks_mapping_key = "ntopng.prefs.networks.sites"
   local mapping_list = ntop.getHashAllCache(old_sites_networks_mapping_key) or {}
   for network_id, site_id in pairs(mapping_list) do
      -- Only if the network is present at startup (-m option), convert the key, otherwise
      -- keep it, in case the user will add it in the future
      local net_stats = interface.getNetworkStats(tonumber(network_id))
      for _, stats in pairs(net_stats or {}) do
	 local net_cidr = stats.network_key
	 ntop.delHashCache(old_sites_networks_mapping_key, tostring(network_id))
	 ntop.setHashCache(REDIS_NETWORKS_SITES_KEY, net_cidr, site_id)
      end
   end

   local ret = ntop.getNetworks()

   for net,id in pairs(ret) do
      local alias = ntop.getNetworkAliasById(id)

      if(alias ~= nil) then
         local label = net .. "=" .. alias
         local ret = site_utils.mapNetworkToSitePath(id, label)
         
	 -- tprint("Mapping siteId ".. id .. " to " .. label)
         -- tprint(ret)
      end
   end
end
