...
function flowfilter_utils.get_flowfilter_info(id, entity, hide_exporters_name, restrict_filters, ifid, is_aggregated)
    ...
    elseif filter_def.value_type == "snmp_interface" then
        if ntop.isPro and ntop.isPro() then
            -- Options are not built eagerly. The frontend instead searches/paginates this list on
            -- demand via remote_url (see snmp_interface_options.lua).
            filter.value_type = 'array'
            filter.remote_url = "/lua/pro/rest/v2/get/flow/snmp_interface_options.lua"
        end
    end
