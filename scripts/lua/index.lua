...
if ntop.isnEdge() or ntop.isAppliance() then
    ...
    if sys_config.isFirstStart() then
        print(ntop.httpRedirect(
                  ntop.getHttpPrefix() .. "/lua/system_setup_ui/" ..
                      first_start_page))
        return
    end
end
...
if interface.isPcapDumpInterface() then
    ...
    print(ntop.httpRedirect(ntop.getHttpPrefix() .. "/lua/if_stats.lua?ifid=" ..
...
page_utils.print_header_and_set_active_menu_entry(
    page_utils.menu_entries.traffic_dashboard)
...
local context = {
    ...
    template_list_endpoint = ntop.getHttpPrefix() ..
        "/lua/rest/v2/get/dashboard/template/list.lua"
}

local json_context = json.encode(context)

template_utils.render("pages/vue_page.template", {
    vue_page_name = "Dashboard",
