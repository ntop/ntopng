...
local base_url = ntop.getHttpPrefix() .. '/lua/pool_details.lua?pool=' ..
                     pool_id
local page_params = {}
...
if (ntop.isEnterpriseM and ntop.isEnterpriseM() or ntop.isnEdge and ntop.isnEdge()) and pool_id ~=
    ...
    print(template.gen("modal_confirm_dialog.html", {
        dialog = {
            ...
            message = i18n("host_pools.confirm_reset_pool_quotas",
                           {pool = pool_name}),
  ...
  <button class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#reset_quotas_dialog" style="float:right;">]]
    ...
    if ntop.isnEdge and ntop.isnEdge() then
        ...
        print [[/lua/pro/nedge/admin/nf_edit_user.lua?page=categories&username=]]
        print(username)
        print [["><button class="btn btn-secondary" type="button" style="float:right; margin-right:1em;">]]
        print(i18n("nedge.edit_quotas"))
        print [[</button></a>]]
    end
    print [[<br/><br/>]]
