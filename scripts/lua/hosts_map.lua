...
for i,v in pairs(MODES) do
	dropdown_menu[#dropdown_menu+1] = '<a class="dropdown-item '.. (bubble_mode == v.mode and 'active' or '') ..'" tabindex="-1" href="?bubble_mode='..tostring(v.mode)..'">'..v.label..'</a>'
end

-- register the bubble chart for the hosts map
widget_gui_utils.register_bubble_chart(widget_name, 0, {
	Datasource("rest/v1/charts/host/map.lua", {bubble_mode = bubble_mode})
})

template_utils.render("pages/hosts_map.template", {
	widget_gui_utils = widget_gui_utils,
	hosts_map = {
		dropdown_menu = table.concat(dropdown_menu, ''),
