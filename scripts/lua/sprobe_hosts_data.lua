...
for key, _ in pairs(hosts) do
   ...
   print('\n{"source": "root", "source_num": 0, "source_type": "host", "source_pid": -1, "source_name": "root", "target": "'..key..'", "target_num": 0, "target_type": "host", "target_pid": -1, "target_name": "'.. getResolvedHostAddress(hostkey2hostinfo(key))..'", "type": "host2host"}')
   n = n + 1
end
end
...
for key, value in pairs(links) do
   if(n > 0) then print(",") end
   print('\n{"source": "'..key..'", "source_num": '.. links[key]["num"]..', "source_type": "host", "source_pid": -1, "source_name": "'..getResolvedHostAddress(hostkey2hostinfo(key))..'", "target": "'..value["peer"]..'", "target_num": '.. value["num"]..', "target_type": "host", "target_pid": -1, "target_name": "'.. getResolvedHostAddress(hostkey2hostinfo(value["peer"]))..'", "type": "host2host"}')
   n = n + 1
end
