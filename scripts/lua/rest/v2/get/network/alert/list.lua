...
if not download then
   local alerts, recordsFiltered, info = network_alert_store:select_request(nil, "*")
   ...
   extra_headers["Content-Disposition"] = "attachment;filename=\"network_alerts_export_"..os.time()..".csv\""
   rest_utils.vanilla_payload_response(rest_utils.consts.success.ok, rsp, "application/octet-stream", extra_headers)
   network_alert_store:select_request(nil, "*", download)
end
