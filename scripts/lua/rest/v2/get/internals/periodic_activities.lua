...
for script, stats in pairsByKeys(scripts_stats, asc) do
   if not isEmptyString(filter_script) and script ~= filter_script then goto continue end
   ...
   -- Apply issue filter after the row is built
   if not isEmptyString(filter_issue) then
      ...
      if not has_issue then
         data[#data] = nil -- remove the row we just added
      end
   end
