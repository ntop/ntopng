...
for _, value in ipairs(flows_stats.flows) do
   ...
   if (value["in_index"] ~= nil and value["out_index"] ~= nil) then
      ...
      if ntop.isPro and ntop.isPro() then
         ...
         out_port_name = format_portidx_name(device_ip, value["out_index"], true)
         -- get exporter info
   ...
   record["throughput"] = {
      ...
      bps = (8 * value["throughput_bps"] or 0), -- This is in BYTES per seconds, see Flow.cpp:7005!
   ...
   if (not isEmptyString(value["protos.http.last_method"])) then
      ...
      record["application"]["rsp_status_code"] = http_utils.getResponseStatusCode(value["protos.http.last_return_code"])
   end
