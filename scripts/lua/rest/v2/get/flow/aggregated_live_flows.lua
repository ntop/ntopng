...
local function build_response()
    ...
    for _, data in pairs(aggregated_info or {}) do
        ...
        elseif (criteria_type_id == 2) then
            if (vlan and (tonumber(vlan) ~= tonumber(data.cli_vlan_id))) then
                goto continue
            end
        elseif (criteria_type_id == 3 or criteria_type_id == 5) then
            if (vlan and (tonumber(vlan) ~= tonumber(data.srv_vlan_id))) then
                goto continue
            end
        else
            if (vlan and tonumber(vlan) ~= tonumber(data.vlan_id)) then
        ...
        if (criteria_type_id == 3 or criteria_type_id == 4 or criteria_type_id == 5) then
            local srv_info = set_host_info(data.srv_vlan_id, data.server_ip, data.server_name, data.is_srv_in_mem,
                data.vlan_id)
        ...
        if (criteria_type_id == 2 or criteria_type_id == 4 or criteria_type_id == 5) then
            local cli_info = set_host_info(data.cli_vlan_id, data.client_ip, data.client_name, data.is_cli_in_mem,
                data.vlan_id)
