...
if not download then
   local alerts, recordsFiltered, info = mac_alert_store:select_request(nil, "*")
   ...
   extra_headers["Content-Disposition"] = "attachment;filename=\"mac_alerts_export_"..os.time()..".csv\""
   rest_utils.vanilla_payload_response(rest_utils.consts.success.ok, rsp, "application/octet-stream", extra_headers)
   mac_alert_store:select_request(nil, "*", download)
end
