local lang = {
  ...
  ["device_protocols_description"] = "このページではdevice typeとdirectionごとに許容可能なアプリケーションプロトコルを設定可能です。（アプリケーションプロトコル観点でクライアントあるいはサーバとしてデバイスを設定できます). Preference配下のAlert設定でDevice Protocols Alertsを有効化することにより, anomalous protocolを検出するとalertsが生成されます",
  ...
  ["alert_messages"] = {
    ...
    ["host_remote_to_remote"] = "リモートホスト <a href=\"%{url}\">%{ip}</a> [ <a href=\"%{mac_url}\">%{mac}</a> ] が別のリモートホストに接続しました。 Remote-to-remote フローは<a href=\"%{flow_alerts_url}\">flow alerts page</a>で閲覧可能です",
  ...
  ["delete_data"] = {
    ...
    ["note_persistent_data"] = "すべての永続データが削除されました。データには蓄積されたtimeseries, flows, alerts, and Redisキャッシュを含みます",
  },
  ["details"] = {
  ...
  ["nedge"] = {
    ...
    ["data_reset_msg"] = "すべてのデバイスデータを本当に削除しますか? データにはstored timeseries, flows, alerts, Redis cachesとhost countersを含みます. システムセットアップのデバイス設定はリセットにより影響を受けません。<br>これは%{product}の再起動につながり、このweb GUIは数秒無反応になります",
  ...
  ["network_alert_config"] = {
    ["trigger_alerts_for_network"] = "Network %{network}のalertをトリガします",
  ...
  ["prefs"] = {
    ...
    ["disable_alerts_generation_description"] = "すべてのalertsの生成をオン／オフにする",
    ...
    ["max_num_alerts_per_entity_description"] = "alarmable entityごとの最大alerm数。Alarmable entitiesはhosts, networks, interfaces。 Alertsは周期的にチェックされ、エンティティごとの最大数を超えた場合には, もっとも古いalertsは削除され、設定された最大数の80%%を保つようにされます。 Default: 1024.",
    ["max_num_alerts_per_entity_title"] = "Entityごとの最大Alert数",
    ...
    ["max_num_flow_alerts_description"] = "flow alertsの最大数。Alertsは周期的にチェックされ、エンティティごとの最大数を超えた場合には, もっとも古いalertsは削除され、設定された最大数の80%%を保つようにされます。 Default: 16384.",
    ...
    ["syslog_alert_format_description"] = "syslog alertsの出力フォーマットを選択して下さい",
    ...
    ["toggle_alert_nagios_description"] = "Nagios NSCA (Nagios Service Check Acceptor)を介してalerts notificationsをオン・オフする",
    ...
    ["toggle_alerts_notifications_description"] = "third-party endpointsへのalert通知をオン・オフする",
    ...
    ["toggle_dns_alerts_description"] = "DNS queryが無効な場合にalertの生成をオンオフする",
    ["toggle_dns_alerts_title"] = "DNS Alerts",
    ["toggle_dropped_flows_alerts_description"] = "設定されたpolicyに従いflowがブロックされたときのinformative alertの生成を有効にする",
    ...
    ["toggle_elephant_flows_alerts_description"] = "elephant flowが検出された場合のalert生成をオン・オフする。 これは望ましくない動作 (例. data exfiltration)を回避するために有効です",
    ["toggle_elephant_flows_alerts_title"] = "Elephant Flows Alerts",
    ["toggle_email_notification_description"] = "emailでのalerts notificationsをオン・オフする",
    ["toggle_email_notification_title"] = "Toggle Email Notification",
    ["toggle_flow_alerts_iface_description"] = "network interfaceがalertされた際のflow alert生成をオン・オフする",
    ...
    ["toggle_longlived_flows_alerts_description"] = "long-lived flowが検出された際のalert生成をオン・オフします。 これは望ましくない動作(例. data exfiltration)の検出に役立ちます",
    ...
    ["toggle_mining_alerts_description"] = "cryptocurrencies miningホストからのデータ送受信を検出した際のalertをオン・オフします",
    ...
    ["toggle_remote_to_remote_alerts_description"] = "flowの client と serverがリモートの場合のalert生成をオン・オフします",
    ...
    ["toggle_slack_notification_description"] = "slackを利用したalerts notificationsをオン・オフします。 設定方法は<a href='%{url}'>here</a>を参照して下さい",
    ...
    ["toggle_ssl_alerts_description"] = "serverにより提供されたSSL certificateがcertificate Common nameと一致しない場合のalert生成をオン・オフします",
  ...
  ["show_alerts"] = {
    ...
    ["alerts"] = "Alerts",
    ["alerts_are_disabled_message"] = "Alertsは無効化されています。 有効化するためにはpreferencesページをチェックしてください",
    ...
    ["delete_config_message"] = "%{conf}sのcommon %{granularity} alerts設定を本当に削除しますか",
    ...
    ["host_delete_config_btn"] = "Host %{host} の設定されたAlertsを本当に削除しますか",
    ...
    ["iface_delete_config_confirm"] = "インタフェースに対する%{granularity} alerts設定を本当に削除しますか",
    ...
    ["last_minute"] = "Last Minute",
    ["minute"] = "Minute",
    ["network_delete_config_btn"] = "Network %{network}設定されたAlertsを削除する",
    ["network_delete_config_confirm"] = "ネットワークの %{granularity} alerts設定を本当に削除しますか",
    ["no_recorded_alerts_message"] = "インタフェース %{ifname}に対して記録されているalertはありません",
    ...
    ["purge_subj_alerts_confirm"] = "すべての %{subj} alertsを本当に削除しますか?",
    ["reset_alert_database"] = "Flush Alerts Data",
    ["reset_alert_database_message"] = "このアクションはすべてのalert関連のデータを削除します。このデータには<ul><li>Runtime alerts status</li><li>Alerts configuration</li><li>Persistent alerts SQLite storage</li></ul>を含みます<br>本当によろしいでしょうか",
    ["startup"] = "Startup",
  },
  ["sites_page"] = {
    ["contacts"] = "Contacts",
  ...
  ["user_activity"] = {
    ["alerts_deleted"] = "ユーザ '%{user}' purged '%{status}' alerts.",
