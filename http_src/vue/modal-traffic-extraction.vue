				...
				<div class="tab-content" style="height:100%;">
					<div class="row">
						<div class="form-group mb-3 col-md-3 has-feedback">
							<button class="btn btn-sm btn-secondary" type="button" @click="show_hide_menu">{{ i18n('advanced') }}<i
									:class="{ 'fas fa-caret-down': show_menu, 'fas fa-caret-up': !show_menu }"></i></button>
						</div>

						<div class="form-group mb-3 col-md-9 text-right asd">
							<label class="radio-inline" style="margin-left: 1rem;"><input type="radio" name="extract_now"
									v-model="extract_now" value="true" checked=""> {{ i18n('traffic_recording.extract_now') }} </label>
							<label class="radio-inline"><input type="radio" name="extract_now" v-model="extract_now" value="false">
								{{ i18n('traffic_recording.queue_as_job') }} </label>
						</div>
					</div>
					...
					<div v-show="show_menu" class="row" id="pcapDownloadModal_advanced" style="">
						<div class="form-group mb-3 col-md-12 has-feedback">
							<br>
							<label class="form-label">{{ i18n('traffic_recording.filter_bpf') }} <a class="ntopng-external-link"
									href="https://www.ntop.org/guides/n2disk/filters.html"><i
										class="fas fa-external-link-alt"></i></a></label>
							<div class="input-group">
								<span class="input-group-addon"><span class="glyphicon glyphicon-filter"></span></span>
								<input name="bpf_filter" v-model="bpf_filter" class="form-control input-sm" data-bpf="bpf"
									autocomplete="off" spellcheck="false">
								<span v-show="invalid_bpf" style="margin: 0px;padding:0;display:block;"
									class="invalid-feedback ">{{ i18n('invalid_value') }}</span>
							</div>
							<br>
