        ...
        <div class="card-body">
          ...
          <div id="table_devices_vue">
            ...
            <modal-add-device-exclusion ref="modal_add_device" :title="title_add" :body="body_add" :footer="footer_add"
              :list_notes="list_notes_add" @add="add_device">
            </modal-add-device-exclusion>
            <modal-edit-device-exclusion ref="modal_edit_device" :title="title_edit" @edit="edit_row">
            </modal-edit-device-exclusion>

            <datatable ref="table_devices_exclusion" :table_buttons="config_devices.table_buttons"
              :columns_config="config_devices.columns_config" :data_url="config_devices.data_url"
...
export default {
  ...
  methods: {
    ...
    delete_row: async function () {
      ...
      let params = { device: row.mac_address.mac, csrf: this.$props.page_csrf };
      let url = `${http_prefix}/lua/pro/rest/v2/delete/device/exclusion.lua`;
      try {
        let headers = {
          'Content-Type': 'application/json'
    ...
    edit_row: async function (params) {
      ...
      if (params.mac_alias != null)
        params.mac_alias = params.mac_alias.replaceAll(/(?:\t| )/g, '');
