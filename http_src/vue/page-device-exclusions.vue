    ...
    configDevices.columns_config = [
      {
        ...
        render: function (rowData, type, script) {
          ...
          return DataTableUtils.createActionButtons([
            { class: `pointer`, handler: jump_to_historical_flow, icon: 'fa-stream', title: i18n('db_explorer.historical_data'), hidden: DatatableVue.is_clickhouse_enabled },
...
export default {
    ...
    methods: {
        ...
        delete_row: async function() {      
          ...
          let params = { device: row.mac_address, csrf: page_csrf };
          let url = `${http_prefix}/lua/pro/rest/v2/delete/device/exclusion.lua`;
          try {
            let headers = {
              'Content-Type': 'application/json'
