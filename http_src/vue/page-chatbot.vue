  ...
  <div class="llm-chat-page d-flex" style="height: 90vh; min-height: 500px;">
      ...
      <div class="chat-sidebar-inner d-flex flex-column h-100">
        ...
        <div class="sidebar-chat-list flex-grow-1 overflow-auto px-2 pb-2" style="position: relative;">
          ...
          <div v-for="chat in chatHistory" :key="chat.chat_id" class="chat-history-item"
            ...
            @click="chat.isNew ? null : loadChat(chat.chat_id)" :title="chat.title">
    ...
    <div class="chat-main d-flex flex-column flex-grow-1" style="min-width: 0; position: relative;">
      ...
      <div class="chat-header d-flex align-items-center gap-2 px-3 py-2 flex-shrink-0">
        ...
        <button class="sidebar-toggle-btn flex-shrink-0" data-bs-toggle="tooltip" data-bs-placement="top"
          :title="debugStatus || 'Send chat debug dump to server'"
          :class="{ active: !!debugStatus }" :disabled="messages.length === 0"
          @click="sendDebug">
        ...
        />
        <button v-if="activeChatId" class="sidebar-toggle-btn ms-auto flex-shrink-0" title="Share" @click="shareConversation">
  ...
  try {
    await ntopng_utility.http_request(
      ...
      { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chatId: id, title, csrf: props.context.csrf }) }, true);
...
async function deleteChat(chatId) {
  ...
  try {
    await ntopng_utility.http_request(
      ...
      { method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chatId, csrf: props.context.csrf }) }, true);
...
:root[data-theme='dark'] .hljs-comment,:root[data-theme='dark'] .hljs-quote { color:#8b949e; font-style:italic; }
:root[data-theme='dark'] .hljs-keyword,:root[data-theme='dark'] .hljs-selector-tag,:root[data-theme='dark'] .hljs-deletion { color:#ff7b72; }
:root[data-theme='dark'] .hljs-string,:root[data-theme='dark'] .hljs-addition { color:#a5d6ff; }
:root[data-theme='dark'] .hljs-title,:root[data-theme='dark'] .hljs-section { color:#d2a8ff; }
:root[data-theme='dark'] .hljs-number,:root[data-theme='dark'] .hljs-literal { color:#f2cc60; }
:root[data-theme='dark'] .hljs-built_in,:root[data-theme='dark'] .hljs-type { color:#ffa657; }
:root[data-theme='dark'] .hljs-attr,:root[data-theme='dark'] .hljs-attribute { color:#7ee787; }
:root[data-theme='dark'] .hljs-variable,:root[data-theme='dark'] .hljs-template-variable { color:#e3b341; }
:root[data-theme='dark'] .hljs-punctuation { color:#c9d1d9; }

/* Shared markdown */
.llm-chat-page .markdown-body p:last-child { margin-bottom:0; }
...
.llm-chat-page .markdown-body pre.code-block code { background:none; padding:0; font-size:0.82em; color:var(--code-text); }
...
.llm-chat-page .markdown-body ul,.llm-chat-page .markdown-body ol { padding-left:1.4rem; margin-bottom:0.5rem; }
...
.llm-chat-page .markdown-body table { border-collapse:collapse; width:100%; margin:0.5rem 0; font-size:0.85em; }
.llm-chat-page .markdown-body th,.llm-chat-page .markdown-body td { border:1px solid var(--chat-border); padding:0.35rem 0.65rem; }
.llm-chat-page .markdown-body th { background:var(--inline-code-bg); color:var(--chat-text); font-weight:600; }
.llm-chat-page .markdown-body td { color:var(--chat-text); }
.llm-chat-page .markdown-body a { color:var(--ntop-orange, #FF8F00); }
.llm-chat-page .markdown-body h1,.llm-chat-page .markdown-body h2,.llm-chat-page .markdown-body h3,
.llm-chat-page .markdown-body h4,.llm-chat-page .markdown-body h5,.llm-chat-page .markdown-body h6 {
  color:var(--chat-text); margin-top:0.75rem; margin-bottom:0.35rem; font-weight:600;
}
.llm-chat-page .markdown-body hr { border:none; border-top:1px solid var(--chat-border); margin:0.75rem 0; }
</style>

<style scoped>
/* Page shell */
.llm-chat-page { border-radius:12px; overflow:hidden; border:1px solid var(--chat-border); }
...
.chat-sidebar.open { width:240px; }
.chat-sidebar-inner { width:240px; }
.sidebar-header { border-bottom:1px solid var(--sidebar-header-border); }
.sidebar-title { font-size:0.68rem; letter-spacing:0.06em; color:var(--chat-muted); }
.btn-icon-subtle {
  width:24px; height:24px; display:flex; align-items:center; justify-content:center;
  background:transparent; border:none; border-radius:6px; color:var(--chat-muted);
  font-size:0.75rem; cursor:pointer; transition:background 0.15s,color 0.15s; padding:0;
}
.btn-icon-subtle:hover { background:var(--sidebar-item-hover); color:var(--chat-text); }
.btn-new-chat {
  display:flex; align-items:center; justify-content:center; background:transparent;
  border:1px dashed var(--chat-border); border-radius:8px; color:var(--chat-muted);
  font-size:0.78rem; padding:0.35rem 0.5rem; cursor:pointer;
...
.chat-history-item:hover { background:var(--sidebar-item-hover); }
.chat-history-item.active { background:var(--sidebar-item-active); border-left-color:var(--sidebar-item-active-border); }
.chat-history-item.active .chat-history-title,.chat-history-item.active .chat-history-icon { color:var(--sidebar-item-active-text); }
.chat-history-icon { font-size:0.8rem; color:var(--chat-muted); width:16px; text-align:center; flex-shrink:0; }
.chat-history-title {
  font-size:0.78rem; color:var(--chat-text); white-space:nowrap; overflow:hidden;
  text-overflow:ellipsis; line-height:1.35; flex-grow:1; min-width:0;
}
.chat-history-actions { display:flex; gap:2px; opacity:0; transition:opacity 0.12s; pointer-events:none; }
.chat-history-item:hover .chat-history-actions,.chat-history-item.active .chat-history-actions { opacity:1; pointer-events:auto; }
.chat-item-action-btn {
  width:20px; height:20px; display:flex; align-items:center; justify-content:center;
  background:transparent; border:none; border-radius:4px; font-size:0.62rem; color:var(--chat-muted);
  cursor:pointer; padding:0; transition:background 0.12s,color 0.12s;
}
.chat-item-action-btn:hover { background:var(--sidebar-item-hover); color:var(--chat-text); }
.chat-item-delete-btn:hover { background:rgba(220,53,69,0.10); color:#dc3545; }
.rename-overlay {
  position:absolute; inset:0; z-index:200; background:rgba(0,0,0,0.18);
...
.rename-popup {
  ...
  border-radius:10px; box-shadow:0 8px 24px rgba(0,0,0,0.15); width:200px;
}
.rename-input { background:var(--input-bg) !important; color:var(--input-text) !important; border-color:var(--input-border) !important; }
.btn-rename-confirm {
  background:var(--send-btn-bg); color:#fff; border:none; border-radius:7px;
  font-size:0.78rem; padding:0.3rem 0; cursor:pointer; transition:background 0.15s;
}
.btn-rename-confirm:hover { background:var(--send-btn-hover); }
.btn-rename-cancel {
  background:transparent; border:1px solid var(--chat-border); border-radius:7px;
  font-size:0.78rem; padding:0.3rem 0.6rem; color:var(--chat-muted); cursor:pointer;
}

/* Header */
.chat-header { background:var(--chat-header-bg); border-bottom:1px solid var(--chat-border); }
.sidebar-toggle-btn {
...
/* Message area */
.chat-messages { background:var(--ntop-bg-color, #f8f9fa); position:relative; }
...
/* Empty state */
.empty-state-icon {
  width:60px; height:60px; border-radius:50%; background:var(--empty-icon-bg);
  display:flex; align-items:center; justify-content:center;
  color:var(--empty-icon-color); font-size:1.5rem;
}
.preset-grid { display:grid; grid-template-columns:repeat(2, 1fr); gap:8px; max-width:640px; margin:0 auto; }
.preset-chip {
  background:transparent; border:1px solid var(--chat-border); border-radius:20px;
  color:var(--chat-text); font-size:0.78rem; padding:0.35rem 0.9rem; cursor:pointer;
  transition:background 0.15s,border-color 0.15s,color 0.15s; white-space:normal; text-align:left;
}
.preset-chip:hover:not(:disabled) { background:rgba(255,143,0,0.10); border-color:var(--ntop-orange, #FF8F00); color:var(--ntop-orange, #FF8F00); }
.preset-chip:disabled { opacity:0.4; cursor:not-allowed; }

/* Chat bubbles — wider on the full page */
.chat-bubble { padding:0.65rem 0.9rem; border-radius:14px; max-width:90%; animation:fadeUp 0.18s ease-out; }
@keyframes fadeUp {
  from { opacity:0; transform:translateY(6px); }
  to   { opacity:1; transform:translateY(0); }
}
.user-bubble { background:var(--user-bubble-bg); color:#fff; box-shadow:0 2px 8px var(--user-bubble-shadow); border-bottom-right-radius:4px; }
.user-bubble .bubble-meta-user { color:rgba(255,255,255,0.7); }
.assistant-bubble { background:var(--assistant-bubble-bg); border:1px solid var(--assistant-bubble-border); box-shadow:0 1px 6px var(--assistant-bubble-shadow); color:var(--chat-text); border-bottom-left-radius:4px; }
.assistant-bubble .bubble-meta-assistant { color:var(--chat-muted); }
.error-bubble { background:var(--error-bubble-bg); border:1px solid var(--error-bubble-border); color:var(--error-bubble-text); border-bottom-left-radius:4px; }
.error-label { color:var(--error-bubble-text); }
.chat-avatar { width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:0.8rem; flex-shrink:0; }
...
/* Typing indicator */
.typing-dot { width:7px; height:7px; border-radius:50%; background:var(--chat-muted); opacity:0.5; animation:typingPulse 1.2s infinite ease-in-out; }
...
/* Scroll overlay buttons */
.scroll-overlay-btns {
  position:sticky; bottom:8px; right:8px; display:flex; flex-direction:column; gap:4px;
  width:fit-content; align-self:flex-end; pointer-events:none; margin-right:8px;
}
.scroll-overlay-btn {
  width:32px; height:32px; display:flex; align-items:center; justify-content:center;
  background:var(--provider-pill-bg); border:1px solid var(--chat-border); border-radius:50%;
  color:var(--chat-muted); font-size:0.72rem; cursor:pointer;
...
/* Footer */
.chat-footer { background:var(--chat-footer-bg); border-top:1px solid var(--chat-border); }
.chat-input {
...
.btn-send {
  background:var(--send-btn-bg); color:#fff; border:none; border-radius:10px; padding:0 1rem;
  font-size:0.85rem; font-weight:500; cursor:pointer; box-shadow:0 2px 8px var(--send-btn-shadow);
...
.timeout-alert { background:var(--timeout-bg); border:1px solid var(--timeout-border); color:var(--timeout-text); border-radius:8px; padding:0.4rem 0.75rem; }
