    ...
    <div class="card  card-shadow">
      ...
      <div class="card-body">
        <div id="manage_configurations_backup">          
          <Datatable ref="table_manage_configurations_backup"
          :id="table_config.id"
...
const format_flows_icon = function(data, rowData) {
  ...
  debugger;
  const utcSeconds = data.split(".")[3];
  const date = new Date(utcSeconds*1000); 
  return `<a href="${http_prefix}/lua/rest/v2/get/system/configurations/backup.lua?epoch=${utcSeconds}&download=true">${date}</a>`;
