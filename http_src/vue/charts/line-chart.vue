    ...
    <div class="line-body">
      ...
      <div v-if="!loading && tooltip.visible" class="line-tooltip"
        ...
        <div v-for="(row, i) in tooltip.rows" :key="i" class="tt-row">
          <span class="tt-dot" :style="row.present === false
            ? { background: 'transparent', border: '1.5px solid #888', boxSizing: 'border-box' }
            : { background: row.color }"></span>
          <span class="tt-series" :style="row.present === false ? { color: '#888' } : {}">{{ row.label }}</span>
          <span class="tt-sep">·</span>
          <span class="tt-val" :style="{ color: row.present === false ? '#888' : row.color }">
            {{ row.value != null ? row.value : (row.present === false ? 'No data' : 'Available') }}
          </span>
        </div>
      </div>
    </div>
    <div v-if="!loading && series.length && !no_data && !isPresence" class="line-legend">
...
function renderSwimlane(data, W, _H, iW, iH) {
  const MP = { top: 8, right: 20, bottom: 28, left: 64 };
  const swimH   = Math.max(16, Math.min(32, (iH - (data.length - 1) * 4) / Math.max(data.length, 1)));
  const totalH  = data.length * (swimH + 4) - 4;
  const svgH    = totalH + MP.top + MP.bottom;

  svgEl.attr("viewBox", `0 0 ${W} ${svgH}`).attr("height", svgH);
...
axX.selectAll(".tick text")
  ...
  const valFmt = formatterUtils.getFormatter(unit || "number", null, null, formatted_label) ?? (v => v);
...
function onSwimlaneMove(ev, data, ACTIVE_COLOR, MP) {
  ...
  const rows = data.map(s => {
    ...
    return { label: s.label, color: present ? ACTIVE_COLOR : "#888", present };
  });
