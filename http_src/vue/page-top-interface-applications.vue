...
function create_timeseries_url_link(row) {
    const epoch_begin = Math.floor(new Date().getTime() / 1000) - props.context.uptime
    const url = `${http_prefix}/lua/if_stats.lua?ifid=${props.context.ifid}&page=historical&epoch_begin=${epoch_begin}&epoch_end=${Math.floor(new Date().getTime() / 1000)}&ts_schema=top:${props.context.iface_ts_id}&ts_query=ifid:${props.context.ifid},protocol:${row.application.name}`
    return url;
}
