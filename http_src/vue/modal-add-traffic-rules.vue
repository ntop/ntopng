<!-- (C) 2022 - ntop.org     -->
<template>
    <modal @showed="showed()" ref="modal_id" :size="modal_size">
        ...
        <template v-slot:body>
            ...
            <div class="form-group ms-2 me-2 mt-3 row">
                ...
                <div class="col-sm-10">
                    <div class="btn-group btn-group-toggle" data-bs-toggle="buttons">
                        <label class="btn no-wrap " :class="[rule_type == 'Host' ? 'btn-primary active' : 'btn-secondary']">
                            <input class="btn-check" type="radio" name="rule_type" value="hosts"
                                @click="set_rule_type('Host')"> {{
                                    _i18n("if_stats_config.add_rules_type_host") }}
                        </label>
                        <label class="btn no-wrap "
                            :class="[rule_type == 'interface' ? 'btn-primary active' : 'btn-secondary']">
                        ...
                        <label v-if="flow_device_timeseries_available == true && isExporterInterface == true"
                            class="btn no-wrap " :class="[rule_type == 'exporter' ? 'btn-primary active' : 'btn-secondary']">
                            <input @click="set_rule_type('exporter')" class="btn-check" type="radio" name="rule_type"
                                value="exporter">
                            {{ _i18n("if_stats_config.add_rules_type_flow_exporter") }}
                        </label>
                        <label v-if="has_host_pools == true" class="btn no-wrap "
                        ...
                        <label v-if="has_cidr == true" class="btn no-wrap "
                            :class="[rule_type == 'CIDR' ? 'btn-primary active' : 'btn-secondary']">
                            <input @click="set_rule_type('CIDR')" class="btn-check" type="radio" name="rule_type"
                                value="CIDR"> {{
                                    _i18n("if_stats_config.add_rules_type_cidr") }}
                        </label>
                        <label v-if="props.has_vlans == true" class="btn no-wrap "
                            :class="[rule_type == 'vlan' ? 'btn-primary active' : 'btn-secondary']">
                            <input @click="set_rule_type('vlan')" class="btn-check" type="radio" name="rule_type"
                                value="vlan"> {{
                                    _i18n("if_stats_config.add_rules_type_vlans") }}
                        </label>
                        <label v-if="props.has_profiles == true" class="btn no-wrap "
                            :class="[rule_type == 'profiles' ? 'btn-primary active' : 'btn-secondary']">
                            <input @click="set_rule_type('profiles')" class="btn-check" type="radio" name="rule_type"
                                value="profiles"> {{
                                    _i18n("if_stats_config.add_rules_type_profiles") }}
                        </label>
                        <label v-if="props.has_asn == true" class="btn no-wrap "
                            :class="[rule_type == 'asn' ? 'btn-primary active' : 'btn-secondary']">
                            <input @click="set_rule_type('asn')" class="btn-check" type="radio" name="rule_type"
                                value="asn"> {{
            ...
            <div v-if="rule_type == 'Host'" class="form-group ms-2 me-2 mt-3 row">
                ...
                <div class="col-sm-10">
                    <input v-model="host" @input="check_empty_host" class="form-control" type="text"
            ...
            <div v-if="rule_type == 'exporter' && flow_device_timeseries_available == true && isExporterInterface == true"
                ...
                <label class="col-form-label col-sm-2 mt-3">
                ...
                <template v-else-if="rule_type == 'host_pool'">
                    <div class="col-10">
                        <SelectSearch v-model:selected_option="selected_host_pool_metric"
                            @select_option="change_metric_type_hp()" :options="host_pool_metric_list">
                        </SelectSearch>
                    </div>
                </template>
                ...
                <template v-else-if="rule_type == 'CIDR'">
                    <div class="col-10">
                        <SelectSearch v-model:selected_option="selected_network_metric"
                            @select_option="change_metric_type_hp()" :options="network_metric_list">
                        </SelectSearch>
                    </div>
                </template>
                <template v-else-if="rule_type == 'vlan'">
                    <div class="col-10">
                        <SelectSearch v-model:selected_option="selected_vlan_metric" :options="vlan_metric_list"
                            @select_option="change_vlan_threshold">
                        </SelectSearch>
                    </div>
                </template>
                <template v-else-if="rule_type == 'profiles'">
                    <div class="col-10">
                        <SelectSearch v-model:selected_option="selected_profile_metric" :options="profiles_metric_list">
                        </SelectSearch>
                    </div>
                </template>
                <template v-else-if="rule_type == 'asn'">
                    <div class="col-10">
                        <SelectSearch v-model:selected_option="selected_asn_metric" :options="asn_metric_list"
                            @select_option="change_asn_threshold">
                        </SelectSearch>
                    </div>
                </template>
            </div>
            ...
            <div class="form-group ms-2 me-2 mt-3 row" style="margin-top:3px">
                ...
                <template v-if="visible">
                    ...
                    <div class="col-3" :class="[metric_type.id == 'throughput' ? 'p-0' : '']">
                        <div class="btn-group float-end btn-group-toggle" data-bs-toggle="buttons">
                            <template v-if="metric_type.id == 'throughput'"
                                v-for="measure in throughput_threshold_list">
                                <input :value="measure.value" :id="measure.id" type="radio" class="btn-check"
                                    autocomplete="off" ref="threshold_measure" name="threshold_measure">
                                <label class="btn " :id="measure.id" @click="set_active_radio"
                                    v-bind:class="[measure.active ? 'btn-primary active' : 'btn-secondary']"
                                    :for="measure.id">{{
                                    measure.label }}</label>
                            </template>
                            <template v-if="metric_type.id == 'percentage'"
                                v-for="measure in percentage_threshold_list">
                                <input :value="measure.value" :id="measure.id" type="radio" class="btn-check"
                                    autocomplete="off" ref="threshold_measure" name="threshold_measure">
                                <label class="btn " :id="measure.id" @click="set_active_radio"
                                    v-bind:class="[measure.active ? 'btn-primary active' : 'btn-secondary']"
                    ...
                    <div class="col-sm-2 btn-group float-end btn-group-toggle" data-bs-toggle="buttons">
                        <template v-for="measure in sign_threshold_list">
                            <input :value="measure.value" :id="measure.id" type="radio" class="btn-check"
                                autocomplete="off" ref="threshold_sign" name="threshold_sign">
                ...
                <div :class="[visible ? 'col-sm-2' : 'col-sm-8']">
                    <template v-if="metric_type.id == 'percentage'">
                        <input value="1" ref="threshold" type="number" name="threshold" class="form-control" max="100"
                            min="1" required>
                    </template>
                    <template v-else>
                        <input value="1" ref="threshold" type="number" name="threshold" class="form-control" max="1023"
                            min="1" required>
                    </template>
                </div>
            </div>
        ...
        <template v-slot:footer>
            ...
            <template v-if="is_edit_page == false">
                <button type="button" @click="add_" class="btn btn-primary"
                    :disabled="disable_add && rule_type == 'Host'">{{
                        _i18n('add') }}</button>
            </template>
            <template v-else>
...
const flow_exp_ts_available_url = "/lua/pro/rest/v2/get/flowdevice/timeseries_enabled.lua" 
...
async function update_exporter_interfaces() {
    ...
    const url_device_exporter_details =
        NtopUtils.buildURL(`${http_prefix}/lua/pro/rest/v2/get/exporters/exporters_interfaces.lua?exporter_uuid=${selected_exporter_device.value.id}&ifid=${selected_exporter_device.value.ifid}&ip=${selected_exporter_device.value.ip}`);
...
onBeforeMount(async () => {
    ...
    invalid_add.value = false;
    const rsp = await ntopng_utility.http_request(flow_exp_ts_available_url)
    if (rsp && rsp.exporterTimeseriesEnabled != null) {
        flow_device_timeseries_available.value = rsp.exporterTimeseriesEnabled;
    }
})
