...
void usage() {
  printf("ntopng %s%s v.%s - " NTOP_COPYRIGHT "\n\n"
   ...
   "                                    | Add an '=' followed by a name to give that network\n"
   "                                    | An alias. It is not possible to modify by the gui\n"
   "                                    | the alias of a network setted using the command line.\n"
   "                                    | (e.g. -m \"192.168.1.0/24=local_network_1\")\n"
...
void Prefs::reloadPrefsFromRedis() {
  ...
  housekeeping_frequency      = getDefaultPrefsValue(CONST_RUNTIME_PREFS_HOUSEKEEPING_FREQUENCY, HOUSEKEEPING_FREQUENCY),
    ...
    max_num_days_before_delete_alert = getDefaultPrefsValue(CONST_MAX_NUM_DAYS_ALERTS_BEFORE_DEL, ALERTS_MAX_DAYS_BEFORE_PURGE), 
...
void Prefs::lua(lua_State* vm) {
  ...
  lua_push_uint64_table_entry(vm, "max_num_days_before_delete_alert", max_num_days_before_delete_alert);
