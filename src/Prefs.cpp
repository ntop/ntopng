...
Prefs::Prefs(Ntop *_ntop) {
  ...
  enable_dns_resolution = sniff_dns_responses = sniff_name_responses = true;
...
void Prefs::refreshBehaviourAnalysis() {
  ...
  devices_learning_period = getDefaultPrefsValue(CONST_PREFS_DEVICES_ANALYSIS_LEARNING_PERIOD, CONST_DEVICES_LEARNING_TIME);
}
	...
	*elastic_url = NULL, *elastic_user = NULL, *elastic_pwd = NULL;
	...
	if(dump_flows_on_clickhouse) {
	  ...
	  if(!client_found) {
	    ...
	    ntop->getTrace()->traceEvent(TRACE_WARNING, "Expected %s or %s", CLICKHOUSE_CLIENT, CLICKHOUSE_ALT_CLIENT);
...
void Prefs::lua(lua_State* vm) {
  ...
  lua_push_bool_table_entry(vm, "flow_table_probe_order", flow_table_probe_order);
  ...
  lua_push_uint64_table_entry(vm, "devices_learning_period", devices_learning_period);
