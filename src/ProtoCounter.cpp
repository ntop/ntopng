...
void ProtoCounter::print(u_int16_t proto_id, NetworkInterface *iface) {
  if(bytes.getTotal())
    printf("[%s] [pkts: %llu/%llu][bytes: %llu/%llu][duration: %u sec]\n",
	   ...
	   (long long unsigned)packets.getSent(), (long long unsigned)packets.getRcvd(),
	   (long long unsigned)bytes.getSent(),   (long long unsigned)bytes.getRcvd(),
	   duration);
}

/* *************************************************/

void ProtoCounter::lua(lua_State* vm, NetworkInterface *iface,
		       bool tsLua, bool diff) {
  ...
  if(name != NULL) {	
	...
	lua_push_str_table_entry(vm, "breed", iface->get_ndpi_proto_breed_name(proto_id));
...
#ifdef NTOPNG_PRO
	if(behavior_bytes_traffic)
	  behavior_bytes_traffic->luaBehavior(vm, "l7_traffic_behavior",
	...
	if(bytes_thpt) {
	  ...
	  lua_insert(vm, -2); lua_rawset(vm, -3);
	}
...
void ProtoCounter::updateStats(const struct timeval *tv, time_t nextMinPeriodicUpdate) {
  if(!bytes_thpt)
    bytes_thpt = new (std::nothrow)ThroughputStats();
  ...
  if(tv->tv_sec >= nextMinPeriodicUpdate) {
    if(!behavior_bytes_traffic)
      behavior_bytes_traffic = new (std::nothrow)BehaviorAnalysis(0.9 /* Alpha parameter */, 0.1 /* Beta parameter */, 0.05 /* Significance */, true /* Counter */);
    
    if(behavior_bytes_traffic)
      behavior_bytes_traffic->updateBehavior(NULL, bytes.getSent() + bytes.getRcvd(), NULL, false);    
  }
#endif
}
...
void ProtoCounter::incStats(u_int32_t when,
...
void ProtoCounter::addProtoJson(json_object *my_object, NetworkInterface *iface) {
  ...
  json_object_object_add(inner1, "sent", json_object_new_int64(bytes.getSent()));
  json_object_object_add(inner1, "rcvd", json_object_new_int64(bytes.getRcvd()));
  json_object_object_add(inner,  "bytes", inner1);

  inner1 = json_object_new_object();
  if(!inner1) { json_object_put(inner); return; }
  
  json_object_object_add(inner1, "sent", json_object_new_int64(packets.getSent()));
  json_object_object_add(inner1, "rcvd", json_object_new_int64(packets.getRcvd()));
...
#ifdef NTOPNG_PRO
  if(behavior_bytes_traffic)
    behavior_bytes_traffic->resetStats();
#endif
