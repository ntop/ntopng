...
static void address_tree_serialize_funct(ndpi_patricia_node_t* node, void* data,
  ...
  if (user_data)
    ndpi_serialize_string_uint64((ndpi_serializer*)user_data, address,
                                 ndpi_patricia_get_node_u64(node));
  else
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "[AddressTree] %s", address);
}
...
char* AddressTree::serialize(char* buf, u_int buf_len) {
  ...
  ndpi_patricia_walk_tree_inorder(ptree_v4, address_tree_serialize_funct,
                                  &serializer);

  if (ptree_v6)
    ndpi_patricia_walk_tree_inorder(ptree_v6, address_tree_serialize_funct,
                                    &serializer);

  ndpi_serialize_end_of_block(&serializer);

  ndpi_serialize_start_of_list(&serializer, "mac");
  for (std::map<u_int64_t, int64_t>::iterator it = macs.begin();
       it != macs.end(); ++it) {
    char key[32], val[8];
    u_int8_t* mac = (u_int8_t*)&it->first;
