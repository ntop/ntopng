...
nDPIStats::~nDPIStats() {
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    ...
    if (counters[i] != NULL) free(counters[i]);

    if (bytes_thpt && bytes_thpt[i]) delete bytes_thpt[i];
  }
  ...
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    if (counters[i] != NULL) {
      if (counters[i]->bytes.sent || counters[i]->bytes.rcvd)
        printf(
            "[%s] [pkts: %llu/%llu][bytes: %llu/%llu][duration: %u sec][thpt: "
            "%.2f]\n",
            ...
            (long long unsigned)counters[i]->bytes.rcvd, counters[i]->duration,
      ...
      if (cat_counters[i].bytes.sent + cat_counters[i].bytes.rcvd) {
        ...
        if (!tsLua) {
          ...
          lua_push_uint64_table_entry(
              vm, "bytes",
              cat_counters[i].bytes.sent + cat_counters[i].bytes.rcvd);
          lua_push_uint64_table_entry(vm, "bytes.sent",
...
void nDPIStats::updateStats(const struct timeval *tv) {
  if (!bytes_thpt) return;

  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    if (!counters[i]) continue;

    if (!bytes_thpt[i]) bytes_thpt[i] = new (std::nothrow) ThroughputStats();

    if (bytes_thpt[i])
      bytes_thpt[i]->updateStats(
          tv, counters[i]->bytes.sent + counters[i]->bytes.rcvd);

#ifdef NTOPNG_PRO
    if (tv->tv_sec >= nextMinPeriodicUpdate) {
      if (!behavior_bytes_traffic) continue;
...
void nDPIStats::incFlowsStats(u_int16_t proto_id) {
  if (proto_id < (MAX_NDPI_PROTOS)) {
    if (counters[proto_id] != NULL) counters[proto_id]->total_flows++;
  }
}
...
json_object *nDPIStats::getJSONObject(NetworkInterface *iface) {
  ...
  for (int proto_id = 0; proto_id < MAX_NDPI_PROTOS; proto_id++) {
    if (counters[proto_id] != NULL) {
      ...
      if (name != NULL) addProtoJson(my_object, counters[proto_id], name);
    }
  }
...
void nDPIStats::resetStats() {
  for (int i = 0; i < MAX_NDPI_PROTOS; i++) {
    ...
    if (counters[i] != NULL) {
      ...
      if (bytes_thpt && bytes_thpt[i]) bytes_thpt[i]->resetStats();
    }
  }
