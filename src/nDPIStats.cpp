...
nDPIStats::nDPIStats(bool _enable_throughput_stats, bool _enable_behavior_stats) {
#ifdef NTOPNG_PRO
  nextMinPeriodicUpdate = 0;
#endif
  enable_throughput_stats = _enable_throughput_stats, enable_behavior_stats = _enable_behavior_stats;
}
...
nDPIStats::nDPIStats(nDPIStats &stats) {
  ...
  for(it = stats.counters.begin(); it != stats.counters.end(); ++it) {
    ...
    if(c != NULL) {
      std::unordered_map<u_int16_t, ProtoCounter *>::iterator cbr = counters.find(it->first);

      if(cbr == counters.end()) {
	ProtoCounter *p = cbr->second;
	ProtoCounter *pc = new (std::nothrow) ProtoCounter(it->first,
#ifdef NTOPNG_PRO
...
void nDPIStats::sum(nDPIStats *stats) {
  ...
  for(it = stats->counters.begin(); it != stats->counters.end(); ++it) {
    ...
    ProtoCounter *c = it->second;
    std::unordered_map<u_int16_t, ProtoCounter*>::iterator it1 = counters.find(proto_id);
  ...
  for(it1 = stats->cat_counters.begin(); it1 != stats->cat_counters.end(); ++it1) {
...
void nDPIStats::incStats(u_int32_t when, u_int16_t proto_id,
  ...
  std::unordered_map<u_int16_t, ProtoCounter *>::iterator cbr = counters.find(proto_id);
  ProtoCounter *pc;
...
void nDPIStats::incCategoryStats(u_int32_t when, ndpi_protocol_category_t category_id,
  ...
  std::unordered_map<u_int16_t, CategoryCounter>::iterator it = cat_counters.find(category_id);;
...
void nDPIStats::incFlowsStats(u_int16_t proto_id) {
  std::unordered_map<u_int16_t, ProtoCounter *>::iterator pc = counters.find(proto_id);

  if(pc != counters.end())
    pc->second->inc_total_flows();
}
