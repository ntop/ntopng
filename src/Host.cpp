...
Host::~Host() {
  ...
  */
  ...
  if(rare_dest_revise) ndpi_bitmap_free(rare_dest_revise);
...
void Host::dumpRareDestToRedis() {
  ...
  if((!redis) || (!iface) || (!rare_dest) || (!rare_dest_revise)) return;
  ...
  snprintf(param_ser, sizeof(param_ser), "%lu", size);
