...
void Host::lua(lua_State* vm, AddressTree* ptree, bool host_details,
  ...
  if (!veryBasicInfo) {
    ...
    lua_push_str_table_entry(vm, "iphex",
                             ip.get_ip_hex(buf_id, sizeof(buf_id)));
    ...
    if (stats)
      stats->lua(vm, mask_host,
                 Utils::bool2DetailsLevel(verbose, host_details)),
    ...
    if (more_then_one_device)
      lua_push_bool_table_entry(vm, "more_then_one_device",
                                more_then_one_device);
      ...
      */
      ...
      if (strlen(buf) == 0 || strcmp(buf, ipaddr) == 0) {
        if (isBroadcastHost() || isMulticastHost() ||
           ...
           * address queue */
          ntop->getRedis()->pushHostToResolve(
              ipaddr, false, true /* Fake to resolve it ASAP */);
        }
      }
...
char* Host::get_visual_name(char* buf, u_int buf_len) {
  ...
  if ((sym_name == NULL) || (sym_name[0] == '\0')) return (buf);
...
bool Host::addIfMatching(lua_State* vm, AddressTree* ptree, char* key) {
  ...
  if (strcasestr((ipbuf_ptr = Utils::formatMac(m ? m->get_mac() : NULL, ipbuf,
      ...
      strcasestr(getDHCPName(ipbuf, sizeof(ipbuf)), key)) { /* Match by name */
...
TrafficShaper* Host::get_shaper(ndpi_protocol ndpiProtocol) {
...
#ifdef SHAPER_DEBUG
  {
    ...
    ntop->getTrace()->traceEvent(
        ...
        ip.print(buf, sizeof(buf)), vlan_id, ndpiProtocol.proto.app_protocol,
...
void Host::offlineSetMDNSInfo(char* const str) {
  ...
  if (names.mdns_info || !str || (str[0] == '_')) return; /* Already set */
...
bool Host::setOS(ndpi_os _os, OSLearningMode mode) {
  ...
  if (_os == ndpi_os_unknown) return (false);
  ...
  snprintf(msg, sizeof(msg), "%s (%s)", Utils::OS2Str(_os),
           Utils::learningMode2str(mode));
  ...
  if ((cur_mac != NULL) && (cur_mac->getDeviceType() == device_unknown)) {
    ...
    if (dt != device_unknown) {
      if (getDeviceType() == device_unknown) setDeviceType(dt);
...
void Host::serializeAttributes(ndpi_serializer* serializer) {
  ...
  ndpi_serialize_string_string(serializer, "name",
                               get_visual_name(buf, sizeof(buf)));
  ...
  ndpi_serialize_string_boolean(serializer, "broadcast_domain_host",
                                isBroadcastDomainHost());
  ...
  ndpi_serialize_string_boolean(serializer, "drop_all_host_traffic",
                                dropAllTraffic());
#endif
}
...
void Host::alert2JSON(HostAlert* alert, bool released, ndpi_serializer* s) {
  ...
  /* rowid is used by engaged (in-memory) alerts only to delete them when
   * disengaged */
...
bool Host::triggerAlert(HostAlert* alert) {
  ...
  if (hasCheckEngagedAlert(alert->getCheckType())) {
    if (getCheckEngagedAlert(alert->getCheckType()) == alert) {
      ...
      // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Refreshing alert already
      // engaged"
  ...
  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Triggering alert engaged with
  // ID = %llu", alert->getRowID());
...
void Host::releaseAlert(HostAlert* alert) {
  ...
  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "Releasing alert engaged with ID
  // = %llu", alert->getRowID());
...
void Host::toggleRxOnlyHost(bool rx_only) {
...
#ifdef DEBUG
  {
    ...
    ntop->getTrace()->traceEvent(
        TRACE_WARNING, "%s [is-rx-only: %s][isUnicast(): %s][isLocal: %s]",
        print(buf, sizeof(buf)), rx_only ? "true" : "false",
        isUnicastHost() ? "true" : "false", isLocalHost() ? "true" : "false");
  }
#endif
...
#ifdef DEBUG
  {
    ...
    ntop->getTrace()->traceEvent(
        TRACE_WARNING, "%s [is-rx-only: %s] [%u rx-only hosts]",
        print(buf, sizeof(buf)), rx_only ? "true" : "false",
...
void Host::setnDPIOS(ndpi_os _os) {
  if (os_type != ndpi_os_unknown) {
    ...
    if (os_type == _os)
      ...
      /* ntop->getTrace()->traceEvent(TRACE_WARNING, "OS change detected [%u ->
       * %u]", os_type, _os); */
