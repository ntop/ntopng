	...
	if((!strcasecmp(level, "authNoPriv")) || (!strcasecmp(level, "authPriv"))) {
	  ...
	  if(!strcasecmp(level, "authPriv")) {
	    ...
	    if(!strcasecmp(privacy_protocol, "DES")) {
	      snmpSession->session.securityPrivProto = snmp_duplicate_objid(usmDESPrivProtocol,
									    USM_PRIV_PROTO_DES_LEN);
	      snmpSession->session.securityPrivProtoLen = USM_PRIV_PROTO_DES_LEN;
	    } else
#endif
	      if(!strncasecmp(privacy_protocol, "AES", 3)) {
	      snmpSession->session.securityPrivProto = snmp_duplicate_objid(usmAESPrivProtocol,
