...
Categorization::Categorization(char *_api_key) {
  ...
  ntop->getTrace()->traceEvent(TRACE_NORMAL, "Host categorizazion successfully enabled. Using API key: %s", api_key);
}
...
void Categorization::categorizeHostName(char *_url, char *buf, u_int buf_len) {
  ...
  if (ntop->getPrefs()->is_categorization_enabled()) {
    ...
    snprintf(key, sizeof(key), "%s.%s", DOMAIN_CATEGORY, _url); // Where is DOMAIN_CATEGORY declared?
        ...
        if (replyCode == 200) {
          ...
          if (replyCode == 204) {
            ...
            if (REQUEST_REPLY[0] == '\0') {
              snprintf(REQUEST_REPLY, sizeof(REQUEST_REPLY), "reliable");
            }
          }
          else {
            if (replyCode == 400) {
...
void* Categorization::categorizeLoop() {
  ...
  while (!ntop->getGlobals()->isShutdown()) {
    ...
    if((rc == 0) && (domain_name[0] != '\0')) {
      char buf[256]; // buf must have the same size of REQUEST_REPLY in categorizeHostName function.
