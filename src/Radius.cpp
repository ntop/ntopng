...
bool Radius::authenticate(const char *user, const char *password, bool *has_unprivileged_capabilities, bool *is_admin) {
  /* Reset the return */
  radius_ret = 0;
  ...
  /* rh initialization, initialize the 'Radius Header' with all the key-value required */
...
bool Radius::updateLoginInfo() {
  ...
  if(((!radiusServer) && !(radiusServer = (char*)calloc(sizeof(char), MAX_RADIUS_LEN))) ||
      ...
      ((!radiusUnprivCapabilitiesGroup) && !(radiusUnprivCapabilitiesGroup = (char*)calloc(sizeof(char), MAX_RADIUS_LEN))) ||
  ...
  ntop->getTrace()->traceEvent(TRACE_DEBUG, "Radius: server - %s | secret - %s | admin group - %s | capabilities group - %s", 
    radiusServer, 
    radiusSecret, 
    radiusAdminGroup, 
    radiusUnprivCapabilitiesGroup);
...
bool Radius::startSession() {
  return true;
}

/* *************************************** */

bool Radius::stopSession() {
  return true;
}

/* *************************************** */

bool Radius::updateSession() {
  return true;
}
