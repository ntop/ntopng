...
float ContinuousPingStats::getSuccessRate(float *min_rtt, float *max_rtt,
  ...
  float pctg = min(
      (stats.num_ping_sent == 0)
          ? 0
          : (float)(stats.num_ping_rcvd * 100) / (float)(stats.num_ping_sent),
      100.f);

  *min_rtt = stats.min_rtt, *max_rtt = stats.max_rtt;
  *mean = (stats.num_ping_rcvd == 0) ? 0 : stats.rtt_sum / stats.num_ping_rcvd;
  *jitter = (stats.num_ping_rcvd < 2)
                ? 0
                : (stats.diff_sum / (stats.num_ping_rcvd - 1));

#ifdef TRACE_PING
  ntop->getTrace()->traceEvent(
      TRACE_NORMAL,
      "[sent: %u][rcvd: %u][RTT: %.3f/%.3f][Mean/Jitter: %.3f / %.3f]",
      stats.num_ping_sent, stats.num_ping_rcvd, stats.min_rtt, stats.max_rtt,
      *mean, *jitter);
#endif
