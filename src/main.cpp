...
#endif
{
  ...
  */
  if (sqlite3_config(SQLITE_CONFIG_MULTITHREAD) != SQLITE_OK) {
    ntop->getTrace()->traceEvent(
        TRACE_WARNING,
        "Unable to set SQLITE_CONFIG_MULTITHREAD for sqlite, exiting.");
    exit(1);
  }
  sqlite3_initialize();
#endif
  ...
  if ((argc >= 2) && (argv[1][0] != '-')) {
    ...
    if (argc > 2) rc = prefs->loadFromCLI(argc, argv);
  } else
#endif
  ...
  if (prefs->is_appliance()) {
    ...
    ntop->getTrace()->traceEvent(TRACE_WARNING,
                                 "Disabling user change in appliance mode");
  ...
  ntop->getTrace()->traceEvent(TRACE_INFO, "System has %d CPU cores",
                               ntop->getNumCPUs());
#endif
  ...
  for (int i = 0; i < MAX_NUM_INTERFACE_IDS; i++) {
    ...
    (!defined(__i686__)) && (!defined(__ARM_ARCH))
...
#endif
      {
        ...
        if ((iface == NULL) && (strncmp(ifName, "ntap:", 3) == 0) &&
      ...
      ntop->getTrace()->traceEvent(
          TRACE_NORMAL,
          "Unable to open interface %s [%d]: %s. Falling back to pcap.", ifName,
          err, err == EAFNOSUPPORT ? "PF_RING not loaded" : strerror(err));
  ...
  if (ntop->getFirstInterface() == NULL) {
    ...
    ntop->getTrace()->traceEvent(TRACE_ERROR,
                                 ...
                                 "https://nmap.org/npcap/ driver");
#else
    ntop->getTrace()->traceEvent(
        TRACE_ERROR, "Startup error: missing super-user privileges ?");
#endif
  ...
  if (prefs->get_pid_path() != NULL) {
    ...
    if (fd != NULL) {
      ...
      if (n > 0) {
        ...
        ntop->getTrace()->traceEvent(
            TRACE_ERROR,
            "The PID file %s is empty: is your disk full perhaps ?",
            prefs->get_pid_path());
    } else
      ntop->getTrace()->traceEvent(TRACE_ERROR,
                                   "Unable to store PID in file %s: %s",
  ...
  */
  ...
  if ((fd = fopen(path, "w")) == NULL) {
    ntop->getTrace()->traceEvent(TRACE_ERROR,
                                 "Unable to write on %s as '%s' [%s]: %s. "
                                 "Please specify a different directory (-d)",
                                 ntop->get_working_dir(), prefs->get_user(),
                                 path, strerror(errno));
    exit(EXIT_FAILURE);
  } else {
  ...
  if (prefs->is_log_to_file_enabled()
    ...
    Utils::mkdir_tree(ntop->get_working_dir());
    snprintf(path, sizeof(path), "%s/ntopng.log",
             ntop->get_working_dir() /* "C:\\Windows\\Temp" */);
    ntop->fixPath(path);
  ...
  /* This method returns after a shutdown has been requested.
     runHousekeepingTasks() is performed within this method until it has
     returned */
  ntop->start();
