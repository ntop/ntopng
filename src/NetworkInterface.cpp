  ...
  if((srcMac = getMac(eth->h_source, true /* Create if missing */, true /* Inline call */))) {
    ...
    if(host_pools
      ...
      if(!host_pools->findMacPool(srcMac->get_mac(), &mac_pool) || (mac_pool == NO_HOST_POOL_ID)) {
        ...
        host_pools->addToPool(mac_str, ntop->getPrefs()->get_auto_assigned_pool_id());
      }
    }
#endif
  }
 ...
 post_get_flow:
  if(flow == NULL) {
    ...
    if(flow->is_swap_requested()
       && ((src2dst_direction && flow->get_packets_srv2cli()) /* Wait until at least a packet is observed in both directions */
	...
	swapped_flow->setDetectedProtocol(flow->get_detected_protocol());
      ...
      flow = swapped_flow;   /* Swap the pointers */
      goto post_get_flow;    /* Start over */
    }
