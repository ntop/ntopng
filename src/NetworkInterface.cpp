...
bool NetworkInterface::nDPILoadIPCategory(char *what, ndpi_protocol_category_t id, char *list_name) {
  bool success = true;

  // ntop->getTrace()->traceEvent(TRACE_NORMAL, "%s(%p) [%s]", __FUNCTION__, ndpi_struct_shadow, what);

  if(what && ndpi_struct_shadow)
    success = (ndpi_load_ip_category(ndpi_struct_shadow, what, id, (void*)list_name) == 0);
...
void NetworkInterface::deleteDataStructures() {
  ...
  if(obs_hash)              { delete(obs_hash);   obs_hash = NULL;  }
...
bool NetworkInterface::walker(u_int32_t *begin_slot,
  ...
  case walker_obs:
    ret = obs_hash ? obs_hash->walk(begin_slot, walk_all, walker, user_data) : false;
    break;
 ...
 decode_packet_eth:
  ...
  case ETHERTYPE_IP:
	...
	vlan_id = (ip6 ? ip6->ip6_src.u6_addr.u6_addr8[15] +
		   ip6->ip6_dst.u6_addr.u6_addr8[15] : iph->saddr + iph->daddr) & 0xFFF;
...
void NetworkInterface::findFlowHosts(u_int16_t vlan_id, u_int16_t observation_domain_id,
				     u_int32_t private_flow_id,
  ...
  if((*src) == NULL) {
    ...
    if(_src_ip
      ...
      (*src) = new (std::nothrow) LocalHost(this, src_mac, vlan_id, observation_domain_id, _src_ip);
      INTERFACE_PROFILING_SECTION_EXIT(4);
    } else {
      INTERFACE_PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new RemoteHost", 5);
      (*src) = new (std::nothrow) RemoteHost(this, src_mac, vlan_id, observation_domain_id, _src_ip);
  ...
  if((*dst) == NULL) {
    ...
    if(_dst_ip
      ...
      INTERFACE_PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new LocalHost", 4);
      (*dst) = new (std::nothrow) LocalHost(this, dst_mac, vlan_id, observation_domain_id, _dst_ip);
      INTERFACE_PROFILING_SECTION_EXIT(4);
    } else {
      INTERFACE_PROFILING_SECTION_ENTER("NetworkInterface::findFlowHosts: new RemoteHost", 5);
      (*dst) = new (std::nothrow) RemoteHost(this, dst_mac, vlan_id, observation_domain_id, _dst_ip);
      INTERFACE_PROFILING_SECTION_EXIT(5);
    }
...
static bool find_obs_point_by_id(GenericHashEntry *he, void *user_data, bool *matched) {
  struct obs_point_find_info *info = (struct obs_point_find_info*)user_data;
  ObservationPoint *_obs_point = (ObservationPoint*)he;

  if((info->obs_point == NULL) && info->obs_point_id == _obs_point->getObsPoint()) {
...
static bool obs_point_search_walker(GenericHashEntry *he, void *user_data, bool *matched) {
  struct flowHostRetriever *r = (struct flowHostRetriever*)user_data;
...
int NetworkInterface::getFlows(lua_State* vm,
  ...
  if(sortFlows(begin_slot, walk_all, &retriever, allowed_hosts, host, client, server, flow_info, p, sortColumn) < 0)
    return(-1);
...
int NetworkInterface::getFlowsGroup(lua_State* vm,
  ...
  if(sortFlows(&begin_slot, walk_all, &retriever, allowed_hosts, NULL, NULL, NULL, NULL, p, groupColumn) < 0) {
    return(-1);
  }
...
int NetworkInterface::sortObsPoints(struct flowHostRetriever *retriever, char *sortColumn) {
  int (*sorter)(const void *_a, const void *_b);
  u_int32_t begin_slot = 0;
  bool walk_all = true;
  ...
  retriever->actNumEntries = 0,
    retriever->maxNumEntries = getObsHashSize();
  retriever->elems = (struct flowHostRetrieveList*)calloc(sizeof(struct flowHostRetrieveList), retriever->maxNumEntries);
  ...
  if((!strcmp(sortColumn, "column_obs_point")) || (!strcmp(sortColumn, "column_"))) retriever->sorter = column_obs_point, sorter = numericSorter;
  else if(!strcmp(sortColumn, "column_score"))        retriever->sorter = column_score,        sorter = numericSorter;
  else if(!strcmp(sortColumn, "column_since"))        retriever->sorter = column_since,        sorter = numericSorter;
  else if(!strcmp(sortColumn, "column_thpt"))         retriever->sorter = column_thpt,         sorter = numericSorter;
  else if(!strcmp(sortColumn, "column_traffic"))      retriever->sorter = column_traffic,      sorter = numericSorter;
  ...
  // make sure the caller has disabled the purge!!
  walker(&begin_slot, walk_all,  walker_obs, obs_point_search_walker, (void*)retriever);
...
void NetworkInterface::lua_hash_tables_stats(lua_State *vm) {
  ...
  GenericHash *gh[] = {
    ...
    vlans_hash, ases_hash, oses_hash, countries_hash, obs_hash
  };
...
VLAN* NetworkInterface::getVLAN(u_int16_t u_int16_t, bool create_if_not_present, bool isInlineCall) {
  VLAN *ret = NULL;
...
ObservationPoint* NetworkInterface::getObsPoint(u_int16_t obs_point, bool create_if_not_present, bool is_inline_call) {
...
int NetworkInterface::getActiveObsPointsList(lua_State* vm, const Paginator *p) {
  struct flowHostRetriever retriever;
  DetailsLevel details_level;
...
bool NetworkInterface::compute_protocol_flow_stats(GenericHashEntry *node, void *user_data, bool *matched) {
  ...
  /* <0 (16 bit)><u_int16_t (16 bit)><app_protocol (16 bit)><master_protocol (16 bit) */
...
static bool asc_str_info_cmp(FlowsStats* a, FlowsStats* b) {
...
void NetworkInterface::build_lua_rsp(lua_State *vm, FlowsStats *fs,
				     u_int filter_type, u_int32_t size, u_int *num) {
  char buf[128];
  bool add_client = false, add_server = false, add_app_proto = false, add_info = false;
  ...
  if(add_info) {
    lua_push_str_table_entry(vm,    "info",  fs->getInfoKey() );
...
void NetworkInterface::getFilteredLiveFlowsStats(lua_State* vm) {
  ...
  std::unordered_map<string, FlowsStats*> info_count;
