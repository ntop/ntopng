...
void NetworkInterface::processFlow(ZMQ_Flow *zflow) {
  ...
  */
  ...
  incStats(true /* ingressPacket */,
	   now, zflow->src_ip.isIPv4() ? ETHERTYPE_IP : ETHERTYPE_IPV6,
...
int NetworkInterface::getActiveMacList(lua_State* vm,
  ...
  if(sortMacs(&retriever, bridge_iface_idx, vlan_id, sourceMacsOnly,
	      hostMacsOnly, manufacturer, sortColumn, pool_filter, devtype_filter, location_filter) < 0) {
    enablePurge(false);
    return -1;
  }
...
int NetworkInterface::getActiveMacManufacturers(lua_State* vm,
  ...
  if(sortMacs(&retriever, bridge_iface_idx, vlan_id, sourceMacsOnly,
	      hostMacsOnly, NULL, (char*)"column_manufacturer", (u_int16_t)-1, devtype_filter, location_filter) < 0) {
    enablePurge(false);
    return -1;
  }
...
int NetworkInterface::getActiveDeviceTypes(lua_State* vm,
  ...
  if(sortMacs(&retriever, bridge_iface_idx, vlan_id, sourceMacsOnly,
	      hostMacsOnly, manufacturer, (char*)"column_device_type", (u_int16_t)-1, (u_int8_t)-1, location_filter) < 0) {
    enablePurge(false);
    return -1;
  }
