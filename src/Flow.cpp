...
Flow::Flow(NetworkInterface *_iface,
  ...
  */
  ...
  synTime.tv_sec = synTime.tv_usec = 0, ackTime.tv_sec = ackTime.tv_usec = 0,
  synAckTime.tv_sec = synAckTime.tv_usec = 0, rttSec = 0,
  cli2srv_window = srv2cli_window = 0,
  ...
  memset(&tcp_seq_s2d, 0, sizeof(tcp_seq_s2d)),
      memset(&tcp_seq_d2s, 0, sizeof(tcp_seq_d2s));
...
void Flow::incFlowDroppedCounters() {
  if (!flow_dropped_counts_increased) {
    ...
    if (h) {
      ...
      if (cli_pool != NO_HOST_POOL_ID) h->incPoolNumDroppedFlows(cli_pool);
    }
#endif
...
void Flow::periodic_stats_update(const struct timeval *tv) {
  ...
  get_partial_traffic_stats(&periodic_stats_update_partial, &partial,
                            &first_partial);
  ...
  if (hp) {
    ...
    if (cli_host
      ...
      if (ndpiDetectedProtocol.app_protocol != NDPI_PROTOCOL_UNKNOWN &&
        ...
        hp->incPoolStats(tv->tv_sec, cli_host_pool_id,
                         ndpiDetectedProtocol.master_protocol, category_id,
                         diff_sent_packets, diff_sent_bytes, diff_rcvd_packets,
                         diff_rcvd_bytes);
      ...
      if (hp->enforceQuotasPerPoolMember(cli_host_pool_id)) {
        cli_host->incQuotaEnforcementStats(
            tv->tv_sec, ndpiDetectedProtocol.master_protocol, diff_sent_packets,
            diff_sent_bytes, diff_rcvd_packets, diff_rcvd_bytes);
        cli_host->incQuotaEnforcementStats(
            tv->tv_sec, ndpiDetectedProtocol.app_protocol, diff_sent_packets,
            diff_sent_bytes, diff_rcvd_packets, diff_rcvd_bytes);
    ...
    if (srv_host
      ...
      if (hp->enforceQuotasPerPoolMember(srv_host_pool_id)) {
        srv_host->incQuotaEnforcementStats(
            tv->tv_sec, ndpiDetectedProtocol.master_protocol, diff_rcvd_packets,
            diff_rcvd_bytes, diff_sent_packets, diff_sent_bytes);
        srv_host->incQuotaEnforcementStats(
            tv->tv_sec, ndpiDetectedProtocol.app_protocol, diff_rcvd_packets,
            diff_rcvd_bytes, diff_sent_packets, diff_sent_bytes);
...
void Flow::lua(lua_State *vm, AddressTree *ptree, DetailsLevel details_level,
  ...
  if (details_level >= details_high) {
    ...
    if (prevAdjacentAS)
      lua_push_int32_table_entry(vm, "prev_adjacent_as", prevAdjacentAS);
    if (nextAdjacentAS)
      lua_push_int32_table_entry(vm, "next_adjacent_as", nextAdjacentAS);
...
void Flow::formatGenericFlow(json_object *my_object) {
  ...
  json_object_object_add(my_object,
      Utils::jsonLabel(CONNECTION_STATE, "CONNECTION_STATE",
                                        jsonbuf, sizeof(jsonbuf)),
                        json_object_new_int(calculateConnectionState()));
  if (isHTTP()) {
...
void Flow::alert2JSON(FlowAlert *alert, ndpi_serializer *s) {
  ...
  ndpi_serialize_string_int32(
      s, "l7_master_proto",
      detection_completed ? ndpiDetectedProtocol.master_protocol : -1);
  ndpi_serialize_string_int32(
      s, "l7_proto",
  ...
  */
  ...
  if (last_conntrack_update > 0) {
    /*
      ...
      - the conntrack scan time that we  assume is less than
      MIN_CONNTRACK_UPDATE
  ...
  if (src2dst_direction) {
    if (debug)
      ntop->getTrace()->traceEvent(TRACE_WARNING,
                                   ...
                                   tcp_seq_s2d.last, tcp_seq_s2d.next);
        ...
        if ((seq_num == tcp_seq_s2d.next - 1) &&
          ...
          if (debug)
            ntop->getTrace()->traceEvent(
                ...
                tcp_seq_s2d.last, seq_num);
        } else {
          cnt_ooo++;
          update_last_seqnum =
              ((seq_num - 1) > tcp_seq_s2d.last) ? true : false;
          if (debug)
            ntop->getTrace()->traceEvent(
    ...
    if (debug)
      ntop->getTrace()->traceEvent(TRACE_WARNING,
                                   ...
                                   tcp_seq_d2s.last, tcp_seq_d2s.next);
        ...
        if ((seq_num == tcp_seq_d2s.next - 1) &&
          ...
          if (debug)
            ntop->getTrace()->traceEvent(
                ...
                tcp_seq_d2s.last, seq_num);
        } else {
          cnt_ooo++;
          update_last_seqnum =
              ((seq_num - 1) > tcp_seq_d2s.last) ? true : false;
          if (debug)
            ntop->getTrace()->traceEvent(TRACE_WARNING,
                                         "[dst2src] [last: %u][next: %u]",
                                         tcp_seq_d2s.last, tcp_seq_d2s.next);
...
bool Flow::updateDirectionShapers(bool src2dst_direction,
...
void Flow::updateFlowShapers(bool first_update) {
  ...
  if ((!first_update) && (iface->getIfType() == interface_type_NETFILTER) &&
      (((old_verdict != passVerdict)) || (old_cli2srv_in != cli2srv_in) ||
       (old_cli2srv_out != cli2srv_out) || (old_srv2cli_in != srv2cli_in) ||
...
#ifdef SHAPER_DEBUG
  {
    ...
    ntop->getTrace()->traceEvent(TRACE_NORMAL, "[SHAPERS] %s",
                                 print(buf, sizeof(buf)));
  }
#endif
}
  ...
  if (cli_host && srv_host) {
    ...
    if ((above_quota =
      ...
      srv_src = policy_source_default;
                  ...
                  srv_host->checkQuota(ndpiDetectedProtocol, &srv_src, now)))
      ;
...
void Flow::setProtocolJSONInfo() {
  if (!json_protocol_info) {
    ...
    if (ndpi_init_serializer(json_serializer, ndpi_serialization_format_json) ==
        -1) {
...
void Flow::lua_get_flow_connection_state(lua_State *vm) {
  lua_push_uint64_table_entry(vm, "conn_state", calculateConnectionState());
}

/* **************************************************** */

bool Flow::checkS1ConnState() {
return((src2dst_tcp_flags & TCP_3WH_MASK) == (TCP_3WH_MASK) && 
      (dst2src_tcp_flags & TCP_3WH_MASK) == (TCP_3WH_MASK) &&                            /* 3WH OK */
      (src2dst_tcp_flags & (TH_FIN | TH_ACK)) != (TH_FIN | TH_ACK) &&                  /* NO FIN ACK in src2dst */
      (src2dst_tcp_flags & TH_RST) != (TH_RST) && (dst2src_tcp_flags & TH_RST) != (TH_RST) /* NO RST */);
}

/* **************************************************** */

Flow::ConnectionStates Flow::calculateConnectionState() {
  
  if(!isTCP())
    return(setCurrentConnectionState(NO_STATE)); 

  /* Check S0 or RSTOS0 or REJ or SH */
  if ((src2dst_tcp_flags & TH_SYN) == (TH_SYN) && 
      (dst2src_tcp_flags & TH_SYN) != (TH_SYN)) {
    if ((dst2src_tcp_flags & TH_RST) != (TH_RST)) {
      if ((src2dst_tcp_flags & TH_RST) == (TH_RST)) {
        return(setCurrentConnectionState(RSTOS0));
      } else {
        if ((src2dst_tcp_flags & TH_FIN) == (TH_FIN)) {
  ...
  /* Check RSTRH */
  if ((dst2src_tcp_flags & (TH_SYN | TH_ACK | TH_RST)) == (TH_SYN | TH_ACK | TH_RST) && 
      (src2dst_tcp_flags & TH_SYN) != (TH_SYN)) 
    return(setCurrentConnectionState(RSTRH));

  /* Check SHR */
  if (((dst2src_tcp_flags & (TH_SYN | TH_ACK | TH_FIN)) == (TH_SYN | TH_ACK | TH_FIN) && 
      (src2dst_tcp_flags & TH_SYN) != (TH_SYN)) )
  ...
  bool is_possible_s1 = checkS1ConnState();
  /* Check SF */
  if ((is_possible_s1) &&
      (src2dst_tcp_flags & (TH_FIN | TH_ACK)) != (TH_FIN | TH_ACK) &&
      (dst2src_tcp_flags & (TH_FIN | TH_ACK)) != (TH_FIN | TH_ACK))
    return(setCurrentConnectionState(SF));

  /* Check S2 */
  if ((is_possible_s1) && 
      (src2dst_tcp_flags & TH_FIN) == (TH_FIN) && 
      (dst2src_tcp_flags & TH_FIN) != (TH_FIN))
  ...
  /* Check RSTO */
  if ((is_possible_s1) && 
      (src2dst_tcp_flags & TH_RST) == (TH_RST))
  ...
  // FOR ZMQ PARSER TODO: MAYBE FIX? 
  if (getCurrentConnectionState() != NO_STATE)
