...
void Flow::formatGenericFlow(json_object *my_object) {
  ...
  json_object_object_add(my_object, Utils::jsonLabel(FIRST_SWITCHED, "FIRST_SWITCHED", jsonbuf, sizeof(jsonbuf)), json_object_new_int(get_partial_first_seen()));
  json_object_object_add(my_object, Utils::jsonLabel(LAST_SWITCHED, "LAST_SWITCHED", jsonbuf, sizeof(jsonbuf)),   json_object_new_int(get_partial_last_seen()));
...
void Flow::updateDNSFlood(const struct bpf_timeval *when, bool src2dst_direction) {
  if(cli_host) cli_host->updateDNSAlertsCounter(when->tv_sec, src2dst_direction);
  if(srv_host) srv_host->updateDNSAlertsCounter(when->tv_sec, !src2dst_direction);
}
