...
void Fingerprint::lua(const char *key, lua_State *vm) {
  ...
  for (std::map<std::string, FingerprintStats>::const_iterator it = fp.begin();
       it != fp.end(); ++it) {
    lua_newtable(vm);
...
void Fingerprint::prune() {
  if (fp.size() > MAX_NUM_FINGERPRINT) {
    ...
    while (it != fp.end()) {
      if (it->second.num_uses < 3) dropper.push_back(it->first);

      ++it;
    } /* while */

    for (std::vector<std::string>::iterator it1 = dropper.begin();
         it1 != dropper.end(); ++it1)
      fp.erase(*it1);
  }
}
