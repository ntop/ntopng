...
int Utils::ping(string target){
	...
	if(to.sin_addr.s_addr != (u_int)-1){
		...
		if(!hp){
			ntop->getTrace()->traceEvent(TRACE_ERROR, "unknown host %s\n", target.c_str());
			return -1;
		}
	...
	if((packet = (u_char *)malloc((u_int)packlen)) == NULL){
		ntop->getTrace()->traceEvent(TRACE_ERROR, "malloc error\n");
		return -1;
	}
	...
	if(i < 0 || i != cc){
		...
		ntop->getTrace()->traceEvent(TRACE_INFO, "wrote %s %d chars, ret= %d\n", hostname.c_str(), cc, i);
	}
