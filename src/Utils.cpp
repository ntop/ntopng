...
static cap_value_t cap_values[] = {
    ...
    CAP_NET_BIND_SERVICE /* Listen on non-privileges ports (e.g. UDP 162 for
                            traps) */
};
...
void Utils::lua_getpaths_recursively(lua_State* vm, const char* path,
                                     const char* filename) {
  DIR* d = opendir(path);
...
int Utils::ifname2id(const char* name) {
  ...
  if (ntop->getRedis()) {
    if (ntop->getRedis()->hashGet((char*)CONST_IFACE_ID_PREFS, (char*)name, rsp,
                                  sizeof(rsp)) == 0) {
      /* Found */
      return (atoi(rsp));
    } else {
...
bool Utils::isValidUTF8(const u_char* param, size_t length) {
  ...
  while (i < length) {
    ...
    if (byte < 0x80) {
      ...
      // 3-byte character
      if (i + 2 >= length || (param[i + 1] >> 6) != 0b10 ||
          (param[i + 2] >> 6) != 0b10) {
        return (false);  // Invalid continuation byte
      }
      i += 3;
      ...
      // 4-byte character
      if (((i + 3) >= length) || ((param[i + 1] >> 6) != 0b10) ||
          ((param[i + 2] >> 6) != 0b10) || ((param[i + 3] >> 6) != 0b10)) {
        return (false);  // Invalid continuation byte
      }
      i += 4;
    } else {
...
static void readCurlStats(CURL* curl, HTTPTranferStats* stats, lua_State* vm) {
...
#else
  // TODO: Deprecated with new curl versions, working on current ntopng curl
  // version, use the commented lines above Deprecated since curl 7.55.0 ->
  // https://curl.se/libcurl/c/CURLINFO_SIZE_DOWNLOAD.html
  ...
  ntop->getTrace()->traceEvent(
      ...
      "[REDIRECT_TIME %.02f][STARTTRANSFER_TIME %.02f][TOTAL_TIME "
      "%.02f][BYTES_DOWNLOAD_T %.02f][BYTES_UPLOAD_T %.02f]",
      stats->namelookup, stats->connect, stats->appconnect, stats->pretransfer,
      stats->redirect, stats->start, stats->total, stats->bytes_download,
      stats->bytes_upload);
}
...
bool Utils::sendMail(lua_State* vm, char* from, char* to, char* cc,
  ...
  retry_sendMail:
    ...
    if (verbose)
      ntop->getTrace()->traceEvent(TRACE_NORMAL, "Adding from: %s", from);

    recipients = curl_slist_append(recipients, to);
    if (verbose)
      ntop->getTrace()->traceEvent(TRACE_NORMAL, "Adding to: %s", to);
    ...
    if (cc && cc[0]) {
      ...
      if (ccs) {
        ...
        while (rec != NULL) {
          if (verbose)
            ntop->getTrace()->traceEvent(TRACE_NORMAL, "Adding cc: %s", rec);
    ...
    if (res != CURLE_OK) {
      ...
      if (ntop->getTrace()->get_trace_level() < TRACE_LEVEL_DEBUG && !verbose)
        ntop->getTrace()->traceEvent(TRACE_WARNING,
                                     "Run ntopng with -v6 for more details");
      ret = false;
    }
...
char* Utils::getURL(char* url, char* buf, u_int buf_len) {
...
#else
  snprintf(buf, buf_len, "%s/lua%s", ntop->get_HTTPserver()->get_scripts_dir(),
           &url[4]);
#endif
  ...
  if ((stat(buf, &s) == 0) && (S_ISREG(s.st_mode))) {
    ...
    /* ntop->getTrace()->traceEvent(TRACE_WARNING, "===>>> Missing %s [%s]",
     * url, buf); */
    return (url);
  }
}
...
char* Utils::formatMacAddress(const u_int8_t* const mac, char* buf,
                              u_int buf_len) {
  if (mac == NULL)
    buf[0] = '\0';
  else
...
u_int16_t Utils::getIfMTU(const char* ifname) {
  ...
  if (stat(ifname, &buf) == 0) return (CONST_MAX_PACKET_SIZE);
...
int Utils::retainWriteCapabilities() {
  ...
  /* CAP_PERMITTED: It is a superset for the effective capabilities that the
     process may assume. If the capability is available in this set, a process
     transitions it to an effective set and drops it later. But once a process
     has dropped capability from the permitted set, it can not re-aquire
  */
  ...
  /* CAP_INHERITABLE: This set is reserved for execve() syscall. If the
     capability is set to inheritable, it will be added permitted set when the
     program is executed with execve() syscall */
  if (cap_set_flag(caps, CAP_INHERITABLE, num_cap, cap_values, CAP_SET) == -1)
    ntop->getTrace()->traceEvent(
        TRACE_WARNING, "Capabilities cap_set_flag error: %s", strerror(errno));

  /* CAP_EFFECTIVE: Capabilities used by the kernel to perform permission checks
   * for the thread */
...
static int _setWriteCapabilities(int enable) {
  ...
  if (caps) {
#ifdef TRACE_CAPABILITIES
    ntop->getTrace()->traceEvent(
        TRACE_NORMAL, "[CAPABILITIES] BEFORE [enable: %u][%s][tid: %u]", enable,
        cap_to_text(caps, NULL), gettid());
#endif
    ...
    if (rc != -1)
      ntop->getTrace()->traceEvent(
          TRACE_NORMAL, "[CAPABILITIES] AFTER  [enable: %u][%s][tid: %u]",
    ...
    ntop->getTrace()->traceEvent(TRACE_WARNING,
                                 "Capabilities cap_get_proc error");
    rc = -1;
  }
...
#if !defined(__APPLE__) && !defined(__FreeBSD__)
  if (ntop && !ntop->hasDroppedPrivileges()) return (-99);
...
u_int32_t Utils::findInterfaceGatewayIPv4(const char* ifname) {
  ...
  /* Note: netstat is deprecated (e.g. not available on latest Ubuntu), try with
   * ip */
  if (rc == 0) {
    snprintf(cmd, sizeof(cmd),
             "ip -4 route list 0.0.0.0/0 dev %s 2>/dev/null | awk '{print $3}'",
             ifname);
...
bool Utils::execCmd(char* cmd, std::string* out) {
  ...
  if (ntop->getGlobals()->isShutdownRequested()) {
    ntop->getTrace()->traceEvent(TRACE_WARNING,
                                 "Skipping command during shutdown (%s)", cmd);
    return (false);
  }
  ...
  *hint = ndpi_os_unknown;
  ...
  if (strcasestr(os, "iPhone")) {
    ...
    *hint = ndpi_os_windows;
    return (device_workstation);
  } else if (strcasestr(os, "Linux") || strcasestr(os, "Debian") ||
...
void Utils::deferredExec(const char* command) {
  ...
  if (system(command_buf) == -1)
    fprintf(stderr, "Failed command %s: %d/%s", command_buf, errno,
            strerror(errno));
}
#endif
...
void Utils::tlv2serializer(ndpi_serializer* tvl_serializer,
...
char* Utils::ifname2devname(const char* ifname, char* devname,
  ...
  if (colon) {
    ...
    prefix[len] = '\0';
    if (!pfring_is_module_prefix(prefix)) colon = NULL;
  }
#endif
...
int Utils::pollSocket(int sock, int timeout /* msec */) {
  ...
  if (sock > FD_SETSIZE) {
    ntop->getTrace()->traceEvent(
        TRACE_WARNING, "Unable to poll socket %d (too many file descriptors)");
    return -1;
  }
...
/* Poll a list of sockets and bahaves like a poll() (same timeout and return
 * value) Sockets which are not ready are cleared from the array (set to -1) */
int Utils::pollSockets(int socks[], u_int num, int timeout /* msec */) {
  ...
  for (int i = 0; i < num; i++) {
    if (socks[i] > FD_SETSIZE) {
      ntop->getTrace()->traceEvent(
          TRACE_WARNING,
          "Unable to poll socket %d (too many file descriptors)");
      return -1;
    }
  ...
  for (int i = 0; i < num; i++)
    if (rc <= 0 || !FD_ISSET(socks[i], &rset)) socks[i] = -1;
#else
  ...
  if (num > max_num_sockets) {
    ntop->getTrace()->traceEvent(
        TRACE_WARNING, "Too many file descriptors %u/%u", num, max_num_sockets);
    num = max_num_sockets;
  }
  ...
  for (u_int i = 0; i < num; i++)
    if (rc <= 0 || !(pfd[i].revents & POLLIN)) socks[i] = -1;
#endif
...
bool Utils::nwInterfaceExists(char* if_name) {
  ...
  if ((colon = strchr(base_if, ':')) != NULL) *colon = '\0';
        ...
        if ((response_len + 7) <= len) {
          ...
          while (num_objects > 0) {
            ...
            if ((object_len + offset) < len) {
              switch (object_id) {
                case 0:
                  strncpy(buf, &response[offset],
                          l = ndpi_min(sizeof(buf) - 1, object_len));
                  buf[l] = '\0';
...
bool Utils::readEthernetIPDeviceInfo(char* device_ip, u_int8_t timeout_sec,
                                     lua_State* vm) {
  struct hostent* server = NULL;
  struct sockaddr_in serv_addr;
  int sockfd = -1;
  ...
  retval = sendto(sockfd, (const char*)etherip_query, sizeof(etherip_query), 0,
    ...
    if (ret > 0) {
      ...
      if (len > 24 /* EtherIP/TCP Len */) {
        ...
        if ((response_id == 0x63) /* List Indentity */
            && ((response_len + 24) <= len)) {
          u_int16_t offset = 24;
          u_int16_t num_objects =
              /* ntohs */ (*((u_int16_t*)&response[offset]));

          while (num_objects > 0) {
            u_int16_t type_id =
                /* ntohs */ (*((u_int16_t*)&response[offset + 2]));
            u_int8_t l;
            u_int16_t object_len =
                /* ntohs */ (*((u_int16_t*)&response[offset + 4]));

            if ((type_id == 0x0c /* CIP Identity */) &&
                ((object_len + offset) < len)) {
              offset += 6;
              l = /* ntohs */ (*((u_int16_t*)&response[offset]));
              offset += 2;
              ...
              if (l == 1 /* Protocol version */) {
                ...
                if (!found) lua_push_int32_table_entry(vm, "vendor_id", l);
                ...
                snprintf(str, sizeof(str), "%u.%02u", response[offset],
                         response[offset + 1]);
                offset += 2;
                ...
                u32 = /* ntohl */ (*((u_int32_t*)&response[offset]));
                offset += 4;
...
bool Utils::checkNetworkList(char* network_list, char* rsp,
                             ...
                             void* user_data) {
  /* callback is a function that receive a network as argument and a pointer to
   * rsp, used in case of errors, to give a feedback and return a boolean, if
   * the callback worked or not
   */
...
bool Utils::setProcessLimit(int resource, u_int32_t upper_limit) {
  ...
  new_limit.rlim_cur =
      (rlim_t)upper_limit; /* Soft limit (enforced by the kernel) */
  new_limit.rlim_max =
      (rlim_t)upper_limit; /* Hard limit (max value the soft limit can reach) */

  if (setrlimit(RLIMIT_NOFILE, &new_limit) == -1) {
    ntop->getTrace()->traceEvent(
        TRACE_WARNING,
        "Failed to set resource limit [resource: %d][limit: %u]: %s", resource,
        upper_limit, strerror(errno));
    return (false);
  }

  ntop->getTrace()->traceEvent(
      TRACE_INFO, "Set resource limit succeeded [resource: %d][limit: %u]",
...
/*
  ...
  however in some cases data is produced live and send to the socket
  directly from the backend, thus HTTP headers and other data from Lua
  needs to be sent/flushed.
*/
void Utils::flushHTTPBuffer(lua_State *vm) {
