...
// TODO: handle IPv6
/* Code from https://github.com/fgrehm/pucrs-ping (written by fgrehm) */
int Utils::ping(string target){
	int s, i, cc, packlen, datalen = PING_DEFDATALEN;
	struct hostent *hp;
	struct sockaddr_in to, from;
	//struct protoent   *proto;
	struct ip *ip;
	u_char *packet, outpack[PING_MAXPACKET];
	...
	if(to.sin_addr.s_addr != (u_int)-1){
		...
		if(!hp){
			ntop->getTrace()->traceEvent(TRACE_WARNING, "Unknown host %s: aborting ping operation\n", target.c_str());
			return -1;
		}
	...
	packlen = datalen + PING_MAXIPLEN + PING_MAXICMPLEN;
	if((packet = (u_char *)malloc((u_int)packlen)) == NULL){
		ntop->getTrace()->traceEvent(TRACE_WARNING, "Not enough memory left: aborting ping operation\n");
		return -1;
	}
	...
	if(i < 0 || i != cc){
		...
		ntop->getTrace()->traceEvent(TRACE_DEBUG, "Wrote %s %d chars, ret= %d", hostname.c_str(), cc, i);
	}
	...
	while(cont){
		...
		if(ret < (hlen + ICMP_MINLEN)){
			ntop->getTrace()->traceEvent(TRACE_WARNING, "Packet too short (%i bytes) from %s", ret, hostname.c_str());
			return -1;
		}
