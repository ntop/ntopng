...
static int ntop_get_grouped_interface_hosts(lua_State* vm) {
  ...
  if(lua_type(vm, 8) == LUA_TNUMBER)  pool_filter    = (u_int16_t)lua_tonumber(vm, 9);
  if(lua_type(vm, 9) == LUA_TNUMBER) ipver_filter   = (u_int8_t)lua_tonumber(vm, 10);
...
static int ntop_lua_require(lua_State* L) {
  ...
  if(script_path == "" ||
     __ntop_lua_handlefile(L, (char *)script_path.c_str(), true))
    return 0;
