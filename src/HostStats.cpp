...
void HostStats::getJSONObject(json_object *my_object, DetailsLevel details_level) {
  if(details_level >= details_high) {
    ...
    json_object_object_add(my_object, "anomalous_flows.as_client", json_object_new_int(anomalous_flows_as_client));
    json_object_object_add(my_object, "anomalous_flows.as_server", json_object_new_int(anomalous_flows_as_server));
...
void HostStats::lua(lua_State* vm, bool mask_host, bool host_details, bool verbose) {
  ...
  if(host_details) {
    ...
    lua_push_uint64_table_entry(vm, "anomalous_flows.as_client", anomalous_flows_as_client);
    lua_push_uint64_table_entry(vm, "anomalous_flows.as_server", anomalous_flows_as_server);
